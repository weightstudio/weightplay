(function (root) {
  ...
  const TITLES = {
    ...
    cribbage: { en: "Cribbage", "zh-Hant": "克里比奇", "zh-Hans": "克里比奇", ja: "クリベッジ", ko: "크리비지", es: "Cribbage", "pt-BR": "Cribbage", fr: "Criblage", de: "Cribbage", it: "Cribbage", ru: "Криббедж", hi: "क्रिबेज", ar: "كريبدج" },
  ...
  const HEARTS_SHELL_COPY = {
    ar: {
      ...
      preview: "لعب متاح للعامة",
      previewCopy: "العب القلوب مجاناً في المتصفح من دون حساب أو شراء. مرّر ثلاث بطاقات، واتبع النوع، وتجنب بطاقات الجزاء خلال جولة خدع كاملة.",
      faq: "الأسئلة الشائعة",
      faqQuestion: "هل يُحفظ التقدم؟",
      faqAnswer: "نعم، في هذا المتصفح فقط.",
      opponents: ["أنت", "أوركيد", "مانجو", "نوفا"],
      lead: "تبدأ أول خدعة ببطاقة 2♣",
      leadBy: "{name} يقود الخدعة التالية",
      metaDescription: "العب القلوب مجاناً في المتصفح: مرّر ثلاث بطاقات واتبع النوع وتجنب بطاقات الجزاء في جولة خدع كاملة.",
    },
  };
  ...
  const CRIB_PEGGING_RESET_COPY = {
    ...
    ar: { go: "تنهي جو هذا العد ويعود إلى 0. ابحث عن زوج أو تسلسل أو 15 أو 31 في السلسلة التالية.", thirtyOne: "ينهي 31 هذا العد ويعود إلى 0. ابحث عن زوج أو تسلسل أو 15 أو 31 آخر في السلسلة التالية." },
  };
  ...
  const CRIB_PEGGING_SCORE_COPY = {
    ...
    ar: { join: " + ", reasons: { fifteen: "15", thirtyOne: "31", pair: "زوج", three: "ثلاثية", four: "رباعية", run: "تسلسل من {length}" }, earned: "نقاط العدّ +{points}: {reasons}. العدّ {count}/31.", zero: "نقاط العدّ +0. العدّ {count}/31.", go: "جو: +1 نقطة. يعود العدّ إلى 0." },
  };
  ...
  const GO_FISH_BATTLE_COPY = {
    en: { heading: "How to play", paragraph: "Complete four-of-a-kind books. Choose two, three, or four players." },
    "zh-Hant": { heading: "遊戲玩法", paragraph: "完成四張同點數牌的組牌。可選擇兩人、三人或四人。" },
    "zh-Hans": { heading: "游戏玩法", paragraph: "完成四张同点数牌的组牌。可选择两人、三人或四人。" },
    ja: { heading: "遊び方", paragraph: "同じランク4枚の組を完成させます。2〜4人で遊べます。" },
    ko: { heading: "게임 방법", paragraph: "같은 랭크 네 장 세트를 완성하세요. 2·3·4인 게임으로 플레이할 수 있습니다." },
    es: { heading: "Cómo jugar", paragraph: "Completa grupos de cuatro cartas del mismo rango. Puedes elegir 2, 3 o 4 jugadores." },
    "pt-BR": { heading: "Como jogar", paragraph: "Complete grupos de quatro cartas do mesmo valor. Escolha 2, 3 ou 4 jogadores." },
    fr: { heading: "Comment jouer", paragraph: "Complétez des familles de quatre cartes du même rang. Vous pouvez choisir 2, 3 ou 4 joueurs." },
    de: { heading: "Spielanleitung", paragraph: "Bilde Vierlinge aus vier Karten desselben Rangs. Wähle 2, 3 oder 4 Spieler." },
    it: { heading: "Come si gioca", paragraph: "Completa combinazioni di quattro carte dello stesso valore. Puoi scegliere 2, 3 o 4 giocatori." },
    ru: { heading: "Как играть", paragraph: "Соберите четвёрки из четырёх карт одного ранга. Можно выбрать 2, 3 или 4 игроков." },
    hi: { heading: "कैसे खेलें", paragraph: "एक ही रैंक के चार पत्तों का सेट पूरा करें। 2, 3 या 4 खिलाड़ी चुनें।" },
    ar: { heading: "طريقة اللعب", paragraph: "أكمل مجموعات من أربع بطاقات من الرتبة نفسها. يمكنك اختيار لاعبين أو ثلاثة أو أربعة." },
  };
  ...
  const CRAZY_EIGHTS_COPY = {
    en: { summary: "Non-wild cards by suit: {counts}. A suit with more cards can keep more options open.", draw: "Drew 1 card — hand now {hand}; draws this round: {draws}.", illegal: "That card does not match the active suit or rank. Choose another card or draw.", suits: ["Clubs", "Diamonds", "Hearts", "Spades"] },
    "zh-Hant": { summary: "各花色的非萬用牌：{counts}。手上牌較多的花色，通常能保留更多選擇。", draw: "抽到 1 張牌——目前手牌 {hand} 張；本局抽牌 {draws} 次。", illegal: "這張牌不符合目前花色或點數。請選另一張牌或抽牌。", suits: ["梅花", "方塊", "紅心", "黑桃"] },
    "zh-Hans": { summary: "各花色的非万能牌：{counts}。手中牌较多的花色，通常能保留更多选择。", draw: "抽到 1 张牌——当前手牌 {hand} 张；本局抽牌 {draws} 次。", illegal: "这张牌不符合当前花色或点数。请选择另一张牌或抽牌。", suits: ["梅花", "方块", "红心", "黑桃"] },
    ja: { summary: "ワイルドではないカードのスート別枚数：{counts}。枚数の多いスートを選ぶと、次の選択肢を残しやすくなります。", draw: "1枚引きました。手札は{hand}枚、今回のドローは{draws}回です。", illegal: "そのカードは現在のスートにもランクにも合いません。別のカードを選ぶか、引いてください。", suits: ["クラブ", "ダイヤ", "ハート", "スペード"] },
    ko: { summary: "와일드가 아닌 카드의 무늬별 수: {counts}. 카드가 많은 무늬를 고르면 다음 선택지를 더 남길 수 있습니다.", draw: "카드 1장을 뽑았습니다 — 현재 손패 {hand}장, 이번 라운드 뽑기 {draws}회.", illegal: "이 카드는 현재 무늬나 숫자와 맞지 않습니다. 다른 카드를 고르거나 뽑으세요.", suits: ["클럽", "다이아몬드", "하트", "스페이드"] },
    es: { summary: "Cartas no comodín por palo: {counts}. Elegir un palo con más cartas puede mantener más opciones abiertas.", draw: "Robaste 1 carta: ahora tienes {hand}; robos en esta ronda: {draws}.", illegal: "Esa carta no coincide con el palo ni el rango activos. Elige otra carta o roba.", suits: ["tréboles", "diamantes", "corazones", "picas"] },
    "pt-BR": { summary: "Cartas não coringa por naipe: {counts}. Escolher um naipe com mais cartas pode manter mais opções abertas.", draw: "Você comprou 1 carta — agora tem {hand}; compras nesta rodada: {draws}.", illegal: "Essa carta não corresponde ao naipe nem ao valor ativos. Escolha outra carta ou compre.", suits: ["paus", "ouros", "copas", "espadas"] },
    fr: { summary: "Cartes non jokers par couleur : {counts}. Choisir une couleur plus présente peut garder davantage d'options.", draw: "Vous avez pioché 1 carte — main de {hand}; pioches dans cette manche : {draws}.", illegal: "Cette carte ne correspond ni à la couleur ni à la valeur actives. Choisissez une autre carte ou piochez.", suits: ["trèfles", "carreaux", "cœurs", "piques"] },
    de: { summary: "Nicht-Wildkarten nach Farbe: {counts}. Eine Farbe mit mehr Karten kann mehr Möglichkeiten offenhalten.", draw: "1 Karte gezogen — jetzt {hand} auf der Hand; Ziehungen in dieser Runde: {draws}.", illegal: "Diese Karte passt weder zur aktiven Farbe noch zum Rang. Wähle eine andere Karte oder ziehe.", suits: ["Kreuz", "Karo", "Herz", "Pik"] },
    it: { summary: "Carte non jolly per seme: {counts}. Scegliere un seme più numeroso può lasciare più opzioni aperte.", draw: "Hai pescato 1 carta — ora ne hai {hand}; pescate in questa mano: {draws}.", illegal: "Questa carta non corrisponde né al seme né al valore attivo. Scegli un'altra carta o pesca.", suits: ["fiori", "quadri", "cuori", "picche"] },
    ru: { summary: "Обычные карты по мастям: {counts}. Масть с большим числом карт может сохранить больше вариантов.", draw: "Вы взяли 1 карту — теперь карт на руке: {hand}; доборов в раунде: {draws}.", illegal: "Эта карта не подходит к активной масти или рангу. Выберите другую карту или возьмите карту.", suits: ["крести", "бубны", "черви", "пики"] },
    hi: { summary: "गैर-वाइल्ड पत्ते सूट के अनुसार: {counts}। जिस सूट के पत्ते अधिक हों, उसे चुनने से अधिक विकल्प खुले रह सकते हैं।", draw: "1 पत्ता लिया — हाथ में अब {hand} पत्ते; इस राउंड में लिए गए पत्ते: {draws}।", illegal: "यह पत्ता सक्रिय सूट या रैंक से मेल नहीं खाता। दूसरा पत्ता चुनें या पत्ता लें।", suits: ["क्लब", "डायमंड", "हार्ट", "स्पेड"] },
    ar: { summary: "البطاقات غير الجوكر حسب النوع: {counts}. اختيار النوع الذي تملك منه بطاقات أكثر قد يبقي خيارات أكثر متاحة.", draw: "سحبت بطاقة واحدة — في يدك الآن {hand} بطاقة؛ مرات السحب في الجولة: {draws}.", illegal: "هذه البطاقة لا تطابق النوع أو الرتبة النشطة. اختر بطاقة أخرى أو اسحب.", suits: ["النوادي", "الماس", "القلوب", "البستوني"] },
  };
  ...
  const crazyEightsIllegalText = () => crazyEightsText("illegal");
  ...
  const __wpReadMeasurement = () => ({ ...__wpMeasurement,
    ...
    paused: Boolean(Boolean(__wpMeasurement.paused)), node: battle,
  ...
  __wpNotifyMeasurement();
    ...
    if (id === "hearts" || id === "crazy-eights" || id === "gin-rummy" || id === "cribbage") (__wpNotifyMeasurement(), resultText?.setAttribute("data-runtime-localize", "off"));
    if (id === "gin-rummy") {
    ...
    if (id === "old-maid" && resultCloseButton) {
      ...
      (__wpNotifyMeasurement(), resultCloseButton.setAttribute("aria-label", t("back")));
    ...
    { const __wpNextScreen = ({main:"main",stage:"stage",battle:"battle",})["battle"] ?? null;
      ...
      openMain() { battle.hidden = true; main.hidden = false; rootElement.dataset.screen = "main"; (__wpNotifyMeasurement(), result.hidden = true); window.scrollTo({ top: 0, left: 0, behavior: "auto" }); window.dispatchEvent(new Event("weightplay:shell-sync"));
    { const __wpNextScreen = ({main:"main",stage:"stage",battle:"battle",})["main"] ?? null;
      ...
      result(won, message = "") {
        ...
        if (won === null) (__wpNotifyMeasurement(), resultTitle.setAttribute('data-runtime-localize', 'off'));
    ...
    __wpMeasurement.ended = true; __wpMeasurement.outcome = won === null ? "draw" : won ? "win" : "lose"; __wpNotifyMeasurement();
},
    ...
    document.querySelector("#resultNewGame")?.addEventListener("click", () => { if (id === "old-maid") oldMaidTrack("new_game", { source: "result" }); (__wpNotifyMeasurement(), result.hidden = true); game.reset(); controller.openBattle(); });
    document.querySelector("#resultRestart")?.addEventListener("click", () => { if (id === "old-maid") oldMaidTrack("restart", { source: "result" }); resultRecorded = false; (__wpNotifyMeasurement(), result.hidden = true); __wpReplayStart(() => game.reset()); render(); });
    document.querySelector("#resultClose")?.addEventListener("click", () => { if (id === "old-maid" || id === "hearts") { if (id === "old-maid") oldMaidTrack("back", { source: "result" }); controller.openMain(); } else { (__wpNotifyMeasurement(), result.hidden = true); render(); } });
    ...
    document.querySelector("#restartBtn")?.addEventListener("click", () => { if (id === "old-maid") oldMaidTrack("restart", { source: "main" }); __wpReplayStart(() => game.reset()); controller.openBattle(); });
    ...
    document.querySelector("#battleRestartBtn")?.addEventListener("click", () => { __wpReplayStart(() => game.reset()); render(); });
    ...
    const ensureMainControls = () => {
      ...
      restart.addEventListener("click", () => { __wpReplayStart(() => game.reset()); controller.openBattle(); });
    ...
    // WP-GAME-ANALYTICS: model reset is a new deal; opening Main is not.
    // The wrapper runs only after the authored method succeeds and preserves this/return.
    const resetModel = game.reset;
    game.reset = function (...args) {
      const value = resetModel.apply(this, args);
      __wpMeasurement.roundKey = {}; __wpMeasurement.started = __wpMeasurement.screen === "battle"; __wpMeasurement.restart = false;
      __wpMeasurement.ended = false; __wpMeasurement.paused = false; __wpMeasurement.outcome = "complete";
      __wpNotifyMeasurement(); return value;
    };
    for (const method of ["pause", "resume", "stop"]) {
      if (typeof game[method] !== "function") continue;
      const original = game[method];
      game[method] = function (...args) {
        const value = original.apply(this, args);
        if (method === "stop") { __wpMeasurement.ended = true; __wpMeasurement.outcome = "abandon"; }
        else __wpMeasurement.paused = method === "pause";
        __wpNotifyMeasurement(); return value;
      };
    }
    game.reset();
  ...
  const CARD_GAME_GUIDES = {
    ...
    "go-fish": "Complete four-of-a-kind books. Choose two, three, or four players.",
  ...
  function makeHearts(controller) {
    ...
    const legal = (handCards, trick) => {
      const leadSuit = trick[0]?.card.suit;
      const following = leadSuit ? handCards.filter((item) => item.suit === leadSuit) : [];
      if (following.length) return following;
      // A player who cannot follow suit may discard any card. When leading,
      // Hearts and Q♠ stay locked until hearts are broken unless the hand has
      // no non-penalty card left; in that case every card is legal so a trick
      // cannot deadlock on a misleading first-trick cue.
      if (leadSuit) return handCards;
      const safe = handCards.filter((item) => item.suit !== "hearts" && !(item.suit === "spades" && item.rank === 12));
      return s.heartsBroken || !safe.length ? handCards : safe;
    };
    ...
    return {
      ...
      view() { const trickHtml = s.trick.map((entry) => `<div>${aiNames[entry.player]} ${cardMarkup(entry.card, 0)}</div>`).join(""); const action = s.phase === "pass" ? `<button class="primary-btn" data-action="pass" ${s.selected.size !== 3 ? "disabled" : ""}>${t("pass")} 3</button>` : `<p class="card-help">${s.heartsBroken ? "♥ " : ""}${t("yourTurn")}</p>`; const shellCopy = heartsShellCopy(); const firstLead = shellCopy?.lead || "2♣ leads the first trick"; const lead = s.hands[0].length === 13 && !s.trick.length ? firstLead : shellCopy?.leadBy ? shellCopy.leadBy.replace("{name}", aiNames[s.lead] || aiNames[0]) : `${aiNames[s.lead] || aiNames[0]} leads`; return { phase: s.phase === "pass" ? t("pass") : (s.heartsBroken ? "♥" : "♥ · " + t("waiting")), status: s.turn === 0 ? t("yourTurn") : t("aiTurn"), help: s.phase === "pass" ? heartsPassText(s.hands[0], s.selected) : heartsText(), score: s.scores[0], opponents: s.hands.slice(1).map((cards, index) => heartsOpponentMarkup(aiNames[index + 1], cards.length, `${t("points")}: ${s.scores[index + 1]}`)).join(""), center: `<div class="card-table-label">${t("table")}</div><div class="table-row">${trickHtml || `<span>${lead}</span>`}</div>`, hand: cardsMarkup(s.hands[0], { selected: s.selected, hidden: false }), actions: action }; },
    };
  }
  ...
  function makeCrazyEightsFixed(controller) {
    const s = { hands: [[], [], [], []], stock: [], discard: [], activeSuit: null, pendingSuit: false, turn: 0, playerDraws: 0, playerWildSuit: null, playerWildSuitCards: 0, drawCue: "", illegalCue: "" };
    const names = ["You", "AI North", "AI East", "AI West"];
    ...
    return {
      reset() { Object.assign(s, { hands: [[], [], [], []], stock: deck(), discard: [], activeSuit: null, pendingSuit: false, turn: 0, playerDraws: 0, playerWildSuit: null, playerWildSuitCards: 0, drawCue: "", illegalCue: "" }); for (let i = 0; i < 5; i += 1) s.hands.forEach((cards) => cards.push(s.stock.pop())); s.discard.push(s.stock.pop()); s.activeSuit = s.discard[0].suit; },
      card(index) { if (s.turn === 0 && !s.pendingSuit) { s.drawCue = ""; const item = s.hands[0][index]; if (legal(item)) { s.illegalCue = ""; play(0, item); } else { s.illegalCue = crazyEightsIllegalText(); } } },
      action(action, selected) { if (action === "draw" && s.turn === 0 && !s.pendingSuit) { s.illegalCue = ""; const drawn = draw(0); if (drawn) s.drawCue = crazyEightsDrawText(s.hands[0].length, s.playerDraws); if (legal(drawn)) play(0, drawn); } if (action === "suit" && s.turn === 0 && s.pendingSuit) { s.drawCue = ""; s.illegalCue = ""; s.activeSuit = selected; s.playerWildSuit = selected; s.playerWildSuitCards = s.hands[0].filter((item) => item.suit === selected && item.rank !== 8).length; s.pendingSuit = false; next(); } },
      view() { const illegalCue = s.illegalCue ? `<p class="card-choice-summary card-crazy-eights-illegal-cue" data-runtime-localize="off" role="status" aria-live="polite" aria-atomic="true">${s.illegalCue}</p>` : ""; const drawCue = s.drawCue ? `<p class="card-choice-summary card-crazy-eights-draw-cue" data-runtime-localize="off" role="status" aria-live="polite" aria-atomic="true">${s.drawCue}</p>` : ""; return { phase: `${t("play")} ${SYMBOLS[s.activeSuit] || "8"}`, status: s.turn === 0 ? t("yourTurn") : t("aiTurn"), help: s.pendingSuit ? t("chooseSuit") : `${t("play")}: ${rankText(s.discard.at(-1).rank)}${SYMBOLS[s.activeSuit]}`, score: s.hands[0].length, opponents: names.slice(1).map((name, index) => opponentMarkup(name, s.hands[index + 1].length)).join(""), center: `<div class="card-table-label">${t("discard")}</div><div class="table-row">${cardMarkup(s.discard.at(-1), 0)}${s.stock.length ? `<button class="playing-card is-face-down" data-action="draw" aria-label="${t("draw")}"></button>` : ""}</div>${illegalCue}${drawCue}${s.pendingSuit ? `<p class="card-choice-summary" role="status" aria-live="polite">${suitPreview()}</p><div class="card-choice-panel">${SUITS.map((suit) => `<button class="secondary-btn" data-action="suit" data-value="${suit}">${SYMBOLS[suit]}</button>`).join("")}</div>` : ""}`, hand: cardsMarkup(s.hands[0]), actions: `<button class="secondary-btn" data-action="draw" ${s.pendingSuit ? "disabled" : ""}>${t("draw")}</button>` }; }
    };
  }
  ...
  function makeCribbageFixed(controller) {
    ...
    return {
      ...
      view() { const playable = legalPeg(0); const battleCopy = cribbageBattleCopy(); const resetCue = s.resetCue ? `<div class="card-crib-transition card-crib-reset" role="status" aria-live="polite">${s.resetCue}</div>` : ""; const choiceCue = s.playerCrib.length === 2 && s.starter ? `<span class="card-crib-choice-plan">${cribChoicePayoffText(s.playerCrib, s.starter, s.dealer)}</span>` : ""; const pegCue = s.phase === "pegging" && s.pegCue ? `<div class="card-crib-score-cue" role="status" aria-live="polite">${s.pegCue}</div>` : ""; return { phase: s.phase === "discard" ? t("selectCards") : `${t("score")}: ${s.count}/31`, status: s.turn === 0 ? t("yourTurn") : t("aiTurn"), help: s.phase === "discard" ? cribbageSelectionText(s.selected.size) : s.turn === 0 ? cribPeggingCoachText(playable.length > 0) : t("aiTurn"), score: s.score[0], opponents: opponentMarkup(battleCopy.ai, s.ai.length, `${t("score")}: ${s.score[1]}`), center: `<div class="card-table-label" data-runtime-localize="off">${t("cribbage")} · ${battleCopy.round} ${s.round} · ${s.starter ? cardText(s.starter) : ""}</div>${s.phase === "pegging" ? `<div class="card-crib-transition" role="status" aria-live="polite">${cribTransitionText(s.starter, s.dealer)} ${choiceCue}</div>${resetCue}` : ""}${makePegBoard(s.score[0], s.score[1])}<div class="table-row">${cardsMarkup(s.playerPeg)}${cardsMarkup(s.aiPeg)}</div>${pegCue}`, hand: cardsMarkup(s.hand, { selected: s.selected }), actions: s.phase === "discard" ? `<button class="primary-btn" data-action="send-crib" ${s.selected.size !== 2 ? "disabled" : ""}>${t("submit")}</button>` : `<button class="secondary-btn" data-action="go" data-runtime-localize="off" ${s.turn !== 0 || playable.length ? "disabled" : ""}>${battleCopy.go}</button>` }; }
    };
  }
