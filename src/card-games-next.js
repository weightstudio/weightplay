(function (root) {
  ...
  const CASINO_COPY = {
    en: { selectPrompt: "Select a hand card, then table cards with the same value or a matching sum.", captureHint: "Value {value}: {rank} same-value option(s), {sum} sum combination(s). Highlighted cards are candidates; you choose the capture.", trailPrompt: "Choose table cards to Capture or Build, or Trail this card when no capture is available.", trail: "Trail", captureFeedback: "Captured {count} cards; immediate card bonus +{bonus}. The card majority is scored at round end.", resultBreakdown: "Captured {cards} cards · special bonuses +{immediate} · majority bonus: {majority} · final score: {score}." },
    "zh-Hant": { selectPrompt: "先選一張手牌，再選相同點數或合計相符的桌面牌。", captureHint: "點數 {value}：同點 {rank} 個選項、合計 {sum} 種組合。高亮牌是候選，你自行選擇捕獲。", trailPrompt: "選擇桌面牌來捕獲或建立牌組；沒有可捕獲的牌時，可將這張牌出到桌面。", trail: "出牌", captureFeedback: "捕獲 {count} 張牌；即時牌面獎勵 +{bonus}。最多牌獎勵在本局結算。", resultBreakdown: "捕獲 {cards} 張牌 · 特殊獎勵 +{immediate} · 最多牌獎勵：{majority} · 最終得分：{score}。" },
    "zh-Hans": { selectPrompt: "先选一张手牌，再选相同点数或合计相符的桌面牌。", captureHint: "点数 {value}：同点 {rank} 个选项、合计 {sum} 种组合。高亮牌是候选，由你选择捕获。", trailPrompt: "选择桌面牌来捕获或建立牌组；没有可捕获的牌时，可将这张牌出到桌面。", trail: "出牌", captureFeedback: "捕获 {count} 张牌；即时牌面奖励 +{bonus}。最多牌奖励在本局结算。", resultBreakdown: "捕获 {cards} 张牌 · 特殊奖励 +{immediate} · 最多牌奖励：{majority} · 最终得分：{score}。" },
    ja: { selectPrompt: "手札を1枚選び、同じ値または合計が一致する場札を選びます。", captureHint: "値 {value}：同じ値 {rank} 件、合計 {sum} 通り。ハイライトは候補です。取るカードは自分で選びます。", trailPrompt: "場札を選んで獲得またはビルドします。獲得できないときは、このカードを場に出します。", trail: "場に出す", captureFeedback: "{count}枚を獲得。即時カードボーナス +{bonus}。最多カード報酬はラウンド終了時に計算されます。", resultBreakdown: "{cards}枚を獲得 · 特別ボーナス +{immediate} · 最多カード報酬：{majority} · 最終スコア：{score}。" },
    ko: { selectPrompt: "손패를 한 장 고른 뒤 같은 값이거나 합이 맞는 테이블 카드를 고르세요.", captureHint: "값 {value}: 같은 값 {rank}개, 합 조합 {sum}개입니다. 강조된 카드는 후보이며 잡을 카드는 직접 고릅니다.", trailPrompt: "테이블 카드를 골라 잡거나 빌드하세요. 잡을 수 없을 때는 이 카드를 테이블에 내려놓습니다.", trail: "내려놓기", captureFeedback: "{count}장 획득. 즉시 카드 보너스 +{bonus}. 최다 카드 보너스는 라운드 종료 시 계산됩니다.", resultBreakdown: "{cards}장 획득 · 특별 보너스 +{immediate} · 최다 카드 보너스: {majority} · 최종 점수: {score}." },
    es: { selectPrompt: "Elige una carta de tu mano y después cartas de mesa del mismo valor o con una suma igual.", captureHint: "Valor {value}: {rank} opción(es) del mismo valor y {sum} combinación(es) de suma. Las cartas resaltadas son candidatas; tú eliges la captura.", trailPrompt: "Elige cartas de mesa para capturar o construir; si no puedes capturar, juega esta carta sobre la mesa.", trail: "Jugar", captureFeedback: "Capturaste {count} cartas; bonificación inmediata +{bonus}. La mayoría de cartas se puntúa al final.", resultBreakdown: "Capturaste {cards} cartas · bonificaciones especiales +{immediate} · mayoría: {majority} · puntuación final: {score}." },
    "pt-BR": { selectPrompt: "Escolha uma carta da mão e depois cartas da mesa com o mesmo valor ou uma soma igual.", captureHint: "Valor {value}: {rank} opção(ões) do mesmo valor e {sum} combinação(ões) de soma. As cartas destacadas são candidatas; você escolhe a captura.", trailPrompt: "Escolha cartas da mesa para capturar ou construir; se não puder capturar, jogue esta carta na mesa.", trail: "Jogar", captureFeedback: "Você capturou {count} cartas; bônus imediato +{bonus}. A maioria de cartas é pontuada no fim.", resultBreakdown: "Você capturou {cards} cartas · bônus especiais +{immediate} · maioria de cartas: {majority} · pontuação final: {score}." },
    fr: { selectPrompt: "Choisissez une carte de votre main, puis des cartes de table de même valeur ou d'une somme égale.", captureHint: "Valeur {value} : {rank} option(s) de même valeur et {sum} combinaison(s) de somme. Les cartes en évidence sont candidates ; vous choisissez la capture.", trailPrompt: "Choisissez des cartes à capturer ou à construire ; sans capture possible, posez cette carte sur la table.", trail: "Poser", captureFeedback: "{count} cartes capturées ; bonus immédiat +{bonus}. La majorité des cartes est comptée à la fin.", resultBreakdown: "{cards} cartes capturées · bonus spéciaux +{immediate} · majorité : {majority} · score final : {score}." },
    de: { selectPrompt: "Wähle eine Handkarte und dann Tischkarten mit gleichem Wert oder passender Summe.", captureHint: "Wert {value}: {rank} gleiche-Wert-Option(en), {sum} Summenkombination(en). Hervorgehobene Karten sind Kandidaten; du wählst selbst.", trailPrompt: "Wähle Tischkarten zum Nehmen oder Bauen; wenn kein Nehmen möglich ist, spiele diese Karte auf den Tisch.", trail: "Ausspielen", captureFeedback: "{count} Karten genommen; sofortiger Kartenbonus +{bonus}. Die Kartenmehrheit zählt am Ende.", resultBreakdown: "{cards} Karten genommen · Sonderboni +{immediate} · Kartenmehrheit: {majority} · Endpunktzahl: {score}." },
    it: { selectPrompt: "Scegli una carta dalla mano, poi carte sul tavolo dello stesso valore o con una somma uguale.", captureHint: "Valore {value}: {rank} opzione/i dello stesso valore e {sum} combinazione/i di somma. Le carte evidenziate sono candidate; scegli tu la cattura.", trailPrompt: "Scegli le carte da catturare o costruire; se non puoi catturare, gioca questa carta sul tavolo.", trail: "Gioca", captureFeedback: "{count} carte catturate; bonus immediato +{bonus}. La maggioranza delle carte vale a fine mano.", resultBreakdown: "{cards} carte catturate · bonus speciali +{immediate} · maggioranza: {majority} · punteggio finale: {score}." },
    ru: { selectPrompt: "Выберите карту из руки, затем карты на столе с тем же значением или подходящей суммой.", captureHint: "Значение {value}: вариантов того же значения — {rank}, комбинаций по сумме — {sum}. Выделенные карты — кандидаты; выбор остаётся за вами.", trailPrompt: "Выберите карты для взятия или сбора; если взять нельзя, выложите эту карту на стол.", trail: "Выложить", captureFeedback: "Взято карт: {count}; немедленный бонус за карты: +{bonus}. Большинство карт учитывается в конце раунда.", resultBreakdown: "Взято карт: {cards} · особые бонусы: +{immediate} · большинство карт: {majority} · итоговый счёт: {score}." },
    hi: { selectPrompt: "हाथ से एक पत्ता चुनें, फिर उसी मान या बराबर योग वाले मेज के पत्ते चुनें।", captureHint: "मान {value}: समान मान के {rank} विकल्प, योग के {sum} संयोजन। उभरे पत्ते उम्मीदवार हैं; पकड़ आप चुनते हैं।", trailPrompt: "पकड़ने या बनाने के लिए मेज के पत्ते चुनें; जब कोई पकड़ संभव न हो, तो इस पत्ते को मेज पर रखें।", trail: "मेज पर रखें", captureFeedback: "{count} पत्ते लिए; तुरंत कार्ड बोनस +{bonus}। सबसे अधिक पत्तों का अंक राउंड के अंत में जुड़ेगा।", resultBreakdown: "{cards} पत्ते लिए · विशेष बोनस +{immediate} · अधिकांश पत्तों का बोनस: {majority} · अंतिम स्कोर: {score}।" },
    ar: { selectPrompt: "اختر بطاقة من يدك، ثم بطاقات على الطاولة بالقيمة نفسها أو بمجموع مطابق.", captureHint: "القيمة {value}: {rank} خيار بالقيمة نفسها و{sum} تركيبة مجموع. البطاقات المميزة مرشحة؛ وأنت تختار الالتقاط.", trailPrompt: "اختر بطاقات لالتقاطها أو بنائها؛ إذا لم يتوفر التقاط، ضع هذه البطاقة على الطاولة.", trail: "ضع على الطاولة", captureFeedback: "تم التقاط {count} بطاقة؛ مكافأة البطاقات الفورية +{bonus}. تُحتسب أغلبية البطاقات عند نهاية الجولة.", resultBreakdown: "تم التقاط {cards} بطاقة · المكافآت الخاصة +{immediate} · مكافأة الأغلبية: {majority} · النتيجة النهائية: {score}." },
  };
