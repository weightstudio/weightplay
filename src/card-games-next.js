(function (root) {
  ...
  function makeGinRummyFixed(controller) {
    ...
    return {
      ...
      action(action) { if (s.turn !== 0 || s.over) return; if (action === "draw-stock" && !s.drawn) drawCard(false); if (action === "draw-discard" && !s.drawn) drawCard(true); if (action === "discard" && s.drawn && s.selected.size === 1) { const index = [...s.selected][0]; const discarded = s.player.splice(index, 1)[0]; if (discarded) s.discard.push(discarded); s.selected.clear(); const stats = meldStats(s.player); if (stats.deadwood === 0) finish(0, t("gin")); else { s.turn = 1; s.drawn = false; setTimeout(aiTurn, 320); } } if (action === "knock" && s.drawn) { const stats = meldStats(s.player); if (stats.deadwood <= 10) { const aiStats = meldStats(s.ai); finish(stats.deadwood <= aiStats.deadwood ? 0 : 1, `${t("knock")} ${stats.deadwood} / ${aiStats.deadwood}`); } } },
  ...
  function makeGoFishFixed(controller) {
    ...
    const next = () => { s.turn = (s.turn + 1) % s.playerCount; if (s.turn !== 0) setTimeout(aiTurn, 300); };
    const ask = (target, rank) => { const matching = s.players[target].filter((item) => item.rank === rank); if (matching.length) { s.players[target] = s.players[target].filter((item) => item.rank !== rank); s.players[0].push(...matching); removeBooks(0); s.selectedRank = null; } else if (s.stock.length) s.players[0].push(s.stock.pop()); else next(); if (!matching.length) next(); if (!s.stock.length && s.players.slice(0, s.playerCount).every((cards) => !cards.length)) finish(); };
  ...
  function makeOldMaidFixed(controller) {
    ...
    const names = ["You", "Fox", "Panda", "Otter"];
    const pair = (player) => { const byRank = new Map(); s.players[player].forEach((item) => { const list = byRank.get(item.rank) || []; list.push(item); byRank.set(item.rank, list); }); byRank.forEach((items) => { const normal = items.filter((item) => !item.oldMaid); if (normal.length >= 2) { normal.slice(0, 2).forEach((item) => { const index = s.players[player].indexOf(item); if (index >= 0) s.players[player].splice(index, 1); }); s.books[player] += 1; } }); };
    ...
    const next = () => { s.turn = (s.turn + 1) % 4; while (!s.players[s.turn].length && s.players.some((cards) => cards.length)) s.turn = (s.turn + 1) % 4; if (s.turn !== 0) setTimeout(aiTurn, 320); };
    const drawFrom = (player, index) => { const target = (player + 1) % 4; const source = s.players[target]; if (!source.length) { next(); return; } const item = source.splice(Math.min(index, source.length - 1), 1)[0]; s.players[player].push(item); pair(player); finishIfDone(); if (!s.over) next(); };
    const aiTurn = () => { if (s.turn !== 0 && !s.over) drawFrom(s.turn, Math.floor(Math.random() * s.players[(s.turn + 1) % 4].length)); };
    return {
      ...
      action() {},
      view() { const target = s.players[1]; return { phase: t("oldMaid"), status: s.turn === 0 ? t("yourTurn") : t("aiTurn"), help: `Draw one hidden card from ${names[(s.turn + 1) % 4]}. Pairs disappear automatically.`, score: s.books[0], opponents: names.slice(1).map((name, index) => opponentMarkup(name, s.players[index + 1].length, `${s.books[index + 1]} ${t("pairs")}`)).join(""), center: `<div class="card-table-label">${t("oldMaid")}</div><div class="table-row">${target.map((_, index) => cardMarkup({ faceDown: true }, index)).join("")}</div>`, hand: cardsMarkup(s.players[0]), actions: "" }; }
    };
  }
