(function (root) {
  ...
  const CASINO_COPY = {
    en: { selectPrompt: "Select a hand card, then table cards with the same value or a matching sum.", captureFeedback: "Captured {count} cards; immediate card bonus +{bonus}. The card majority is scored at round end." },
    "zh-Hant": { selectPrompt: "先選一張手牌，再選相同點數或合計相符的桌面牌。", captureFeedback: "捕獲 {count} 張牌；即時牌面獎勵 +{bonus}。最多牌獎勵在本局結算。" },
    "zh-Hans": { selectPrompt: "先选一张手牌，再选相同点数或合计相符的桌面牌。", captureFeedback: "捕获 {count} 张牌；即时牌面奖励 +{bonus}。最多牌奖励在本局结算。" },
    ja: { selectPrompt: "手札を1枚選び、同じ値または合計が一致する場札を選びます。", captureFeedback: "{count}枚を獲得。即時カードボーナス +{bonus}。最多カード報酬はラウンド終了時に計算されます。" },
    ko: { selectPrompt: "손패를 한 장 고른 뒤 같은 값이거나 합이 맞는 테이블 카드를 고르세요.", captureFeedback: "{count}장 획득. 즉시 카드 보너스 +{bonus}. 최다 카드 보너스는 라운드 종료 시 계산됩니다." },
    es: { selectPrompt: "Elige una carta de tu mano y después cartas de mesa del mismo valor o con una suma igual.", captureFeedback: "Capturaste {count} cartas; bonificación inmediata +{bonus}. La mayoría de cartas se puntúa al final." },
    "pt-BR": { selectPrompt: "Escolha uma carta da mão e depois cartas da mesa com o mesmo valor ou uma soma igual.", captureFeedback: "Você capturou {count} cartas; bônus imediato +{bonus}. A maioria de cartas é pontuada no fim." },
    fr: { selectPrompt: "Choisissez une carte de votre main, puis des cartes de table de même valeur ou d'une somme égale.", captureFeedback: "{count} cartes capturées ; bonus immédiat +{bonus}. La majorité des cartes est comptée à la fin." },
    de: { selectPrompt: "Wähle eine Handkarte und dann Tischkarten mit gleichem Wert oder passender Summe.", captureFeedback: "{count} Karten genommen; sofortiger Kartenbonus +{bonus}. Die Kartenmehrheit zählt am Ende." },
    it: { selectPrompt: "Scegli una carta dalla mano, poi carte sul tavolo dello stesso valore o con una somma uguale.", captureFeedback: "{count} carte catturate; bonus immediato +{bonus}. La maggioranza delle carte vale a fine mano." },
    ru: { selectPrompt: "Выберите карту из руки, затем карты на столе с тем же значением или подходящей суммой.", captureFeedback: "Взято карт: {count}; немедленный бонус за карты: +{bonus}. Большинство карт учитывается в конце раунда." },
    hi: { selectPrompt: "हाथ से एक पत्ता चुनें, फिर उसी मान या बराबर योग वाले मेज के पत्ते चुनें।", captureFeedback: "{count} पत्ते लिए; तुरंत कार्ड बोनस +{bonus}। सबसे अधिक पत्तों का अंक राउंड के अंत में जुड़ेगा।" },
    ar: { selectPrompt: "اختر بطاقة من يدك، ثم بطاقات على الطاولة بالقيمة نفسها أو بمجموع مطابق.", captureFeedback: "تم التقاط {count} بطاقة؛ مكافأة البطاقات الفورية +{bonus}. تُحتسب أغلبية البطاقات عند نهاية الجولة." },
  };
