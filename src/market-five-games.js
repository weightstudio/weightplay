...
(() => {
  ...
  function releaseShot() {
    ...
    g.ball={t:0,made,perfect,aimDelta,powerDelta,releasePower:g.power};const shotMessage=`${perfect?copy.perfect:made?copy.made:copy.missed}${points?` · +${points}`:""} · ${g.lastTip}`;setStatus(shotMessage);canvas.setAttribute("aria-label",`${window.WeightPlayMarketFiveLocale.game().title} · ${shotMessage}`);updateHud();
  }
  function setHoopBrief(){const g=state.game,profile=hoopProfiles[state.stage-1],text=window.WeightPlayMarketFiveLocale.game(),challenge=hoopChallenge(Math.min(profile.shots,g.shots+1)),brief=`${profile.badge} ${text.courts[state.stage-1]} · ${Math.min(profile.shots,g.shots+1)}/${profile.shots} · 🎯 ${Math.round(challenge.aim*100)} · ⚡ ${Math.round(challenge.power*100)}`,cue=state.stage===1&&g.shots===0?` · ${text.hoop.firstShot}`:"";setStatus(`${brief}${cue}`);canvas.setAttribute("aria-label",`${text.title} · ${brief}${cue}`);}
