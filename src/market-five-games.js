...
(() => {
  ...
  function initGame() {
    if (gameId === "animal-hoop-league") state.game = { aim: .42, power: 0, charging: false, shots: 0, score: 0, rival: 0, ball: null };
  ...
  function buildControls() {
    ...
    if (gameId === "animal-hoop-league") host.innerHTML = control("◀", "aim-left") + control(common(14), "shoot", "primary-control") + control("▶", "aim-right");
  ...
  function releaseShot() {
    const g=state.game; g.charging=false; if(g.ball) return;const profile=hoopProfiles[state.stage-1],shot=g.shots+1,challenge=hoopChallenge(shot);g.shots=shot;
    const aimError=Math.abs(g.aim-challenge.aim)/challenge.aimTolerance,powerError=Math.abs(g.power-challenge.power)/challenge.powerTolerance;
    const made=aimError<=1&&powerError<=1,perfect=made&&aimError<=.34&&powerError<=.4,points=perfect?profile.perfect:made?1:0;if(made)g.score+=points;
    if(g.shots%2===0)g.rival+=profile.rival[Math.floor(g.shots/2)-1]||0;
    g.ball={t:0,made,perfect}; setStatus(made?(perfect?`✦ +${points}`:"✓ +1"):"↺"); updateHud();
  }
  function setHoopBrief(){const g=state.game,profile=hoopProfiles[state.stage-1],challenge=hoopChallenge(Math.min(profile.shots,g.shots+1)),brief=`${profile.badge} · ${Math.min(profile.shots,g.shots+1)}/${profile.shots} · 🎯 ${Math.round(challenge.aim*100)} · ⚡ ${Math.round(challenge.power*100)}`;setStatus(brief);canvas.setAttribute("aria-label",`${window.WeightPlayMarketFiveLocale.game().title} · ${brief}`);}
  ...
  function update(dt) {
    ...
    if(gameId==="animal-hoop-league"){
      const profile=hoopProfiles[state.stage-1];if(g.charging)g.power=(g.power+dt*.012*profile.charge)%1;if(g.ball){g.ball.t+=dt*.018;if(g.ball.t>=1){g.ball=null;if(g.shots>=profile.shots)finish(g.score>g.rival,`${g.score} — ${g.rival}`);else setHoopBrief();}}
    }
  ...
  function drawHoop(){const g=state.game,challenge=hoopChallenge(Math.min(hoopProfiles[state.stage-1].shots,g.shots+1)),x=345,w=360;drawHero(65,205,245,310);drawAtlas(0,650,235,78,78);ctx.strokeStyle="#ffe79a";ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(278,330);ctx.quadraticCurveTo(460,70+g.aim*120,690,280);ctx.stroke();ctx.fillStyle="#10283ce8";ctx.fillRect(x,426,w,20);ctx.fillStyle="#ffd166";ctx.fillRect(x+w*(challenge.aim-challenge.aimTolerance),426,w*challenge.aimTolerance*2,20);ctx.fillStyle="#eafcff";ctx.fillRect(x+w*g.aim-3,421,6,30);ctx.fillStyle="#10283ce8";ctx.fillRect(x,470,w,24);ctx.fillStyle="#ffd166";ctx.fillRect(x+w*(challenge.power-challenge.powerTolerance),470,w*challenge.powerTolerance*2,24);ctx.fillStyle="#6be2dc";ctx.fillRect(x,470,w*g.power,24);ctx.font="bold 23px system-ui";ctx.fillStyle="#f5fbff";ctx.fillText("🎯",305,444);ctx.fillText("⚡",305,491);if(g.ball){const t=g.ball.t,bx=278+(690-278)*t,by=330-310*Math.sin(Math.PI*t);drawAtlas(0,bx-34,by-34,68,68);}}
