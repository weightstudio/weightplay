(function () {
  ...
  function buildMines() {
    ...
    function reveal(i) { if (revealed.has(i) || flags.has(i)) return; const previous = snapshot(); if (first) { while (mines.has(i)) { generate(); } previous.mines = [...mines]; first = false; startTimer(); } history.push(previous); if (mines.has(i)) { revealed.add(i); clearInterval(timerId); render(); finish(false, `${t("failed")} ${t("moves")}: ${revealed.size}`); return; } const queue = [i]; while (queue.length) { const current = queue.shift(); if (revealed.has(current) || flags.has(current)) continue; revealed.add(current); const count = neighbours(current, rows, cols).filter((n) => mines.has(n)).length; if (!count) neighbours(current, rows, cols).forEach((n) => { if (!revealed.has(n)) queue.push(n); }); } hintIndex = -1; render(); if (revealed.size >= rows * cols - mineCount) { clearInterval(timerId); finish(true, `${t("solved")} ${t("moves")}: ${revealed.size}`); } else announce(t("turn")); }
  ...
  function buildReversi() {
    ...
    function play(i) { if (locked || !legal(player).includes(i)) return; boardState[i] = 1; flips(i, 1).forEach((n) => { boardState[n] = 1; }); locked = true; render(); setTimeout(aiTurn, 240); }
    function hardScore(i) { const corners = new Set([0, 7, 56, 63]), edges = new Set([...Array(8).keys(), ...Array(8).keys()].flatMap((n) => [n, 56 + n, n * 8, n * 8 + 7])); const cornerTraps = new Map([[9, 0], [14, 7], [49, 56], [54, 63], [1, 0], [8, 0], [6, 7], [15, 7], [48, 56], [57, 56], [55, 63], [62, 63]]); const row = Math.floor(i / 8), col = i % 8; let score = flips(i, 2).length * 4 + 12 - Math.abs(row - 3.5) - Math.abs(col - 3.5); if (corners.has(i)) score += 1000; else if (edges.has(i)) score += 120; const corner = cornerTraps.get(i); if (corner !== undefined && !boardState[corner]) score -= 240; return score; }
    ...
    function aiTurn() { const moves = legal(2); if (!moves.length) { if (!legal(1).length) end(); else { locked = false; render(); } return; } const choice = chooseAiMove(moves); boardState[choice.index] = 2; flips(choice.index, 2).forEach((n) => { boardState[n] = 2; }); reply.textContent = formatReply(choice); reply.dataset.aiChoice = String(choice.index); reply.dataset.aiCaptures = String(choice.captures); const playerMoves = legal(1), nextAiMoves = legal(2); if (!playerMoves.length) { render(); if (!nextAiMoves.length) end(); else { locked = true; setTimeout(aiTurn, 240); } } else { locked = false; render(); } }
    function end() { render(); const p = boardState.filter((v) => v === 1).length, a = boardState.filter((v) => v === 2).length; finish(p > a, `${t("score")}: ${p} – ${a}`); }
    function reset() { difficulty = select.value; boardState = Array(64).fill(0); boardState[27] = 2; boardState[28] = 1; boardState[35] = 1; boardState[36] = 2; locked = false; policy.textContent = text(reversiDifficultyPolicy[difficulty]); reply.textContent = ""; delete reply.dataset.aiChoice; delete reply.dataset.aiCaptures; render(); announce(t("turn")); }
