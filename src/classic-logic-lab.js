(function () {
  ...
  const battleReturnCopy = L(
    "Back to menu", "返回選單", "返回菜单", "メニューに戻る", "메뉴로 돌아가기",
    "Volver al menú", "Voltar ao menu", "Retour au menu", "Zurück zum Menü", "Torna al menu",
    "Вернуться в меню", "मेन्यू पर वापस जाएँ", "العودة إلى القائمة",
  );
  const codeSlotCopy = {
  ...
  function buildSudoku() {
    ...
    for (let i = 0; i < 81; i += 1) {
      const b = cell("", "", "", () => { if (!given.has(i)) { selected = i; render(); } });
      cellButtons.push(b);
      board.append(b);
    }
    for (let n = 1; n <= 9; n += 1) {
      const b = makeButton(String(n), "logic-mini");
      b.addEventListener("click", () => enter(n));
      keypad.append(b);
    }
    const clearButton = makeButton(t("clear"), "logic-mini");
    clearButton.addEventListener("click", () => enter(0));
