(function () {
  ...
  const snakeContinuousCopy = {
    "zh-Hant": { title: "貪食蛇", intro: "貪食蛇是 WeightPlay 原創的格線生存街機遊戲。帶領成長中的星蛇穿過有限棋盤，收集食物火花並持續前進。", how: ["桌面使用方向鍵或 WASD；手機使用方向按鈕或滑動。", "在下一格封閉前先規劃轉向，反向輸入不會打斷目前路線。", "持續前進並收集食物火花，蛇身會逐步變長；沒有固定的食物數量終點。", "撞到牆壁或自己的軌跡會結束本局；重試會保留本機最佳分數。"] },
    "zh-Hans": { title: "贪吃蛇", intro: "贪吃蛇是 WeightPlay 原创的格线生存街机游戏。带领成长中的星蛇穿过有限棋盘，收集食物火花并持续前进。", how: ["桌面使用方向键或 WASD；手机使用方向按钮或滑动。", "在下一格封闭前先规划转向，反向输入不会打断当前路线。", "持续前进并收集食物火花，蛇身会逐步变长；没有固定的食物数量终点。", "撞到墙壁或自己的轨迹会结束本局；重试会保留本机最佳分数。"] },
    ja: { title: "スネーク", intro: "スネークはWeightPlayオリジナルのグリッド生存アーケードです。成長する星のヘビを導き、食べ物を集めながら進み続けます。", how: ["デスクトップは矢印キーまたはWASD、モバイルは方向ボタンかスワイプです。", "次のマスが閉じる前に曲がり、反対方向の入力で進路を壊さないようにします。", "食べ物を集めてヘビを成長させながら進み続けます。固定された個数のゴールはありません。", "壁や自分の軌跡に当たると終了し、リトライしてもローカルのベストは残ります。"] },
    ko: { title: "스네이크", intro: "스네이크는 WeightPlay 오리지널 격자 생존 아케이드입니다. 성장하는 별뱀을 조종해 먹이를 모으며 계속 이동하세요.", how: ["데스크톱은 방향키나 WASD, 모바일은 방향 버튼이나 스와이프를 사용합니다.", "다음 칸이 막히기 전에 회전하고 반대 방향 입력으로 경로를 끊지 않습니다.", "먹이를 모으며 계속 이동해 뱀을 키우세요. 고정된 먹이 개수의 종료 지점은 없습니다.", "벽이나 자신의 궤적에 닿으면 종료되며 재시작해도 로컬 최고 점수는 남습니다."] },
    es: { title: "Serpiente", intro: "Serpiente es un arcade original de supervivencia en cuadrícula de WeightPlay. Guía una serpiente estelar, recoge chispas y sigue avanzando.", how: ["Usa flechas o WASD; en móvil, usa botones de dirección o desliza.", "Gira antes de que se cierre la siguiente casilla y no inviertas la dirección actual.", "Sigue avanzando y recoge chispas mientras la serpiente crece; no hay un límite fijo de comida.", "Chocar con una pared o con tu rastro termina la partida; el mejor resultado local se conserva al reintentar."] },
    "pt-BR": { title: "Snake", intro: "Snake é um arcade original de sobrevivência em grade do WeightPlay. Guie uma serpente estelar, colete faíscas e continue avançando.", how: ["Use setas ou WASD; no celular use botões de direção ou deslize.", "Vire antes que a próxima casa feche e não inverta a direção atual.", "Continue avançando e colete faíscas enquanto a cobra cresce; não há um limite fixo de alimentos.", "Bater na parede ou no próprio rastro encerra a partida; o melhor placar local fica salvo ao tentar novamente."] },
    fr: { title: "Serpent", intro: "Serpent est un arcade original de survie sur grille de WeightPlay. Guide un serpent-étoile, collecte des étincelles et continue d’avancer.", how: ["Utilise les flèches ou WASD ; sur mobile, touche une direction ou glisse.", "Tourne avant que la prochaine case ne se ferme et ne renverse pas la direction actuelle.", "Continue d’avancer et collecte des étincelles pendant que le serpent grandit ; aucun nombre fixe ne termine la partie.", "Un mur ou ta propre trace termine la partie ; le meilleur score local reste après une nouvelle tentative."] },
    de: { title: "Snake", intro: "Snake ist ein originales Raster-Survival-Arcade-Spiel von WeightPlay. Führe eine wachsende Sternenschlange, sammle Funken und bleibe in Bewegung.", how: ["Nutze Pfeile oder WASD; mobil verwendest du Richtungsbuttons oder wischst.", "Drehe vor der nächsten geschlossenen Rasterzelle und kehre nicht die aktuelle Richtung um.", "Bleibe in Bewegung und sammle Funken, während die Schlange wächst; es gibt kein festes Futterlimit.", "Eine Wand oder die eigene Spur beendet den Lauf; beim erneuten Versuch bleibt der lokale Bestwert erhalten."] },
    it: { title: "Snake", intro: "Snake è un arcade originale di sopravvivenza su griglia WeightPlay. Guida un serpente stellare, raccogli scintille e continua a muoverti.", how: ["Usa le frecce o WASD; su mobile usa i pulsanti direzionali o scorri.", "Svolta prima che la prossima casella si chiuda e non invertire la direzione attuale.", "Continua a muoverti e raccogli scintille mentre il serpente cresce; non c’è un limite fisso di cibo.", "Un muro o la tua scia termina la partita; il record locale resta dopo un nuovo tentativo."] },
    ru: { title: "Змейка", intro: "«Змейка» — оригинальная аркада WeightPlay на выживание в сетке. Ведите растущую звёздную змейку, собирайте искры и продолжайте движение.", how: ["Используй стрелки или WASD; на телефоне нажимай направление или делай свайп.", "Поворачивай до закрытия следующей клетки и не разворачивай текущий ход.", "Продолжай движение и собирай искры по мере роста змейки; фиксированного лимита еды нет.", "Стена или собственный след завершает забег; лучший локальный результат сохраняется после повтора."] },
    hi: { title: "साँप", intro: "साँप WeightPlay का मौलिक ग्रिड सर्वाइवल आर्केड है। बढ़ते सितारा-साँप को चलाएँ, भोजन-चिंगारियाँ लें और चलते रहें।", how: ["डेस्कटॉप पर तीर या WASD, मोबाइल पर दिशा बटन या स्वाइप इस्तेमाल करें।", "अगला खाना बंद होने से पहले मोड़ें और मौजूदा दिशा को उलटें नहीं।", "साँप के बढ़ने पर भोजन-चिंगारियाँ लेते हुए चलते रहें; भोजन की कोई तय अंतिम संख्या नहीं है।", "दीवार या अपनी पूँछ से टकराने पर रन समाप्त होता है; दोबारा खेलने पर स्थानीय सर्वश्रेष्ठ स्कोर रहता है।"] },
    ar: { title: "الثعبان", intro: "الثعبان لعبة بقاء أصلية من WeightPlay على شبكة. قد ثعباناً نجمياً ينمو، واجمع شرارات الطعام واستمر في الحركة.", how: ["استخدم الأسهم أو WASD؛ على الهاتف اضغط الاتجاه أو اسحب.", "انعطف قبل إغلاق الخلية التالية ولا تعكس الاتجاه الحالي.", "استمر في الحركة واجمع شرارات الطعام بينما ينمو الثعبان؛ لا توجد نهاية بعدد ثابت من الطعام.", "ينتهي اللعب عند الاصطدام بالجدار أو بالمسار الخاص؛ يبقى أفضل سجل محلياً عند الإعادة."] },
  };
