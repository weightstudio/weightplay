(() => {
  ...
  const spaceHint = (key) => SPACE_HINTS[key]?.[locale] || SPACE_HINTS[key]?.en || "";
  ...
  function mazeEnemyStep() {
    ...
    maze.enemies.forEach((enemy) => {
      ...
      if (enemy.x === player.x && enemy.y === player.y) {
        ...
        else { state.level = Math.max(1, state.level); state.maze.player = { x: player.x, y: player.y, dir: player.dir, next: player.next, grace: 6000 }; state.maze.enemies.forEach((item) => { item.x = item.homeX; item.y = item.homeY; }); state.maze.power = 0; state.maze.moveClock = 0; state.maze.enemyClock = -4500; state.maze.lives = (state.maze.lives || 5) - 1; setMessage(tr("hit"), "danger"); tone(120, 0.2, "sawtooth", 0.05); if (state.maze.lives <= 0) finish(false, { stage: state.level, lives: 0 }); }
      }
    });
  }
  ...
  function resetSpace() { state.space = { ship: { x: 480, y: 270, vx: 0, vy: 0, angle: -Math.PI / 2 }, rocks: [], bullets: [], particles: [], rapid: 0, hits: 0, invincible: 3.2, fireClock: 0, combo: 0, comboClock: 0, transition: 0, tutorialClock: 6, guardianFlash: 0, ufo: null, shots: 0, fragments: 0 }; spawnSpaceWave(); state.input = {}; updateHud(); drawSpace(); setMessage(tr("ready")); }
  ...
  function updateSpace(dt) {
    const space = state.space, ship = space.ship, input = state.input; space.fireClock = Math.max(0, space.fireClock - dt); space.rapid = Math.max(0, space.rapid - dt); space.comboClock = Math.max(0, space.comboClock - dt); space.tutorialClock = Math.max(0, space.tutorialClock - dt); space.guardianFlash = Math.max(0, space.guardianFlash - dt); if (!space.comboClock) space.combo = 0;
