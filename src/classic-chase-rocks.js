(() => {
  ...
  const ui = {
    ...
    language: document.getElementById("languageLabel"), locale: document.getElementById("localeSelect"), start: document.getElementById("startBtn"), mainProgress: document.getElementById("mainProgress"), cover: document.getElementById("coverArt"), footer: document.getElementById("footerText"),
  ...
  const SPACE_HINTS = {
    ...
    hitRecoveryCue: { en: "Hit {count}/5 — ship reset. Recovery window: reacquire the next rock.", "zh-Hant": "遭受第 {count}/5 次撞擊——飛船重置。保護時間內重新鎖定下一顆晶礦。", "zh-Hans": "受到第 {count}/5 次撞击——飞船重置。保护时间内重新锁定下一颗晶矿。", ja: "{count}/5回目の被弾。船がリセットされました。回復猶予中に次の岩を狙い直そう。", ko: "피격 {count}/5 — 함선이 재설정되었습니다. 회복 보호 시간에 다음 바위를 다시 조준하세요.", es: "Impacto {count}/5 — la nave se reinició. Durante la recuperación, fija la próxima roca.", "pt-BR": "Acerto {count}/5 — a nave foi reiniciada. Na recuperação, mire a próxima rocha.", fr: "Impact {count}/5 — le vaisseau est réinitialisé. Pendant la récupération, visez le prochain rocher.", de: "Treffer {count}/5 – das Schiff wurde zurückgesetzt. Richte während der Schutzzeit den nächsten Felsen neu aus.", it: "Colpo {count}/5 — la nave è stata ripristinata. Durante il recupero, mira al prossimo asteroide.", ru: "Попадание {count}/5 — корабль сброшен. В окно восстановления наведитесь на следующий осколок.", hi: "हिट {count}/5 — जहाज़ रीसेट हुआ। सुरक्षा समय में अगले पत्थर पर फिर निशाना लगाएँ।", ar: "إصابة {count}/5 — أُعيد ضبط المركبة. خلال نافذة التعافي وجّهها نحو الصخرة التالية." },
  ...
  function updateHud() {
    ...
    if (ui.mainProgress) {
      const total = state.maze?.pelletTotal || 0;
      const cleared = total ? total - state.maze.pellets.size : 0;
      ui.mainProgress.textContent = gameId === "maze"
        ? `${tr("progress")}: ${cleared}/${total}`
        : `${tr("wave")}: ${state.level}`;
    }
