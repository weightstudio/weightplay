(()=>{
  ...
  const fallback={unlocked:1,completed:{},stars:{},best:{},supply:0,crystal:0,commandLevel:0,moraleLevel:0,lens:false};
  function readSave(){try{return{...fallback,...JSON.parse(localStorage.getItem(SAVE_KEY)||'{}')}}catch{return{...fallback}}}
  let save=readSave(),stageIndex=Math.max(0,Math.min(29,Number(save.unlocked||1)-1)),run=null,selectedNode=-1,formation='',inputLocked=false,lastFocus=null;
  ...
  function startBattle(index){stageIndex=index;const data=stageData(index);run={data,turn:1,morale:10+save.moraleLevel*2,supply:4,command:2+save.commandLevel,owned:new Set([0]),scouted:new Set(),failed:new Set(),freeScout:save.lens,ended:false};selectedNode=-1;formation='';screen('battle');$('result').hidden=true;$('leave').hidden=true;$('battleLive').hidden=false;$('battleLive').inert=false;renderBattle();track('game_start')}
  ...
  function finish(won){run.ended=true;const data=run.data;$('battleLive').hidden=true;$('battleLive').inert=true;$('result').hidden=false;const stars=won?Math.max(1,3-Math.floor(run.failed.size/2)):0;$('resultKicker').textContent=won?t('secured'):t('defeat');$('resultTitle').textContent=won?t('victory'):t('defeat');$('resultText').textContent=won?t('victoryText'):t('defeatText');$('resultRewards').textContent=won?`★ ${stars} · +${data.rewardSupply} ${t('supply')} · +${data.rewardCrystal} ${t('crystal')}`:'';$('nextRegion').hidden=!won||stageIndex>=29;if(won){save.completed[stageIndex]=true;save.stars[stageIndex]=Math.max(save.stars[stageIndex]||0,stars);save.unlocked=Math.max(save.unlocked,Math.min(30,stageIndex+2));save.supply+=data.rewardSupply;save.crystal+=data.rewardCrystal;persist();track('game_complete',{stars})}else track('game_fail');requestAnimationFrame(()=>(won&&!$('nextRegion').hidden?$('nextRegion'):$('retry')).focus({preventScroll:true}))}
  function openLeave(){if(!run||run.ended){returnToStage();return}lastFocus=document.activeElement;$('leave').hidden=false;$('battleLive').inert=true;requestAnimationFrame(()=>$('continueBattle').focus({preventScroll:true}))}
  function closeLeave(){$('leave').hidden=true;$('battleLive').inert=false;lastFocus?.focus?.({preventScroll:true})}
  function returnToStage(){$('leave').hidden=true;$('result').hidden=true;screen('stage');renderStage();requestAnimationFrame(()=>document.querySelector('.stage-card.selected')?.focus({preventScroll:true}))}
  function bind(){
    ...
    document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!$('leave').hidden){event.preventDefault();closeLeave()}});
  }
  ...
  window.__animalFrontierDominionTest={stageData,startBattle,snapshot:()=>({save,stageIndex,run:run?{...run,owned:[...run.owned],scouted:[...run.scouted]}:null})};
})();
