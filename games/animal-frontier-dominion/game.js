(()=>{
  ...
  Object.assign(en,{command:'Command',freeScout:'Scout · free',vanguardCounter:'Counters Scouts',rangerCounter:'Counters Wardens',guardCounter:'Counters Raiders',needResourceDetail:'Need {cost} {resource}. Current: {current}.',purchasedDetail:'{name} upgraded to level {level}/3.',needDiamondsDetail:'Need 25 diamonds. Current: {current}.',repelledCounter:'Formation mismatch. Use {formation} against {defender}. Morale -{loss}.',defeatKicker:'EXPEDITION ENDED'});
  Object.assign(zhHant,{command:'指揮',freeScout:'偵察 · 免費',vanguardCounter:'克制遊騎隊',rangerCounter:'克制守衛隊',guardCounter:'克制突擊隊',needResourceDetail:'需要 {cost} {resource}，目前有 {current}。',purchasedDetail:'{name} 已升級至 {level}/3 級。',needDiamondsDetail:'需要 25 顆鑽石，目前有 {current} 顆。',repelledCounter:'陣型不利；對付{defender}請使用{formation}。士氣 -{loss}。',defeatKicker:'遠征受挫'});
  let locale=window.WonderI18n?.actualLocale?.()||document.documentElement.lang||'en';
  const runtimeTranslate=value=>window.WeightPlayGameRuntimeLocalizer?.translate?.(String(value))||String(value),dictionaries={en,'zh-Hant':zhHant,...(window.ANIMAL_FRONTIER_DOMINION_LOCALES||{})};
  ...
  function track(event,data={}){if(event==='game_complete')window.WonderSound?.play?.('win');else if(event==='game_fail')window.WonderSound?.play?.('wrong');window.WonderAnalytics?.track?.(event,{gameId:GAME_ID,stage:stageIndex+1,release:'internal',...data})}
  ...
  function markCentered(index){
    $$('#stageRail .stage-card').forEach((card,i)=>{const current=i===index;card.classList.toggle('centered',current);if(current)card.setAttribute('aria-current','true');else card.removeAttribute('aria-current')});
  }
  function renderStage(center=true){
    ...
    for(let i=0;i<30;i++){
      ...
      card.addEventListener('click',()=>{if(rail.dataset.dragging==='true')return;if(locked){markCentered(i);return}stageIndex=i;startBattle(i)});
      rail.append(card);
    }
  ...
  function startBattle(index){clearMarchTimer();pendingLeave=false;inputLocked=false;stageIndex=Math.max(0,Math.min(29,boundedInteger(index,0,29)));const data=stageData(stageIndex);run={data,turn:1,morale:10+save.moraleLevel*2,supply:4,command:2+save.commandLevel,owned:new Set([0]),scouted:new Set(),failed:new Set(),freeScout:save.lens,ended:false};selectedNode=-1;formation='';screen('battle');$('result').hidden=true;$('leave').hidden=true;$('battleLive').hidden=false;$('battleLive').inert=false;$('battleLive').setAttribute('aria-hidden','false');renderBattle();$('commandValue').textContent=run.command;$('scout').textContent=run.freeScout?t('freeScout'):t('scout');window.WonderSound?.play?.('start');track('game_start')}
  ...
  function march(){
    ...
    const bonus=formationBonus(node.defender),playerPower=run.data.power+2+bonus,enemyPower=node.power+modifier+(bonus?0:2);
    ...
    if(playerPower>=enemyPower){
      ...
      const counter={raiders:'guard',wardens:'ranger',scouts:'vanguard'}[node.defender];
      $('feedback').textContent=t('repelledCounter',{loss,formation:t(counter),defender:t(node.defender)});
  ...
  function bind(){
    ...
    window.addEventListener('wonder:locale-change',event=>{const next=event.detail?.locale;if(!dictionaries[next])return;locale=next;dict=dictionaries[next];applyText();renderMain()});
