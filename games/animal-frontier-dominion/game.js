(()=>{
  ...
  function startBattle(index){clearMarchTimer();pendingLeave=false;inputLocked=false;stageIndex=Math.max(0,Math.min(29,boundedInteger(index,0,29)));const data=stageData(stageIndex);run={data,turn:1,morale:10+save.moraleLevel*2,supply:4,command:2+save.commandLevel,owned:new Set([0]),scouted:new Set(),failed:new Set(),freeScout:save.lens,ended:false};selectedNode=-1;formation='';screen('battle');$('result').hidden=true;$('leave').hidden=true;$('battleLive').hidden=false;$('battleLive').inert=false;$('battleLive').setAttribute('aria-hidden','false');renderBattle();window.WonderSound?.play?.('start');track('game_start')}
  ...
  function finish(won){clearMarchTimer();pendingLeave=false;inputLocked=false;run.ended=true;const data=run.data;$('battleLive').hidden=true;$('battleLive').inert=true;$('battleLive').setAttribute('aria-hidden','true');$('result').hidden=false;const stars=won?Math.max(1,3-Math.floor(run.failed.size/2)):0;$('resultKicker').textContent=won?t('secured'):t('defeat');$('resultTitle').textContent=won?t('victory'):t('defeat');$('resultText').textContent=won?t('victoryText'):t('defeatText');$('resultRewards').textContent=won?`★ ${stars} · +${data.rewardSupply} ${t('supply')} · +${data.rewardCrystal} ${t('crystal')}`:'';const canContinue=won&&stageIndex<29;$('nextRegion').hidden=!canContinue;const primary=canContinue?$('nextRegion'):won?$('resultMap'):$('retry');[$('retry'),$('resultMap'),$('nextRegion')].forEach(button=>button.classList.toggle('primary',button===primary));if(won){save.completed[stageIndex]=true;save.stars[stageIndex]=Math.max(save.stars[stageIndex]||0,stars);save.unlocked=Math.max(save.unlocked,Math.min(30,stageIndex+2));save.supply+=data.rewardSupply;save.crystal+=data.rewardCrystal;persist();track('game_complete',{stars})}else track('game_fail');requestAnimationFrame(()=>primary.focus({preventScroll:true}))}
