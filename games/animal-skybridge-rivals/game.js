(()=>{
  ...
  const __wpReadMeasurement = () => ({ ...__wpMeasurement,
    ...
    paused: Boolean(run?.paused || run?.suspended || lifecyclePaused), node: document.body,
  ...
  const GAME_ID="animal-skybridge-rivals",GAME_VERSION=24,INTERFACE_VERSION=6;
  ...
  function playCue(name,minGap=0){const now=performance.now(),last=cueTimes.get(name)||-Infinity;if(now-last<minGap)return;cueTimes.set(name,now);window.WeightPlayAudio?.play?.(name)}
  function updateSoundToggle(){const muted=window.WeightPlayAudio?.isMuted?.()===true;els.soundToggle.dataset.soundToggle="true";els.soundToggle.textContent=muted?"🔇":"🔊";els.soundToggle.title=t("sound");els.soundToggle.setAttribute("aria-label",t(muted?"soundEnable":"soundDisable"));els.soundToggle.classList.toggle("muted",muted)}
  ...
  function buyUpgrade(key,event){const restoreFocus=document.activeElement?.dataset?.upgrade===key,level=save.upgrades[key]||0;if(level>=5)return;const cost=costs[level];if(save.shards<cost){els.workshopFeedback.textContent=t("noShards");playCue("feedback.error");return}save.shards-=cost;save.upgrades[key]=level+1;persist();els.workshopFeedback.textContent=t("purchased");playCue("reward.upgrade");track("upgrade_purchase",{upgrade:key,from_level:level,to_level:level+1,cost,input_type:eventInputType(event)});renderWorkshop();renderProgress();if(restoreFocus)restoreWorkshopFocus(key)}
  ...
  function collect(actor,color,capacity,isPlayer=false){for(const p of run.pickups){if(p.color!==color||actor.stack>=capacity)continue;if(Math.hypot(p.x-actor.x,p.y-actor.y)<.042){actor.stack++;respawnPickup(p);if(isPlayer){run.player.dash=Math.min(100,run.player.dash+13);const first=!run.firstPickup;run.firstPickup=true;if(first&&run.routeAssistActive){const lane=lanes.reduce((best,x,index)=>Math.abs(x-run.player.x)<Math.abs(lanes[best]-run.player.x)?index:best,0);say(t("routeAssistDock",{lane:lane+1}),1.4,"route-assist-dock")}else say(t("tileCollected"),first?1.05:.6,first?"pickup-first":"pickup");playCue("reward.coin",80)}}}}
  function build(actor,lane,dt,isPlayer=false){
    ...
    if(isPlayer){
      const first=!run.firstBridge;run.firstBridge=true;run.routeAssistActive=false;say(t("building",{lane:lane+1}),first?1.05:.4,first?"bridge-first":"bridge");playCue("feedback.success",140);
    }
  ...
  function useDash(){if(!run||run.ended||run.player.dash<100)return;run.player.dash=0;run.player.dashTime=2.1;say(t("dashUsed"),1);playCue("movement.dash")}
  ...
  function finish(won){if(!run||run.ended)return;if(!won)run.position=playerPosition();run.ended=true;resultActionClaimed=false;playCue(won ? "result.win" : "result.lose");const time=Math.ceil(run.timeLeft),stars=won?1+(time>=Math.ceil(run.stage.time*.42)?1:0)+(run.spills===0?1:0):0;if(won){save.stars[currentStage]=Math.max(save.stars[currentStage]||0,stars);save.best[currentStage]=Math.max(save.best[currentStage]||0,time);save.unlocked=Math.max(save.unlocked,Math.min(30,currentStage+2));save.shards+=4+stars*3;persist()}els.resultKicker.textContent=stageName(run.stage);els.resultTitle.textContent=t(won?"raceWon":"raceLost");els.resultText.textContent=t(won?"winText":"loseText");(__wpNotifyMeasurement(), els.resultPurpose.hidden=!won);els.resultPurpose.textContent=won?t(currentStage<29?"resultPurposeNext":"resultPurposeFinal",{number:currentStage+2}):"";els.resultStats.innerHTML=`<span><b>${t("place")}</b><strong>${won?1:run.position}/4</strong></span><span><b>${t("spills")}</b><strong>${run.spills}</strong></span><span><b>${t("stars")}</b><strong>${"★".repeat(stars)}${"☆".repeat(3-stars)}</strong></span>`;const hasNext=won&&currentStage<29;const primary=hasNext?els.nextRace:won?els.resultStage:els.retry;els.resultStage.disabled=false;els.retry.disabled=false;els.nextRace.hidden=false;els.nextRace.disabled=!hasNext;[els.retry,els.resultStage,els.nextRace].forEach(action=>action.classList.toggle("primary",action===primary));(__wpNotifyMeasurement(), els.result.hidden=false);els.battleLive.inert=true;els.battleLive.setAttribute("aria-hidden","true");renderProgress();track(won?"race_result_win":"race_result_loss",{race:currentStage+1,place:won?1:run.position,spills:run.spills,stars});if(won)track("result_purpose_view",{race:currentStage+1,unlocks_race:Math.min(30,currentStage+2)});clearTimeout(resultFocusTimer);resultFocusTimer=setTimeout(()=>{resultFocusTimer=0;if(document.body.dataset.screen==="battle"&&!els.result.hidden&&!primary.disabled)primary.focus()},0)
  ...
  function suspendLifecycle(){stopPointer();if(run&&!run.ended&&document.body.dataset.screen==="battle"){run.player.targetX=run.player.x;run.player.targetY=run.player.y;(__wpNotifyMeasurement(), lifecyclePaused=true)}}
  function reconcileLifecycle(){if(windowFocused&&documentVisible&&!pageCached){(__wpNotifyMeasurement(), lifecyclePaused=false);lastFrame=performance.now()}else suspendLifecycle()}
  ...
  els.soundToggle.addEventListener("click",()=>{window.WeightPlayAudio?.unlock?.();window.WeightPlayAudio?.setMuted?.(!window.WeightPlayAudio.isMuted());updateSoundToggle();if(!window.WeightPlayAudio?.isMuted?.())playCue("ui.click")});els.dash.addEventListener("click",useDash);els.start.addEventListener("click",event=>{lastInputType=eventInputType(event);setScreen("stage",{entry:"main_start"})});els.stageBack.addEventListener("click",event=>{track("main_return",{from:"race_map",input_type:eventInputType(event)});setScreen("main")});els.battleBack.addEventListener("click",()=>{if(!run||run.ended){setScreen("stage",{entry:"battle_back"});return}(__wpNotifyMeasurement(), run.paused=true);(__wpNotifyMeasurement(), els.leave.hidden=false);els.battleLive.inert=true;els.continueBattle.focus()});els.continueBattle.addEventListener("click",()=>{(__wpNotifyMeasurement(), els.leave.hidden=true);(__wpNotifyMeasurement(), run.paused=false);els.battleLive.inert=false;els.battleBack.focus()});els.leaveStage.addEventListener("click",()=>{(__wpNotifyMeasurement(), els.leave.hidden=true);if(run)run.ended=true;els.battleLive.inert=false;els.battleLive.removeAttribute("aria-hidden");setScreen("stage",{entry:"battle_leave"});centerStage(currentStage,false)});els.battleHelp.addEventListener("click",()=>{(__wpNotifyMeasurement(), run.paused=true);(__wpNotifyMeasurement(), els.tutorial.hidden=false);els.battleLive.inert=true;els.tutorialDone.focus()});els.tutorialDone.addEventListener("click",()=>{save.tutorial=true;persist();(__wpNotifyMeasurement(), els.tutorial.hidden=true);(__wpNotifyMeasurement(), run.paused=false);els.battleLive.inert=false;lastFrame=performance.now();els.arena.focus?.()});els.retry.addEventListener("click",event=>{if(!claimResultAction())return;track("retry",{from:"result",race:currentStage+1,input_type:eventInputType(event)});__wpReplayStart(() => startRace(currentStage,{skipTutorial:true,inputType:eventInputType(event),entry:"retry"}))});els.resultStage.addEventListener("click",event=>{if(!claimResultAction())return;lastInputType=eventInputType(event);(__wpNotifyMeasurement(), els.result.hidden=true);els.battleLive.inert=false;els.battleLive.removeAttribute("aria-hidden");const target=Math.min(save.unlocked-1,29);setScreen("stage",{entry:"result_map"});focusStageCardAfterSettlement(target)});els.nextRace.addEventListener("click",event=>{if(!claimResultAction())return;track("next_race",{from:"result",from_race:currentStage+1,to_race:Math.min(30,currentStage+2),input_type:eventInputType(event)});startRace(Math.min(29,currentStage+1),{skipTutorial:true,inputType:eventInputType(event),entry:"next_race"})});
  ...
  window.__animalSkybridgeRivalsSmoke={
    ...
    grantTiles(n=10){if(run)run.player.stack=Math.min(30,run.player.stack+n);updateHud()},setPlayer(x,y){if(run){run.player.x=run.player.targetX=x;run.player.y=run.player.targetY=y}},setTarget(x,y){if(run){run.player.targetX=x;run.player.targetY=y}},grantProgress(n){if(run)run.player.progress=Math.min(run.stage.goal,run.player.progress+n);updateHud()},setRivalProgress(index=0,n=0){if(run?.rivals[index])run.rivals[index].progress=Math.max(0,Math.min(run.stage.goal,n));updateHud()},step(seconds=.5){if(!run)return;for(let i=0;i<Math.ceil(seconds/0.02);i++)simulate(.02);draw()},complete(won=true){finish(won)},unlockAll(){save.unlocked=30;persist();renderStages()},reset(){save=freshSave();persist();renderProgress();renderStages();renderWorkshop()},setLocale(code){if(L.codes.includes(code)){locale=code;setText()}},get save(){return JSON.parse(JSON.stringify(save))}
  };
