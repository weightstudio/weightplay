(()=>{
  ...
  function renderStages(){if(!els.stageRail)return;els.stageRail.innerHTML="";stages.forEach(stage=>{const unlocked=stage.index<save.unlocked;const card=document.createElement("button");card.type="button";card.className=`stage-card${unlocked?"":" locked"}`;card.dataset.stageIndex=String(stage.index);card.tabIndex=-1;card.disabled=false;card.setAttribute("aria-disabled",String(!unlocked));card.innerHTML=`<small>${t(chapterKeys[stage.chapter])}</small><strong>${t("race",{number:stage.number})}</strong><span>${stage.goal} ◈ · ${stage.time}s</span><span>${unlocked?`★ ${save.stars[stage.index]||0}`:"🔒"}</span>`;card.addEventListener("click",event=>{selectCentered(card);if(!unlocked){els.stageHint.textContent=t("stageLocked");return}reclaimVisibleForeground(event);startRace(stage.index)});els.stageRail.append(card)});requestAnimationFrame(()=>centerStage(Math.min(save.unlocked-1,29),false))}
  function selectCentered(card){els.stageRail.querySelectorAll(".stage-card").forEach(c=>{const selected=c===card;c.classList.toggle("centered",selected);c.tabIndex=selected?0:-1;if(selected)c.setAttribute("aria-current","true");else c.removeAttribute("aria-current")})}
  ...
  function moveStageFocus(key){const cards=[...els.stageRail.querySelectorAll(".stage-card")];if(!cards.length)return;const current=Math.max(0,cards.indexOf(document.activeElement)),rtl=getComputedStyle(els.stageRail).direction==="rtl",step=key==="ArrowRight"?(rtl?-1:1):(rtl?1:-1),target=key==="Home"?cards[0]:key==="End"?cards.at(-1):cards[Math.max(0,Math.min(cards.length-1,current+step))];if(!target)return;centerStage(Number(target.dataset.stageIndex));target.focus({preventScroll:true})}
  ...
  function buyUpgrade(key){const restoreFocus=document.activeElement?.dataset?.upgrade===key,level=save.upgrades[key]||0;if(level>=5)return;const cost=costs[level];if(save.shards<cost){els.workshopFeedback.textContent=t("noShards");playCue("wrong");return}save.shards-=cost;save.upgrades[key]=level+1;persist();els.workshopFeedback.textContent=t("purchased");playCue("upgrade");renderWorkshop();renderProgress();if(restoreFocus)restoreWorkshopFocus(key)}
  ...
  let run=null,lastFrame=0,raf=0,keys=new Set(),activePointerId=null,lifecyclePaused=false,windowFocused=document.hasFocus(),documentVisible=!document.hidden,pageCached=false,resultActionClaimed=false;
  ...
  function finish(won){if(!run||run.ended)return;run.ended=true;resultActionClaimed=false;playCue(won?"win":"wrong");const time=Math.ceil(run.timeLeft),stars=won?1+(time>=Math.ceil(run.stage.time*.42)?1:0)+(run.spills===0?1:0):0;if(won){save.stars[currentStage]=Math.max(save.stars[currentStage]||0,stars);save.best[currentStage]=Math.max(save.best[currentStage]||0,time);save.unlocked=Math.max(save.unlocked,Math.min(30,currentStage+2));save.shards+=4+stars*3;persist()}els.resultKicker.textContent=stageName(run.stage);els.resultTitle.textContent=t(won?"raceWon":"raceLost");els.resultText.textContent=t(won?"winText":"loseText");els.resultStats.innerHTML=`<span><b>${t("place")}</b><strong>${won?1:run.position}/4</strong></span><span><b>${t("spills")}</b><strong>${run.spills}</strong></span><span><b>${t("stars")}</b><strong>${"★".repeat(stars)}${"☆".repeat(3-stars)}</strong></span>`;const hasNext=won&&currentStage<29;const primary=hasNext?els.nextRace:won?els.resultStage:els.retry;els.resultStage.disabled=false;els.retry.disabled=false;els.nextRace.hidden=false;els.nextRace.disabled=!hasNext;[els.retry,els.resultStage,els.nextRace].forEach(action=>action.classList.toggle("primary",action===primary));els.result.hidden=false;els.battleLive.inert=true;els.battleLive.setAttribute("aria-hidden","true");renderProgress();setTimeout(()=>primary.focus(),0)}
  function claimResultAction(){if(els.result.hidden||resultActionClaimed)return false;resultActionClaimed=true;[els.retry,els.resultStage,els.nextRace].forEach(action=>action.disabled=true);return true}
  ...
  els.arena.addEventListener("pointerdown",ev=>{if(activePointerId!==null||!ev.isPrimary)return;activePointerId=ev.pointerId;els.arena.setPointerCapture(ev.pointerId);setTarget(ev)});els.arena.addEventListener("pointermove",ev=>{if(activePointerId===ev.pointerId)setTarget(ev)});els.arena.addEventListener("pointerup",ev=>releasePointer(ev.pointerId));els.arena.addEventListener("pointercancel",ev=>releasePointer(ev.pointerId,{cancelled:true}));
  ...
  function reclaimVisibleForeground(event){if(!event?.isTrusted||document.hidden||pageCached)return false;if(windowFocused)return true;windowFocused=true;reconcileLifecycle();return true}
