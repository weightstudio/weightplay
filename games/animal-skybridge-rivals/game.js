(()=>{
  ...
  function build(actor,lane,dt,isPlayer=false){
    ...
    run.projectiles.push({x:lanes[lane],y:.44,targetX:.5+(lane-1)*.11,targetY:.095,color,life:.46,maxLife:.46,damage:1});
    if(isPlayer){
      say(t("building",{lane:lane+1}),.4);playCue("success",140);
      if(run.stage.mode==="switch"&&actor.progress%Math.max(2,Math.ceil(run.stage.goal/3))===0)run.activeLane=(run.activeLane+1)%3;
  ...
  function draw(){
    ...
    const tint={meadow:"rgba(3,24,46,.14)",split:"rgba(55,19,82,.22)",islands:"rgba(0,82,86,.20)",cross:"rgba(85,46,8,.18)",slalom:"rgba(15,32,92,.24)"}[run?.stage.layout]||"rgba(3,24,46,.14)";
    ...
    drawGuidance(w,h);
    run.rivals.forEach((r,i)=>{drawStack(r,i+1,w,h);drawSprite(i+1,r.x*w,r.y*h,84)});
    drawStack(run.player,0,w,h);drawSprite(0,run.player.x*w,run.player.y*h,90,save.championAura&&run.player.dashTime>0);
