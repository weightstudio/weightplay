(function () {
  ...
  function release() { state.releases += 1; $("releaseCount").textContent = String(state.releases); const target = rounds[state.round].angle; const delta = shortestDelta(state.angle, target); if (delta <= 16) { $("battleStatus").textContent = t("close"); $("battleStatus").className = "battle-status is-good"; tone("feedback.success"); window.setTimeout(() => { if (state.round < rounds.length - 1) { state.round += 1; state.angle = 0; $("battleStatus").textContent = t("ready"); $("battleStatus").className = "battle-status"; renderBattle(); $("angleInput").focus(); } else finish(); }, 360); return; } const side = ((state.angle - target + 360) % 360) < 180 ? "ahead" : "behind"; $("battleStatus").textContent = `${t("miss", { delta })} ${t(side)}`; $("battleStatus").className = "battle-status is-miss"; tone("feedback.error"); }
