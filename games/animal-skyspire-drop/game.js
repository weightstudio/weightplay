(async () => {
  "use strict";
  const {Skyspire3D}=await import("./skyspire-3d.js?v=20260908-true3d-v17");
  let view3d=null;
  ...
  function startBattle(index,{skipTutorial=false}={}){stageIndex=Math.max(0,Math.min(29,index));const stage=buildStage(stageIndex);run={stage,current:0,rotation:0,rotationTarget:0,height:2.3,velocity:0,bounce:0.18,transition:0,departing:null,elapsed:0,accum:0,lastHudAt:0,combo:0,bestCombo:0,power:false,shield:save.upgrades.aegis>=2?2:save.upgrades.aegis>=1?1:0,paused:false,ended:false,result:null,lastFeedback:"",firstRotationShown:false,firstRotationFeedbackUntil:0,pendingFeedback:"",dragging:false,dragPointerId:null,lastX:0,keys:new Set(),landings:0,shieldsUsed:0};els.resultModal.hidden=true;els.leaveModal.hidden=true;els.helpModal.hidden=true;setBattleIsolation(false);els.feedback.textContent="";try{ensure3D();view3d.setStage(stage)}catch(error){release3D();els.forgeFeedback.textContent=commonText("3D graphics could not start. Please reload or use a browser with WebGL enabled.");console.error(error);setScreen("stage");return}setScreen("battle");syncArenaSize();renderHud();refreshBattleUtility();track("game_start");if(!save.tutorial&&!skipTutorial&&!interfaceValidationRun){run.paused=true;openBattleModal(els.helpModal,els.helpClose)}else{save.tutorial=true;persist();focusArena()}}
  ...
  function recordLanding(sector,outcome,details={}){const payload={ring:run.current+1,sector,outcome,...details};track("landing",payload);if(sector==="gap")track("gap_result",payload);if(sector==="hazard")track("hazard_result",payload)}
  function passRing(kind){const cleared=run.current;run.current++;run.combo++;run.bestCombo=Math.max(run.bestCombo,run.combo);if(run.combo>=threshold())run.power=true;run.departing={index:cleared,alpha:1};run.transition=0;if(kind==="smash")track("comet_smash",{ring:cleared+1,combo:run.combo});run.lastFeedback=t(kind==="gap"?"gapPass":"cometSmash",{n:run.combo});setFeedback(run.lastFeedback);if(run.current>=run.stage.ringCount)finish(true)}
  ...
  function syncArenaSize(){view3d?.resize()}
  function release3D(){
    if(!view3d)return;
  ...
  function draw(){if(run&&view3d)view3d.render(run,ringAngle,hazardWidthAt)}
