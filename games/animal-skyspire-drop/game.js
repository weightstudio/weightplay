(() => {
  ...
  function centerCurrentStage(generation=sceneGeneration){if(screen!=="stage"||generation!==sceneGeneration)return;const card=els.stageRail.querySelector(`[data-stage="${stageIndex}"]`);card?.scrollIntoView({inline:"center",block:"nearest",behavior:"instant"});requestAnimationFrame(()=>markCentered(generation))}
  ...
  function focusStageOwner(card){if(!card)return;const generation=sceneGeneration;clearTimeout(stageScrollTimer);card.scrollIntoView({inline:"center",block:"nearest",behavior:"instant"});selectStageOwner(card);card.focus({preventScroll:true});requestAnimationFrame(()=>{if(screen==="stage"&&generation===sceneGeneration&&card.isConnected)selectStageOwner(card)})}
  function markCentered(generation=sceneGeneration){if(screen!=="stage"||generation!==sceneGeneration)return;const cards=[...els.stageRail.querySelectorAll(".stage-card")];if(!cards.length)return;const railRect=els.stageRail.getBoundingClientRect(),cx=(railRect.left+railRect.right)/2;let best=cards[0],distance=Infinity;for(const card of cards){const rect=card.getBoundingClientRect(),d=Math.abs((rect.left+rect.right)/2-cx);if(d<distance){best=card;distance=d}}selectStageOwner(best)}
