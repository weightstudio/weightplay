(() => {
  ...
  let lang=detectLocale(),save=loadSave(),stageIndex=Math.max(0,Math.min(29,save.unlocked-1)),run=null,raf=0,lastFrame=0,screen="main",activeTab="towers",resizeObserver=null,lifecycleSuspended=document.hidden,windowFocused=document.hasFocus(),modalOpener=null,resultActionClaimed=false,forgeDecisionReadyAt={};
  const images={};
  ...
  function renderStages(){if(!els.stageRail)return;refreshStageLocale();els.stageRail.innerHTML="";for(const stage of stages){const locked=stage.index+1>save.unlocked,stars=Number(save.stars[stage.index]||0);const card=document.createElement("button");card.type="button";card.className=`stage-card${locked?" locked":""}`;card.dataset.stage=String(stage.index);card.tabIndex=stage.index===stageIndex?0:-1;card.setAttribute("aria-disabled",String(locked));card.setAttribute("aria-keyshortcuts","ArrowLeft ArrowRight Home End");card.innerHTML=`<small>${t("chapter",{n:stage.chapter+1})} · ${stage.name}</small><strong>${t("stage",{n:stage.index+1})}</strong><span>${t("ringCount",{n:stage.ringCount})} · ${stars?"★".repeat(stars):locked?t("locked"):t("cleared")}</span><small>${stage.rule}</small>`;card.addEventListener("focus",()=>selectStageOwner(card));card.addEventListener("click",()=>{if(!locked)startBattle(stage.index)});els.stageRail.append(card)}els.stageSummary.textContent=`${Math.min(save.unlocked,30)} / 30 · ★ ${Object.values(save.stars).reduce((sum,n)=>sum+Number(n||0),0)}`;selectStageOwner(els.stageRail.querySelector(`[data-stage="${stageIndex}"]`)||els.stageRail.firstElementChild)}
  ...
  function selectStageOwner(best){if(!best)return;const cards=[...els.stageRail.querySelectorAll(".stage-card")];cards.forEach(card=>{const current=card===best;card.classList.toggle("centered",current);card.tabIndex=current?0:-1;if(current)card.setAttribute("aria-current","true");else card.removeAttribute("aria-current")});stageIndex=Number(best.dataset.stage)}
  function focusStageOwner(card){if(!card)return;clearTimeout(stageScrollTimer);card.scrollIntoView({inline:"center",block:"nearest",behavior:"instant"});selectStageOwner(card);card.focus({preventScroll:true});requestAnimationFrame(()=>selectStageOwner(card))}
  ...
  function openBattleModal(modal,focusTarget,opener=null){modalOpener=opener?.isConnected?opener:null;modal.hidden=false;setBattleIsolation(true);requestAnimationFrame(()=>focusTarget?.focus({preventScroll:true}))}
  ...
  function syncResultActions(won){resultActionClaimed=false;const canAdvance=won&&stageIndex<29;const primary=canAdvance?els.next:els.resultMap;for(const action of [els.resultMap,els.next,els.retry]){action.hidden=false;action.disabled=action===els.next&&!canAdvance;action.classList.toggle("primary",action===primary)}requestAnimationFrame(()=>primary.focus({preventScroll:true}))}
