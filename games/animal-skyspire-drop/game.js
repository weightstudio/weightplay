(async () => {
  ...
  function closeBattleModal(modal,{resume=false,restore=true}={}){modal.hidden=true;setBattleIsolation(false);const opener=modalOpener;modalOpener=null;if(resume&&run&&!run.ended){run.paused=false;lastFrame=performance.now();startLoop()}if(restore){if(opener?.isConnected)opener.focus({preventScroll:true});else focusArena()}}
  ...
  function ensure3D(){if(!view3d){view3d=new Skyspire3D(els.arena);view3d.setStage(run.stage);
    els.arena.addEventListener("webglcontextlost",event=>{if(view3d?.canvas!==event.target)return;event.preventDefault();suspendLifecycle();if(run)run.paused=true;showGraphicsError()},{once:true});
  }}
