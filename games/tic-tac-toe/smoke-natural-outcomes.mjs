...
try {
  for (const viewport of viewports) {
    ...
    assert(loss.version === "v8" && loss.outcome === "loss" && loss.winning.length === 3 && loss.winning.every((mark) => mark === "O"), "Tactical rival loss branch failed", { viewport, loss });
  ...
  console.log(JSON.stringify({ status: "PASS", gameVersion: "v8", evidence }, null, 2));
} finally {
  await browser.close();
