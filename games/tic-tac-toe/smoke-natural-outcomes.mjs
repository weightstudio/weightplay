...
try {
  for (const viewport of viewports) {
    ...
    assert(loss.version === "v14" && loss.outcome === "loss" && loss.winning.length === 3 && loss.winning.every((mark) => mark === "O"), "Tactical rival loss branch failed", { viewport, loss });
  ...
  console.log(JSON.stringify({ status: "PASS", gameVersion: "v14", evidence }, null, 2));
} finally {
  await browser.close();
