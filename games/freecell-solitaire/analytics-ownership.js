(() => {
  ...
  const OUTCOMES = new Set(["started", "moved", "occupied", "complete", "failed", "restart", "new_game", "closed", "returned", "visible", "unknown"]);
  const MOVE_TYPES = new Set(["tableauToFoundation", "freeCellToFoundation", "tableauToTableau", "tableauToFreeCell", "freeCellToTableau", "unknown"]);
  ...
  const track = (event, detail = {}) => {
    ...
    try {
      window.WonderAnalytics?.track(event, {
        ...
        free_cell_cards: bounded(detail.freeCellCards, 4),
        remaining_cards: bounded(detail.remainingCards, 52),
      });
    } catch {
