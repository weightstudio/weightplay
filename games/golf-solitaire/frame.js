/* Golf content bindings only. Shared frame owns controls and preferences. */
(() => {
  'use strict';
  let installed = false;
  window.installGolfFrame = view => {
    ...
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', () => window.installGolfFrame(view), {once:true});
      return;
    }
    ...
    if (!root || !main || !battle || !guide || !window.WeightPlayScreenFrame) {
      throw new Error('Golf shared-frame content contract is incomplete');
    }
    ...
    content.className = 'golf-main-content';
    const wrapper = content.querySelector('.poster-frame');
    const image = wrapper.querySelector('img');
    image.className = '';
    image.dataset.wpFramePoster = '';
    image.src = '/assets/interface7-redrawn/golf-solitaire.webp';
    ...
    const sync = () => {
      ...
      const strings = window.GOLF_GUIDE_LOCALES?.[code] || window.GOLF_GUIDE_LOCALES?.en || {};
