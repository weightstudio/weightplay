...
"use strict";
  ...
  const __wpReadMeasurement = () => ({ ...__wpMeasurement,
    ...
    paused: Boolean(run?.paused || run?.suspended || modalOpen), node: document.body,
...
function syncInterface7StageTabs(){if(els.briefTab){const label=INTERFACE7_STAGE_LABELS[locale]||INTERFACE7_STAGE_LABELS.en;els.briefTab.textContent=label;els.briefTab.setAttribute("aria-label",label);els.briefTab.dataset.wpStageSlot="center";els.briefTab.dataset.wpStageRole="stages"}if(els.workshopTab){els.workshopTab.dataset.wpStageSlot="right";els.workshopTab.dataset.wpStageRole="equipment"}}
function installInterface7Compat(){
  if(!document.querySelector('link[href*="interface-7-cleanup.css"]')){const link=document.createElement("link");link.rel="stylesheet";link.href=new URL("interface-7-cleanup.css?v=20260924-interface7-source-cleanup2",document.baseURI).href;document.head.append(link)}
  const stageCanvas=document.querySelector("#stageScreen .stage-canvas"),stageTabs=document.querySelector("#stageScreen .stage-tabs");
  if(els.stageRail){els.stageRail.dataset.wpStageDragOwner="game";els.stageRail.dataset.wpRootvaultLegacyDragSuppressed="true";els.stageRail.addEventListener("wonder:stage-snap",event=>{const index=Number(event.detail?.index);if(!Number.isInteger(index)||index<0)return;els.stageRail.querySelector(`[data-stage-index="${index}"]`)?.focus({preventScroll:true})})}
  if(stageCanvas&&stageTabs&&els.stageRail&&els.briefPanel&&els.workshopPanel){let workspace=stageCanvas.querySelector(":scope > .wp-rootvault-stage-workspace");if(!workspace){workspace=document.createElement("div");workspace.className="wp-rootvault-stage-workspace";workspace.dataset.wpStageWorkspace="";stageCanvas.insertBefore(workspace,stageTabs);workspace.append(els.stageRail,els.briefPanel,els.workshopPanel)}stageTabs.dataset.wpStageTabs="three-slot"}
  els.briefTab?.removeAttribute("data-t");syncInterface7StageTabs();
  const battleActions=document.querySelector("#battleScreen .battle-actions");if(els.battleHelp&&battleActions&&els.battleHelp.parentElement!==battleActions){battleActions.insertBefore(els.battleHelp,battleActions.firstChild);els.battleHelp.dataset.wpGameplayAction="help";els.battleHelp.removeAttribute("data-wp-battle-utility")}
  if(els.resultModal)els.resultModal.dataset.wpBattleSubstate="result";
...
function createBuilder(index,chapter,variant){const rooms=[],pins=[],solution=[];let counter=0;const addRoom=(id,x,y,contents=[],type="")=>{rooms.push({id,x,y,contents:[...contents],type,lock:type==="lock"?"rune":""});return id};const addPin=(from,to,{required=true,lock="",id=""}={})=>{const pinId=id||`p${chapter}${variant}${counter++}`;pins.push({id:pinId,from,to,lock});if(required)solution.push(pinId);return pinId};const route=(from,to,prefix,relay)=>{if(relay){addPin(from,relay,{id:`${prefix}a`});addPin(relay,to,{id:`${prefix}b`})}else addPin(from,to,{id:prefix})};const finish=()=>{rooms.forEach(room=>{[room.x,room.y]=transformPoint([room.x,room.y],variant)});return{index,chapter,variant,rooms,pins,solution,par:solution.length}};return{addRoom,addPin,route,finish,rooms,pins,solution}}
function buildLevel(index){const chapter=Math.floor(index/5),v=index%5,b=createBuilder(index,chapter,v),r=b.addRoom,p=b.addPin,route=b.route;const relayA=v>=2,relayC=v===1||v>=3;
  if(chapter===0){r("A",.16,.2,["taro"]);r("C",.82,.2,["core"]);r("S",.5,.82,[],"sanctuary");r("V",.88,.72,[],"void");if(relayA)r("MA",.22,.52);if(relayC)r("MC",.75,.52);route("A","S","a",relayA?"MA":"");route("C","S","c",relayC?"MC":"");if(v>=1)p("A","V",{required:false,id:"d1"});if(v>=4)p("C","V",{required:false,id:"d2"})}
  if(chapter===1){r("W",.12,.16,["water"]);r("E",.46,.28,["ember"]);r("A",.86,.16,["taro"]);r("C",.83,.72,["core"]);r("S",.47,.84,[],"sanctuary");r("V",.12,.74,[],"void");if(relayA)r("MA",.7,.48);if(relayC)r("MC",.68,.68);p("W","E",{id:"water"});route("A","E","taro",relayA?"MA":"");p("E","S",{id:"safe"});route("C","S","core",relayC?"MC":"");if(v>=1)p("A","V",{required:false,id:"danger"});if(v>=4)p("C","V",{required:false,id:"d2"})}
  if(chapter===2){r("H",.1,.18,["ward"]);r("A",.48,.18,["taro"]);r("X",.48,.54,["shadow"]);r("C",.84,.22,["core"]);r("S",.48,.84,[],"sanctuary");r("V",.86,.73,[],"void");if(relayA)r("MA",.28,.42);if(relayC)r("MC",.75,.55);p("H","A",{id:"ward"});route("A","X","taro",relayA?"MA":"");p("X","S",{id:"shadow"});route("C","S","core",relayC?"MC":"");if(v>=1)p("A","V",{required:false,id:"danger"});if(v>=4)p("C","X",{required:false,id:"d2"})}
  if(chapter===3){r("W",.1,.12,["water"]);r("E",.33,.28,["ember"]);r("H",.62,.1,["ward"]);r("A",.86,.2,["taro"]);r("X",.54,.58,["shadow"]);r("C",.85,.7,["core"]);r("S",.44,.86,[],"sanctuary");r("V",.1,.72,[],"void");if(relayA)r("MA",.69,.4);if(relayC)r("MC",.7,.78);p("W","E",{id:"water"});p("H","A",{id:"ward"});route("A","E","taro",relayA?"MA":"");p("E","X",{id:"ember"});p("X","S",{id:"shadow"});route("C","S","core",relayC?"MC":"");if(v>=1)p("A","X",{required:false,id:"danger"});if(v>=4)p("C","X",{required:false,id:"d2"})}
  if(chapter===4){r("K",.12,.16,["key"]);r("L",.42,.34,["lock"],"lock");r("A",.84,.15,["taro"]);r("C",.82,.72,["core"]);r("S",.42,.84,[],"sanctuary");r("V",.12,.73,[],"void");if(relayA)r("MA",.68,.43);if(relayC)r("MC",.67,.67);p("K","L",{id:"key"});if(relayA){p("A","MA",{id:"taroa",lock:"rune"});p("MA","L",{id:"tarob"})}else p("A","L",{id:"taro",lock:"rune"});p("L","S",{id:"lockway"});route("C","S","core",relayC?"MC":"");if(v>=1)p("K","V",{required:false,id:"d1"});if(v>=4)p("C","V",{required:false,id:"d2"})}
  if(chapter===5){r("W",.08,.1,["water"]);r("E",.3,.3,["ember"]);r("H",.56,.08,["ward"]);r("A",.86,.16,["taro"]);r("K",.09,.52,["key"]);r("L",.36,.56,["lock"],"lock");r("X",.62,.55,["shadow"]);r("C",.88,.68,["core"]);r("S",.5,.87,[],"sanctuary");r("V",.12,.82,[],"void");p("W","E",{id:"water"});p("H","A",{id:"ward"});p("K","L",{id:"key"});p("A","E",{id:"taro",lock:"rune"});p("E","X",{id:"ember"});p("X","S",{id:"shadow"});p("C","S",{id:"core"});if(v>=1)p("A","X",{required:false,id:"d1"});if(v>=2)p("C","X",{required:false,id:"d2"});if(v>=3)p("W","S",{required:false,id:"d3"});if(v>=4)p("K","V",{required:false,id:"d4"})}
  return b.finish()}
const levels=Array.from({length:30},(_,index)=>buildLevel(index));
function createRun(level){return{level,rooms:Object.fromEntries(level.rooms.map(room=>[room.id,{...room,contents:[...room.contents]}])),pins:Object.fromEntries(level.pins.map(pin=>[pin.id,{...pin,pulled:false}])),unlocked:new Set(),armor:false,pulls:0,ended:false,result:null,events:[]}}
function removeOne(contents,type){const index=contents.indexOf(type);if(index<0)return false;contents.splice(index,1);return true}
function resolveRoom(state,room){const events=[];if(room.contents.includes("key")&&room.contents.includes("lock")){removeOne(room.contents,"key");removeOne(room.contents,"lock");state.unlocked.add(room.lock||"rune");events.push("unlocked")}
...
function simulateSolution(level){const state=createRun(level);for(const pinId of level.solution){const result=applyPin(state,pinId);if(!result.ok)return{pass:false,state,pinId,reason:result.reason}}return{pass:Boolean(state.ended&&state.result?.win),state}}
function initLocale(){const path=location.pathname.split("/").filter(Boolean)[0],routeMap={en:"en","zh-tw":"zh-Hant","zh-cn":"zh-Hans",ja:"ja",ko:"ko",es:"es","pt-br":"pt-BR",fr:"fr",de:"de",it:"it",ru:"ru",hi:"hi",ar:"ar"},sharedLocale=window.WonderI18n?.actualLocale?.()||storage.get("weightPlayLocale")||storage.get("weightplay:locale"),saved=storage.get("weightplay:animal-rootvault-pins:locale")||sharedLocale;locale=routeMap[path]||(CODES.includes(saved)?saved:"en");if(!CODES.includes(locale))locale="en";els.localeSelect.innerHTML=CODES.map(code=>`<option value="${code}">${LABELS[code]}</option>`).join("");els.localeSelect.value=locale}
...
function createStageCard(poolIndex){const card=document.createElement("button");card.type="button";card.className="stage-card";card.dataset.wpStagePoolNode=String(poolIndex+1);card.innerHTML="<small></small><strong></strong><span></span><small></small>";return card}
function bindStageCard(card,index){const level=levels[index],locked=index+1>save.unlocked;card.disabled=false;card.dataset.stageIndex=String(index);card.dataset.index=String(index);card.className=`stage-card${locked?" locked":""}`;card.setAttribute("aria-disabled",String(locked));card.setAttribute("aria-keyshortcuts","ArrowLeft ArrowRight Home End");card.setAttribute("aria-posinset",String(index+1));card.setAttribute("aria-setsize",String(levels.length));card.setAttribute("aria-label",`${stageName(index)}, ${locked?t("locked"):t("pulls")} ${level.par}`);const parts=card.children;parts[0].textContent=t("chapter",{n:level.chapter+1});parts[1].textContent=stageName(index);parts[2].textContent=`${locked?t("locked"):t("pulls")} · ${level.par}`;parts[3].textContent=t("chapterRules")[level.chapter]}
function syncStageCards(){for(const card of stageCardPool){const index=Number(card.dataset.stageIndex);bindStageCard(card,index);const active=index===stageIndex;card.classList.toggle("centered",active);card.tabIndex=active?0:-1;if(active)card.setAttribute("aria-current","true");else card.removeAttribute("aria-current");if(index===Math.max(0,save.unlocked-1))card.dataset.wpStageRecommended="true";else delete card.dataset.wpStageRecommended}}
function buildStagePool(){stageWindowStart=desiredStageWindow(stageIndex);els.stageRail.replaceChildren();stageCardPool=Array.from({length:Math.min(STAGE_CARD_POOL_SIZE,levels.length)},(_,offset)=>{const card=createStageCard(offset);bindStageCard(card,stageWindowStart+offset);els.stageRail.append(card);return card});Object.assign(els.stageRail.dataset,{wpStageVirtualized:"bounded-recycle",wpStagePoolSize:String(stageCardPool.length),wpStageTotal:String(levels.length),wpStageWindowStart:String(stageWindowStart),wpStageWindowEnd:String(stageWindowStart+stageCardPool.length-1),wpStageRecycleCount:"0",wpStageCenterObserver:"manual",wpStageVirtualDrag:"true"})}
function moveStageWindow(targetStart){const target=Math.max(0,Math.min(stageWindowLimit(),targetStart));let recycled=0;while(stageWindowStart<target){const card=els.stageRail.firstElementChild,anchor=card?.nextElementSibling,before=anchor?.getBoundingClientRect().left;stageWindowStart++;els.stageRail.append(card);bindStageCard(card,stageWindowStart+stageCardPool.length-1);recycled++;const after=anchor?.getBoundingClientRect().left;if(Number.isFinite(before)&&Number.isFinite(after))els.stageRail.scrollLeft+=after-before}while(stageWindowStart>target){const card=els.stageRail.lastElementChild,anchor=card?.previousElementSibling,before=anchor?.getBoundingClientRect().left;stageWindowStart--;els.stageRail.prepend(card);bindStageCard(card,stageWindowStart);recycled++;const after=anchor?.getBoundingClientRect().left;if(Number.isFinite(before)&&Number.isFinite(after))els.stageRail.scrollLeft+=after-before}stageCardPool=[...els.stageRail.children];Object.assign(els.stageRail.dataset,{wpStageWindowStart:String(stageWindowStart),wpStageWindowEnd:String(stageWindowStart+stageCardPool.length-1)});if(recycled)els.stageRail.dataset.wpStageRecycleCount=String(Number(els.stageRail.dataset.wpStageRecycleCount||0)+recycled);return recycled}
function stageGeometry(){const rail=els.stageRail.getBoundingClientRect(),first=stageCardPool[0]?.getBoundingClientRect(),second=stageCardPool[1]?.getBoundingClientRect(),delta=first&&second?(second.left+second.width/2)-(first.left+first.width/2):0;return{center:rail.left+rail.width/2,pitch:Math.abs(delta)||((first?.width||250)+18),orientation:Math.sign(delta)||1}}
function nearestStageCard(){const geometry=stageGeometry();return stageCardPool.reduce((best,card)=>{const rect=card.getBoundingClientRect(),distance=Math.abs(rect.left+rect.width/2-geometry.center);return!best||distance<best.distance?{card,distance}:best},null)?.card}
function currentStagePosition(){const card=nearestStageCard();if(!card)return stageIndex;const rect=card.getBoundingClientRect(),geometry=stageGeometry();return Math.max(0,Math.min(levels.length-1,Number(card.dataset.stageIndex)+(geometry.center-(rect.left+rect.width/2))/(geometry.pitch*geometry.orientation)))}
function positionStageRail(position){const logical=Math.max(0,Math.min(levels.length-1,position)),anchorIndex=Math.round(logical),recycled=moveStageWindow(desiredStageWindow(anchorIndex));if(recycled)syncStageCards();const card=els.stageRail.querySelector(`[data-stage-index="${anchorIndex}"]`);if(!card)return logical;card.scrollIntoView({inline:"center",block:"nearest",behavior:"instant"});const geometry=stageGeometry(),fraction=logical-anchorIndex;if(Math.abs(fraction)>.0001)els.stageRail.scrollLeft+=fraction*geometry.orientation*geometry.pitch;els.stageRail.dataset.wpStageDragLogical=logical.toFixed(4);return logical}
function selectLogicalStage(index,{focus=false}={}){stageIndex=Math.max(0,Math.min(levels.length-1,index));if(!stageCardPool.length)buildStagePool();moveStageWindow(desiredStageWindow(stageIndex));positionStageRail(stageIndex);syncStageCards();renderBrief();if(focus)els.stageRail.querySelector(`[data-stage-index="${stageIndex}"]`)?.focus({preventScroll:true})}
function installVirtualStageDrag(){if(els.stageRail.dataset.wpStageDragOwner==="game")return;els.stageRail.dataset.wpStageDragOwner="game";let owner=null,startX=0,lastX=0,logical=0,moved=false,suppressClick=false,suppressTimer=0;const restore=()=>{els.stageRail.style.removeProperty("scroll-behavior");els.stageRail.style.removeProperty("scroll-snap-type");delete els.stageRail.dataset.wpStageSettling;delete els.stageRail.dataset.wpDragDown;els.stageRail.classList.remove("wp-stage-dragging")};cancelStageInteraction=()=>{owner=null;clearTimeout(suppressTimer);suppressTimer=0;suppressClick=false;cancelAnimationFrame(stageSettleFrame);stageSettleFrame=0;restore()};els.stageRail.addEventListener("pointerdown",event=>{if(event.isPrimary===false||(event.button!==undefined&&event.button!==0))return;clearTimeout(suppressTimer);suppressTimer=0;suppressClick=false;cancelAnimationFrame(stageSettleFrame);stageSettleFrame=0;owner=event.pointerId;startX=lastX=event.clientX;logical=currentStagePosition();moved=false;els.stageRail.style.setProperty("scroll-behavior","auto","important");els.stageRail.style.setProperty("scroll-snap-type","none","important");els.stageRail.dataset.wpDragDown="1";event.stopImmediatePropagation()},true);document.addEventListener("pointermove",event=>{if(event.pointerId!==owner)return;const delta=event.clientX-lastX;lastX=event.clientX;if(!moved&&Math.abs(event.clientX-startX)>4){moved=true;els.stageRail.classList.add("wp-stage-dragging")}if(moved){const rect=els.stageRail.getBoundingClientRect(),scale=rect.width?els.stageRail.clientWidth/rect.width:1;if(event.cancelable)event.preventDefault();logical=positionStageRail(logical-delta*scale/stageGeometry().pitch)}event.stopImmediatePropagation()},true);const finish=event=>{if(owner===null||(event.pointerId!==undefined&&event.pointerId!==owner))return;owner=null;if(moved){if(event.cancelable)event.preventDefault();const from=logical,target=Math.max(0,Math.min(levels.length-1,Math.round(from))),started=performance.now();stageIndex=target;syncStageCards();renderBrief();positionStageRail(from);els.stageRail.dataset.wpStageSettling="true";const settle=now=>{const progress=Math.max(0,Math.min(1,(now-started)/340)),eased=progress*progress*(3-2*progress);positionStageRail(from+(target-from)*eased);if(progress<1&&screen==="stage")stageSettleFrame=requestAnimationFrame(settle);else{stageSettleFrame=0;if(screen==="stage"){positionStageRail(target);syncStageCards()}restore()}};stageSettleFrame=requestAnimationFrame(settle);suppressClick=true;suppressTimer=setTimeout(()=>{suppressClick=false;suppressTimer=0},500)}else restore();moved=false;event.stopImmediatePropagation()};document.addEventListener("pointerup",finish,true);document.addEventListener("pointercancel",finish,true);els.stageRail.addEventListener("click",event=>{if(!suppressClick)return;clearTimeout(suppressTimer);suppressTimer=0;suppressClick=false;event.preventDefault();event.stopImmediatePropagation()},true)}
function renderStages(){if(!stageCardPool.length)buildStagePool();else{moveStageWindow(desiredStageWindow(stageIndex));syncStageCards()}installVirtualStageDrag();const done=Object.keys(save.completed).filter(key=>save.completed[key]).length;els.stageSummary.textContent=t("progress",{done});if(screen==="stage")requestAnimationFrame(()=>positionStageRail(stageIndex))}
function selectStageOwner(chosen){if(chosen)selectLogicalStage(Number(chosen.dataset.stageIndex))}
function focusStageOwner(card){if(card)selectLogicalStage(Number(card.dataset.stageIndex),{focus:true})}
function markCenteredStage(){stageMarkFrame=0;const card=nearestStageCard();if(card)selectLogicalStage(Number(card.dataset.stageIndex))}
...
els.localeSelect.addEventListener("change",()=>{locale=CODES.includes(els.localeSelect.value)?els.localeSelect.value:"en";storage.set("weightplay:animal-rootvault-pins:locale",locale);storage.set("weightplay:locale",locale);window.WonderI18n?.setLocale?.(locale);if(!LOCALES[locale]&&window.WeightPlayGameRuntimeLocalizer?.locale!==locale){window.location.reload();return}applyLocale()});
els.start.addEventListener("click",()=>{stageIndex=Math.max(0,save.unlocked-1);setScreen("stage");settleFocus("stage")});els.stageBack.addEventListener("click",()=>{setScreen("main");settleFocus("main")});els.briefTab.addEventListener("click",()=>setStageTab("brief"));els.workshopTab.addEventListener("click",()=>setStageTab("workshop"));els.enterStage.addEventListener("click",()=>{if(stageIndex+1<=save.unlocked)startBattle(stageIndex)});els.stageRail.addEventListener("focusin",event=>{const card=event.target.closest?.(".stage-card");if(card)selectStageOwner(card)});els.stageRail.addEventListener("click",event=>{const card=event.target.closest?.(".stage-card");if(!card)return;const index=Number(card.dataset.stageIndex);selectLogicalStage(index);if(index+1<=save.unlocked)startBattle(index)});els.stageRail.addEventListener("keydown",event=>{const card=event.target.closest?.(".stage-card");if(!card)return;const current=Number(card.dataset.stageIndex),rtl=document.documentElement.dir==="rtl";if((event.key==="Enter"||event.key===" ")&&current+1>save.unlocked){event.preventDefault();event.stopImmediatePropagation();return}let next=null;if(event.key==="Home")next=0;else if(event.key==="End")next=levels.length-1;else if(event.key==="ArrowRight")next=current+(rtl?-1:1);else if(event.key==="ArrowLeft")next=current+(rtl?1:-1);if(next===null)return;event.preventDefault();event.stopImmediatePropagation();selectLogicalStage(Math.max(0,Math.min(levels.length-1,next)),{focus:true})},true);els.battleBack.addEventListener("click",openLeave);els.pullPin.addEventListener("click",()=>pullSelected("pointer"));els.restart.addEventListener("click",function (...args) { return __wpReplayStart(() => restartRun.apply(this, args)); });els.battleHelp.addEventListener("click",openHelp);els.helpClose.addEventListener("click",closeHelp);els.leaveContinue.addEventListener("click",continueRun);els.leaveStage.addEventListener("click",()=>{clearPullCompletion();if(boardLayoutFrame)cancelAnimationFrame(boardLayoutFrame);boardLayoutFrame=0;setBattleAnimationsPaused(false);(__wpNotifyMeasurement(), modalOpen=false);(__wpNotifyMeasurement(), els.leaveModal.hidden=true);run=null;busy=false;setScreen("stage");settleFocus("stage")});els.retry.addEventListener("click",()=>commitResultDecision(restartRun));els.resultStages.addEventListener("click",()=>commitResultDecision(()=>{run=null;setScreen("stage");settleFocus("stage")}));els.next.addEventListener("click",()=>commitResultDecision(()=>{track("next_chamber");startBattle(Math.min(29,stageIndex+1),{skipTutorial:true})}));
window.addEventListener("keydown",event=>{if(screen!=="battle"||!run)return;if(event.code==="Escape"){if(!els.helpModal.hidden)closeHelp();else if(!els.leaveModal.hidden)continueRun();else if(els.resultModal.hidden)openLeave()}if((event.code==="Enter"||event.code==="Space")&&!event.repeat&&document.activeElement===els.board&&selectedPin){event.preventDefault();pullSelected("keyboard")}});
