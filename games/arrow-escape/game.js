(() => {
  ...
  function centerStage(number){ensureStageWindow(number);$("stageRail").querySelector(`[data-stage="${number}"]`)?.scrollIntoView({behavior:"auto",inline:"center",block:"nearest"});setCenteredStage(number);}
  ...
  function renderStageRail(){if(!$("stageRail"))return;$("stageSummary").textContent=`${unlocked} / 30`;stageBrowseLogical=selectedStage;if(!stageCardPool.length||stageCardPool.some(card=>!card.isConnected))buildStagePool();ensureStageWindow(selectedStage);centerStage(selectedStage);}
