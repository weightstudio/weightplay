(() => {
  ...
  let selectedStage=unlocked,currentStage=unlocked,state=null,moves=0,busy=false,lastFocus=null,battleGeneration=0,pendingFeedback="",activeEscapes=new Set();
  ...
  function startStage(number){battleGeneration++;currentStage=number;selectedStage=number;state=cloneStage(STAGES[number-1]);moves=0;busy=false;pendingFeedback="";activeEscapes.clear();$("feedback").textContent="";hideModal("resultModal");hideModal("leaveModal");setScreen("battle");renderBattle();requestAnimationFrame(()=>$("battleBack").focus());}
  ...
  function activateBlock(id){if(!state)return;const block=state.blocks.find(item=>item.id===id);if(!block)return;const element=$("board").querySelector(`[data-block="${CSS.escape(id)}"]`);if(block.rotator){block.d=TURN[block.d];delete block.rotator;moves++;$("feedback").textContent=t("rotated");if(activeEscapes.size){element?.classList.remove("rotator");element?.setAttribute("aria-label",GLYPH[block.d]);const glyph=element?.querySelector(".arrow-glyph");if(glyph)glyph.textContent=GLYPH[block.d];$("movesValue").textContent=moves;}else renderBattle();return;}if(block.lock){$("feedback").textContent=t("locked");element?.classList.add("shake");return;}if(block.frozen){$("feedback").textContent=t("frozen");element?.classList.add("shake");return;}const hit=blocker(state,block);if(hit){$("feedback").textContent=t("blocked");markBlocker(hit);element?.classList.add("shake");return;}moves++;const beforeFrozen=new Set(state.blocks.filter(item=>item.frozen).map(item=>item.id)),beforeLocked=new Set(state.blocks.filter(item=>item.lock).map(item=>item.id)),generation=battleGeneration,direction=block.d;element?.classList.add(`escape-${{U:"up",R:"right",D:"down",L:"left"}[direction]}`);state=applyAction(state,{type:"remove",id});activeEscapes.add(id);busy=true;$("movesValue").textContent=moves;$("leftValue").textContent=state.blocks.length;if(block.key&&[...beforeLocked].some(lockId=>!state.blocks.find(item=>item.id===lockId)?.lock))pendingFeedback=t("keyFound");else if([...beforeFrozen].some(frozenId=>!state.blocks.find(item=>item.id===frozenId)?.frozen))pendingFeedback=t("thawed");setTimeout(()=>settleEscapes(generation,id),330);}
  ...
  $("stageRail").addEventListener("click",event=>{const card=event.target.closest(".stage-card:not(:disabled)");if(!card)return;const stageNumber=Number(card.dataset.stage);if(stageNumber>=1&&stageNumber<=unlocked)startStage(stageNumber);});
  ...
  $("board").addEventListener("click",event=>{const block=event.target.closest("[data-block]");if(block)activateBlock(block.dataset.block);});
