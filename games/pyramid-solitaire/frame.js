/* Pyramid content bindings only. Shared frame owns controls and preferences. */
(() => {
  'use strict';
  let installed = false;
  window.installPyramidFrame = view => {
    ...
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', () => window.installPyramidFrame(view), {once:true});
      return;
    }
    ...
    if (!root || !main || !battle || !guide || !window.WeightPlayScreenFrame) {
      throw new Error('Pyramid shared-frame content contract is incomplete');
    }
    ...
    content.className = 'pyramid-main-content';
    const wrapper = content.querySelector('.poster-frame');
    const image = wrapper.querySelector('img');
    image.className = '';
    image.dataset.wpFramePoster = '';
    image.src = '/assets/interface7-redrawn/pyramid-solitaire.webp';
    ...
    const sync = () => {
      ...
      const strings = window.PYRAMID_GUIDE_LOCALES[code] || window.PYRAMID_GUIDE_LOCALES.en;
