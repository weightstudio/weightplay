(() => {
  ...
  const CHAPTERS = [
    ["Meadow Qualifiers","草原資格賽"],["Riverbank Rounds","河岸巡迴賽"],["Canyon Knockouts","峽谷淘汰賽"],
    ["Moonlight Semis","月光準決賽"],["Aurora Finals","極光決賽"],["Crown Series","王冠系列賽"],
  ];
  const CHAPTER_RULES = [
    ["Learn the six zones with a long, exact save cue.","用清楚而充裕的提示學會六個球門區。"],
    ["The golden power window moves faster.","金色力道區移動得更快。"],
    ["The keeper starts reading repeated corners.","守門員開始觀察重複射向的角落。"],
    ["A dim feint may appear before the bright real cue.","明亮真提示前可能先出現暗色假動作。"],
    ["Save windows shorten, but every real cue stays fair.","撲救時間縮短，但每個真提示都保持公平。"],
    ["Combine power, variety, feint reading, and quick saves.","綜合運用力道、變化、辨識假動作與快速撲救。"],
  ];
  const OPPONENTS = [
  ...
  function localizedPair(pairs,index){if(lang==="zh-Hant"||lang==="zh-Hans")return pairs[index][1];return pairs[index][0]}
  ...
  function beep(frequency=440,duration=.07,type="sine"){if(!soundEnabled)return;try{const Audio=window.AudioContext||window.webkitAudioContext;if(!Audio)return;audioContext ||= new Audio();if(audioContext.state==="suspended")audioContext.resume();const oscillator=audioContext.createOscillator(),gain=audioContext.createGain();oscillator.type=type;oscillator.frequency.value=frequency;gain.gain.setValueAtTime(.045,audioContext.currentTime);gain.gain.exponentialRampToValueAtTime(.001,audioContext.currentTime+duration);oscillator.connect(gain).connect(audioContext.destination);oscillator.start();oscillator.stop(audioContext.currentTime+duration)}catch{}}
  ...
  function updateSoundButtons(){const label=t(soundEnabled?"soundOn":"soundOff");["mainSoundBtn","stageSoundBtn","pauseSoundBtn"].forEach(id=>{const node=$(id);if(!node)return;node.textContent=label;node.setAttribute("aria-pressed",String(soundEnabled))})}
  ...
  function renderStage(){
    ...
    $("chapterKicker").textContent=t("chapter",{n:level.chapter+1});$("chapterTitle").textContent=localizedPair(CHAPTERS,level.chapter);$("chapterRule").textContent=localizedPair(CHAPTER_RULES,level.chapter);rail.innerHTML="";rail.dataset.wpStageRecommendation="explicit";
    LEVELS.forEach((item,index)=>{const locked=index>=save.unlocked,selected=index===selectedStage,card=document.createElement("button");card.type="button";card.className=`stage-card${locked?" locked":""}${selected?" selected is-centered":""}`;card.dataset.stage=String(index);card.dataset.stageIndex=String(index);card.dataset.wpStageRecommended=selected&&!locked?"true":"false";card.setAttribute("aria-disabled",String(locked));card.setAttribute("aria-posinset",String(index+1));card.setAttribute("aria-setsize","30");if(selected)card.setAttribute("aria-current","true");const state=locked?t("locked"):save.completed[index]?t("cleared"):t("opponent",{name:item.opponent});const stars=locked?"":"★".repeat(save.stars[index])+"☆".repeat(3-save.stars[index]);card.innerHTML=`<small>${t("chapter",{n:item.chapter+1})}</small><strong>${t("match",{n:index+1})}</strong><span>${state}</span><span>${item.opponent}</span><b class="stars">${stars}</b>`;card.addEventListener("click",event=>{noteInput(event);if(rail.dataset.dragging==="true"||locked)return;selectedStage=index;startMatch(index,"stage")});card.addEventListener("keydown",rejectRepeat);rail.append(card)});
  ...
  function renderBattleShell(){const level=match.level;$("battleRound").textContent=`${t("match",{n:match.stage+1})} · ${t("round",{n:match.round+1})}`;$("battleOpponent").textContent=level.opponent;$("playerScore").textContent=match.player;$("rivalScore").textContent=match.rival;$("turnLabel").textContent=t(match.phase.startsWith("defend")||match.phase==="cue-wait"?"defend":"shoot");renderDots();renderGoalZones();updatePowerUI()}
  ...
  function renderGoalZones(){const grid=$("goalGrid");grid.innerHTML="";ZONES.forEach((zone,index)=>{const button=document.createElement("button");button.type="button";button.className="goal-zone";button.dataset.zone=String(index);button.dataset.number=String(index+1);button.setAttribute("aria-label",zoneLabel(index));button.disabled=!canChooseZone();button.addEventListener("pointerdown",event=>beginZone(index,event));button.addEventListener("pointerup",event=>releaseZone(index,event));button.addEventListener("pointercancel",cancelHold);button.addEventListener("click",event=>chooseDefence(index,event));button.addEventListener("keydown",event=>zoneKeyDown(index,event));button.addEventListener("keyup",event=>zoneKeyUp(index,event));grid.append(button)})}
  ...
  function showCue(zone,fake){const point=ZONES[zone],cue=$("cue");cue.style.left=`${point.x}%`;cue.style.top=`${point.y}%`;cue.className=`cue ${fake?"fake":"real"}`;const button=$("goalGrid").querySelector(`[data-zone="${zone}"]`);button?.classList.add(fake?"cue-fake":"cue-real")}
  function clearCue(){$("cue").className="cue";$("goalGrid").querySelectorAll(".goal-zone").forEach(button=>button.classList.remove("cue-fake","cue-real"))}
  ...
  function setStatus(key){$("battleStatus").textContent=t(key)}
  ...
  $("startBtn").addEventListener("click",()=>{showStage(true);track("stage_open");if(!save.tutorialSeen)openTutorial(true)});$("stageBackBtn").addEventListener("click",()=>showMain(true));$("battleBackBtn").addEventListener("click",requestLeave);$("restartBtn").addEventListener("click",requestRestart);$("helpBtn").addEventListener("click",()=>openTutorial(false));$("pauseBtn").addEventListener("click",()=>openModal("pausePanel","resumeBtn"));$("resumeBtn").addEventListener("click",()=>closeModal("pausePanel",true));
  $("mainSettingsBtn").addEventListener("click",()=>toggleSettings("main",$("mainSettings").hidden));$("mainSettingsClose").addEventListener("click",()=>toggleSettings("main",false));$("stageSettingsBtn").addEventListener("click",()=>toggleSettings("stage",$("stageSettings").hidden));$("stageSettingsClose").addEventListener("click",()=>toggleSettings("stage",false));
  ["mainSoundBtn","stageSoundBtn","pauseSoundBtn"].forEach(id=>$(id).addEventListener("click",()=>setSound(!soundEnabled)));$("leaveContinueBtn").addEventListener("click",()=>closeModal("leavePanel",true));$("leaveStagesBtn").addEventListener("click",()=>{closeModal("leavePanel",false);showStage(true,true)});$("restartContinueBtn").addEventListener("click",()=>closeModal("restartPanel",true));$("restartConfirmBtn").addEventListener("click",()=>{closeModal("restartPanel",false);startMatch(match.stage,"restart")});
