(() => {
  ...
  function renderBattleShell(){$("battleRound").textContent=`${t("match",{n:match.stage+1})} · ${t("round",{n:match.round+1})}`;$("battleOpponent").textContent=opponentName(match.stage);$("playerScore").textContent=match.player;$("rivalScore").textContent=match.rival;$("turnLabel").textContent=t(match.phase.startsWith("defend")||match.phase==="cue-wait"?"defend":"shoot");renderDots();renderGoalZones();updatePowerUI()}
