...
try {
  ...
  if (process.env.WORDLE_SMOKE_SCREENSHOTS === "1") await page.screenshot({ path: path.join(process.env.TEMP || process.env.TMP || ".", "wordle-v3-844x390-battle.png"), fullPage: false });
  ...
  if (process.env.WORDLE_SMOKE_SCREENSHOTS === "1") await page.screenshot({ path: path.join(process.env.TEMP || process.env.TMP || ".", "wordle-v3-844x390-result.png"), fullPage: false });
  assert(errors.length === 0, "Wordle short-landscape smoke emitted browser errors", errors);
  console.log(JSON.stringify({ generatedAt: new Date().toISOString(), viewport: "844x390", route: "Main > Battle > valid guess > natural success Result", battle, result, errors }, null, 2));
  await context.close();
} finally {
  await browser.close();
