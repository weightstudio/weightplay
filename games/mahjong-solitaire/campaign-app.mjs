...
export function mountMahjongCampaign({main,guide,startButton,locale,storage,mainSettings,stageSettings,onMain,sound,onEvent}) {
 ...
 const properties={html:['height','overflow','overflow-y','overscroll-behavior','overscroll-behavior-y','touch-action'],body:['position','inset','width','overflow','overscroll-behavior','touch-action']};
 ...
 function lock(next){
  ...
  if(next){root.style.height='100%';root.style.overflow='hidden';root.style.overscrollBehavior='none';body.style.position='fixed';body.style.inset='0';body.style.width='100%';body.style.overflow='hidden';body.style.overscrollBehavior='none';resetScroll();frame=win.requestAnimationFrame(()=>{frame=0;if(active&&!disposed)resetScroll()});}
