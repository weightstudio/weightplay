...
export function createCampaignBattleView({session,locale,onStages,sound}) {
 ...
 const lifetime=new AbortController();let boardView,disposed=false;
 const leave=()=>{dialog.close();live.inert=false;boardView?.clearEffects();session.stages();root.hidden=true;onStages();};
 const result=createCampaignResultView({locale,stages:leave,next:()=>{session.next();render()},replay:()=>{session.replay();render()}});logical.append(result.root);
 function fit(){
  ...
  canvas.dataset.composition=wide?'wide':'portrait';canvas.style.height=height+'px';
 ...
 function render(){
  ...
  const s=session.snapshot(),c=campaignCopy(locale());root.hidden=s.screen!=='battle';if(root.hidden){boardView?.clearEffects();return;}
  ...
  const feedback=root.querySelector('.mjc-battle-feedback');feedback.textContent=c[s.message]||c.rule;
