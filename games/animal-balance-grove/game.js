(function () {
  ...
  const state = { locale: "en", stage: 0, selected: [], checks: 0, sessionChecks: 0, sound: true, storage: true };
  ...
  const applyLocale = () => {
    document.documentElement.lang = state.locale === "zh-Hant" ? "zh-Hant" : "en";
  ...
  const setLocale = (locale) => { state.locale = copy[locale] ? locale : "en"; try { localStorage.setItem("weightplayLocale", state.locale); } catch (error) { state.storage = false; } applyLocale(); track("locale", { locale: state.locale }); };
