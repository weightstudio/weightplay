(function () {
  ...
  const state = { locale: normalizeLocale(routedLocale || window.WonderI18n?.actualLocale?.() || document.documentElement.lang || savedLocale), stage: 0, selected: [], checks: 0, sessionChecks: 0, sound: !Boolean(window.WonderSound?.isMuted?.()), storage: true };
  const syncSoundState = () => {
    if (typeof window.WonderSound?.isMuted === "function") state.sound = !window.WonderSound.isMuted();
  };
  ...
  const beep = (frequency) => { syncSoundState(); if (!state.sound) return; try { const AudioContext = window.AudioContext || window.webkitAudioContext; if (!AudioContext) return; const context = new AudioContext(); const oscillator = context.createOscillator(); const gain = context.createGain(); oscillator.frequency.value = frequency; gain.gain.setValueAtTime(.035, context.currentTime); gain.gain.exponentialRampToValueAtTime(.001, context.currentTime + .12); oscillator.connect(gain).connect(context.destination); oscillator.start(); oscillator.stop(context.currentTime + .12); oscillator.addEventListener("ended", () => context.close()); } catch (error) { /* audio is optional */ } };
  ...
  window.addEventListener?.("wonder:audio-volume-change", () => { syncSoundState(); applyLocale(); });
