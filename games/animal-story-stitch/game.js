(function () {
  ...
  const queryLocale = new URLSearchParams(window.location.search).get("locale") || new URLSearchParams(window.location.search).get("lang");
  const pathLocale = window.WonderI18n?.localeFromPath?.();
  let locale = normalizeLocale(queryLocale || pathLocale || localStorage.getItem("weightplay-story-stitch-locale") || "en");
  ...
  function applyLocale() {
    ...
    $("settingsBtn").setAttribute("aria-label", copy("settings")); $("stageSettingsBtn").setAttribute("aria-label", copy("settings")); $("battleSettingsBtn").setAttribute("aria-label", copy("settings")); $("backBtn").setAttribute("aria-label", copy("back")); $("stageBackBtn").setAttribute("aria-label", copy("back")); $("battleBackBtn").setAttribute("aria-label", copy("back")); $("closeSettings").setAttribute("aria-label", copy("close")); $("localeSelect").setAttribute("aria-label", copy("language")); $("settingsPanel").setAttribute("aria-label", copy("settings")); $("stageList").setAttribute("aria-label", copy("chapter"));
  ...
  function show(screen) { document.querySelectorAll("main[data-screen], section[data-screen]").forEach((node) => { node.hidden = node.dataset.screen !== screen; }); document.body.dataset.screen = screen; $("settingsPanel").hidden = true; $("settingsBtn").setAttribute("aria-expanded", "false"); $("backBtn").hidden = screen !== "main"; }
  function renderStages() { const root = $("stageList"); if (!root) return; root.replaceChildren(); tales.forEach((tale, index) => { const button = document.createElement("button"); button.className = "stage-card"; button.type = "button"; button.dataset.index = index; button.setAttribute("role", "tab"); button.setAttribute("data-wp-stage-card", ""); button.setAttribute("data-wp-enter-battle", ""); button.innerHTML = `<span><strong>${copy(`tale${index + 1}`)}</strong><small>${copy(`hint${index + 1}`)}</small></span><span class="arrow">${solved.has(index) ? "✓" : "→"}</span>`; button.addEventListener("click", () => startTale(index)); root.appendChild(button); }); }
