(() => {
 ...
 if(sheet)sheet.href=new URL('./style.css?v=3',base).href;
 const version=document.querySelector('meta[name="weightplay-game-version"]');if(version)version.content='3';
 const timer=setTimeout(fail,15000);
 window.addEventListener('pagehide',event=>{if(!event.persisted){abandoned=true;clearTimeout(timer);}},{once:true});
 import(new URL('./game.js?v=3',base).href).then(()=>{clearTimeout(timer);if(abandoned)return;document.getElementById('bootError').hidden=document.documentElement.dataset.pawflowReady==='true';}).catch(error=>{clearTimeout(timer);console.warn('Pawflow startup:',error?.message);fail();});
})();
