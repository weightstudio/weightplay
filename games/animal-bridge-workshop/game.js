(function () {
  ...
  const state = { locale: COPY[requestedLocale] ? requestedLocale : "en", stage: 0, links: [], selectedAnchor: null, selectedMaterial: "beam", sound: true, medals: loadMedals(), result: null, feedbackKey: "", feedbackValues: {} };
  ...
  function track(name, payload) { window.WonderAnalytics?.track?.(name, { game_id: "animal-bridge-workshop", game_version: "v6", interface_version: 6, ...payload }); }
  ...
  function applyCopy() {
    ...
    $("[data-legend='anchor']").textContent = t("legendAnchor"); $("[data-legend='link']").textContent = t("legendLink"); $("[data-legend='animal']").textContent = t("legendAnimal");
    if (state.feedbackKey) $("#feedback").textContent = t(state.feedbackKey, state.feedbackValues);
  ...
  function placeLink() {
    if (state.selectedAnchor === null || state.previewAnchor === null || state.previewAnchor === undefined) { setFeedback("chooseMaterial"); return; }
    const key = edgeKey(state.selectedAnchor, state.previewAnchor); if (state.links.some((link) => link.key === key)) { setFeedback("placed"); return; }
    const stock = stage().stock[state.selectedMaterial]; const used = state.links.filter((link) => link.material === state.selectedMaterial).length; if (used >= stock) { setFeedback("noStock"); return; }
    state.links.push({ key, a: state.selectedAnchor, b: state.previewAnchor, material: state.selectedMaterial }); state.selectedAnchor = null; state.previewAnchor = null; setFeedback("placed"); track("bridge_link_place", { stage: state.stage + 1, material: state.selectedMaterial }); renderStageText(); draw();
  }
  ...
  function testCrossing() {
    if (!state.links.length) { setFeedback("chooseMaterial"); return; }
  ...
  function start() { state.stage = 0; state.links = []; state.selectedAnchor = null; state.previewAnchor = null; state.result = null; state.feedbackKey = ""; $("#resultCard").hidden = true; $("#nextButton").hidden = false; $("#retryButton").hidden = false; $("#feedback").textContent = ""; setScreen("battle"); renderStageText(); draw(); track("bridge_start", {}); }
  function next() { if (state.stage >= STAGES.length - 1) { const medals = Object.values(state.medals); state.result = { type: "complete", medal: medals.includes("Gold") ? "Gold" : medals.includes("Silver") ? "Silver" : "Bronze" }; renderResult(); $("#nextButton").hidden = true; $("#retryButton").hidden = true; return; } state.stage += 1; state.links = []; state.selectedAnchor = null; state.previewAnchor = null; state.result = null; state.feedbackKey = ""; $("#resultCard").hidden = true; $("#feedback").textContent = ""; renderStageText(); draw(); track("bridge_stage_start", { stage: state.stage + 1 }); }
  function retry() { state.links = []; state.selectedAnchor = null; state.previewAnchor = null; state.result = null; state.feedbackKey = ""; $("#resultCard").hidden = true; $("#feedback").textContent = ""; renderStageText(); draw(); }
  function undo() { const link = state.links.pop(); if (link) { setFeedback("placed"); renderStageText(); draw(); } }
  function reset() { state.links = []; state.selectedAnchor = null; state.previewAnchor = null; state.feedbackKey = ""; $("#feedback").textContent = ""; renderStageText(); draw(); }
  function draw() {
