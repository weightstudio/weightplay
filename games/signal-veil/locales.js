(function () {
  ...
  const en = {
    ...
    guideWinTitle:"Success, failure, and saves",guideWin:"The chapter succeeds when the first Lizard Commander is defeated and its device is recovered. If Fia loses all health, the investigation resumes from the last safe checkpoint with permanent level, equipment, quests, and defeated major targets preserved. Use Menu and Save for an immediate checkpoint. No account is required. Progress stays on this device in browser storage; clearing site data or changing browsers or devices can remove or separate the save.",
  ...
  const zhHant = {...en,
    ...
    guideWinTitle:"完成、失敗與存檔",guideWin:"擊敗第一名蜥蜴指揮官並取得裝置即可完成章節。菲雅生命歸零時，會從最近的安全檢查點恢復；永久等級、裝備、任務與主要擊破紀錄仍會保留。可在選單中立即存檔。本作不需要帳號，進度只存在此裝置的瀏覽器儲存空間；清除網站資料或改用其他瀏覽器、裝置可能失去或分開存檔。",
