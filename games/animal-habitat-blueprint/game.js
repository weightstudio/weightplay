(function () {
  ...
  function show(screen) { currentScreen = screen; const inBattle = screen === "battle" || screen === "result"; document.querySelectorAll("[data-screen]").forEach((node) => { if (node === document.body || node === document.documentElement) return; if (node.id === "battleScreen") node.hidden = !inBattle; else if (node.id === "resultScreen") node.hidden = screen !== "result"; else node.hidden = node.dataset.screen !== screen; }); const battleHead = document.querySelector("#battleScreen > .battle-head"); const battlePrompt = $("prompt"); const rules = $("rules"); const selection = $("selection"); const grid = $("habitatGrid"); const battleActions = document.querySelector("#battleScreen > .battle-actions"); const status = $("status"); [battleHead, battlePrompt, rules, selection, grid, battleActions, status].forEach((node) => { if (node) node.hidden = screen !== "battle"; }); const resultScreen = $("resultScreen"); if (resultScreen) resultScreen.hidden = screen !== "result"; const battleScreen = $("battleScreen"); if (battleScreen) battleScreen.dataset.battleState = inBattle ? screen : ""; const root = screen === "result" ? battleScreen : document.querySelector(`[data-screen="${screen}"]`); const topbar = $("topbar"); if (root && topbar) root.prepend(topbar); const back = $("backBtn"); back.dataset.wpReturn = screen === "result" ? "battle" : screen; back.hidden = false; back.classList.toggle("main-return", screen === "main"); const logo = back.querySelector("img"); if (screen === "main" && !logo) { const image = document.createElement("img"); image.src = "../../assets/weightplay-logo.png"; image.alt = ""; back.prepend(image); } else if (screen !== "main" && logo) logo.remove(); $("settingsPanel").hidden = true; const guide = $("gameGuide") || document.querySelector("[data-wp-game-guide],.game-page-info,.game-page-info-static"); if (guide) guide.hidden = screen !== "main"; document.body.dataset.screen = screen === "result" ? "battle" : screen; document.body.dataset.battleSubstate = screen === "result" ? "result" : ""; if (screen === "battle") renderBattle(); if (screen === "result") renderResult(); window.dispatchEvent(new CustomEvent("weightplay:shell-sync")); }
  ...
  function renderResult() { if (!$("resultText")) return; const complete = solved.size === plans.length; const hasNext = planIndex < plans.length - 1; $("resultTitle").textContent = complete ? copy("resultTitle") : copy("resultLevel"); $("resultText").textContent = resultCopy(bestValue()); $("resultMapBtn").hidden = false; $("nextBtn").hidden = false; $("nextBtn").textContent = copy("nextStage"); $("nextBtn").disabled = !hasNext; $("nextBtn").setAttribute("aria-disabled", String(!hasNext)); $("replayBtn").hidden = false; $("replayBtn").disabled = false; $("replayBtn").textContent = copy("replay"); if (complete) { const old = Number(localStorage.getItem("weightplay-habitat-blueprint-best-v1") || 0); if (!old || sessionSwaps < old) localStorage.setItem("weightplay-habitat-blueprint-best-v1", String(sessionSwaps)); $("resultText").textContent = resultCopy(Math.min(old || sessionSwaps, sessionSwaps)); } }
