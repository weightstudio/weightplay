...
let locale="en", screen="main", selected=0, levelIndex=0, queues=[], busCounts=[], history=[], moves=0;
...
function applyLocale(){document.documentElement.lang=locale;document.documentElement.dir=locale==="ar"?"rtl":"ltr";root.querySelectorAll("[data-t]").forEach(e=>e.textContent=t(e.dataset.t));root.querySelectorAll("[data-t-aria]").forEach(e=>e.setAttribute("aria-label",t(e.dataset.tAria)));root.querySelectorAll("[data-t-alt]").forEach(e=>e.alt=t(e.dataset.tAlt));$("locale").value=locale;renderStage()}
...
$("start").onclick=()=>{show("stage");renderStage()};root.querySelectorAll("[data-back]").forEach(b=>b.onclick=()=>show(screen==="battle"?"stage":"main"));$("stageGrid").addEventListener("wonder:stage-snap",event=>{const i=Number(event.detail?.index);if(Number.isInteger(i)&&i>=0)selectStage(i)});$("undo").onclick=undo;$("hint").onclick=hint;$("restart").onclick=()=>startLevel(levelIndex);$("retry").onclick=()=>{$("result").close();startLevel(levelIndex)};$("next").onclick=()=>{$("result").close();if(levelIndex<levels.length-1){selected=Math.min(29,levelIndex+1);show("stage");renderStage()}else show("stage")};$("locale").innerHTML=codes.map(c=>`<option value="${c}">${dict[c]?.label||c}</option>`).join("");root.addEventListener("change",e=>{if(e.target.id!=="locale")return;locale=e.target.value;try{window.WonderI18n?.setLocale?.(locale)}catch{}applyLocale()});applyLocale();show("main");
})();
