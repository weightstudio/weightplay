(function () {
  ...
  const state = { locale: normalizeLocale(routeLocale || routedLocale || savedLocale || window.WonderI18n?.actualLocale?.() || document.documentElement.lang), stage: 0, step: 0, placed: [], selected: null, totalPicks: 0, stagePicks: 0, finished: false, sound: !window.WeightPlayAudio.isMuted(), storage: true, status: "" };
  ...
  const checkChoice = () => { const placement = stages[state.stage].placements[state.step]; if (!state.selected) { state.status = t("ready"); renderBattle(); return; } state.totalPicks += 1; state.stagePicks += 1; const option = placement.options.find((item) => item.key === state.selected); if (state.selected !== placement.correct) { state.status = t("wobbly"); state.selected = null; beep("feedback.error"); track("wobbly", { stage: state.stage + 1, step: state.step + 1 }); renderBattle(); return; } state.placed.push({ ...option, role: placement.role }); state.step += 1; state.selected = null; state.status = t("steady"); beep("feedback.success"); track("steady", { stage: state.stage + 1, step: state.step }); if (state.step >= stages[state.stage].placements.length) { state.finished = true; state.status = t("lookoutReady"); } renderBattle(); };
  ...
  $("startBtn").addEventListener("click", openStageMap); $("mapBtn").addEventListener("click", openStageMap); $("stageBackBtn").addEventListener("click", goHome); $("battleBackBtn").addEventListener("click", openStageMap); $("battleMapBtn").addEventListener("click", openStageMap); $("checkBtn").addEventListener("click", checkChoice); $("clearBtn").addEventListener("click", clearChoice); $("advanceBtn").addEventListener("click", advance); $("resultPrimaryBtn").addEventListener("click", () => startStage(0)); $("resultMapBtn").addEventListener("click", openStageMap); $("resultHomeBtn").addEventListener("click", goHome); $("settingsBtn").addEventListener("click", toggleSettings); $("soundBtn").addEventListener("click", () => { state.sound = window.WeightPlayAudio.setEnabled(!state.sound); applyLocale(); track("sound", { enabled: state.sound }); }); $("localeSelect").addEventListener("change", (event) => setLocale(event.target.value)); $("cushionDialect")?.addEventListener("change", (event) => setLocale(event.target.value)); $("stageInfoBtn").addEventListener("click", () => window.alert(t("stageReady"))); $("battleInfoBtn").addEventListener("click", () => window.alert(t("guideTwo")));
  ensureLocaleOptions();
