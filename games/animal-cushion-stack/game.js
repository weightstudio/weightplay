(function () {
  ...
  const renderTower = () => {
    ...
    const tower = $("tower");
    tower.style.setProperty("--tower-layer-count", String(state.placed.length));
    tower.style.setProperty("--tower-block-height", `${Math.max(10, 24 - state.placed.length * 2)}px`);
    const blocks = state.placed.map((block) => `<div class="tower-block" data-tone="${block.tone}" style="width:${Math.max(28, Math.min(88, block.size))}%" aria-label="${t(block.name)}"><span class="block-atlas-crop tower-block-art" style="${spriteStyle(block.key, 62, 34)}" aria-hidden="true"></span><span class="tower-block-role">${t(block.role)}</span></div>`).join("");
    const current = stage.placements[state.step];
    const baseWidth = current ? current.support : 90;
    tower.innerHTML = `${blocks}<div class="tower-ground" style="width:${Math.min(88, baseWidth)}%" aria-hidden="true"></div>`;
  };
  const renderBattle = () => {
    const stage = stages[state.stage]; const placement = stage.placements[state.step]; $("battleScreen").toggleAttribute("data-wp-stage-finished", state.finished); $("roundLabel").textContent = t("round", { n: state.stage + 1, total: stages.length }); $("battleHint").textContent = placement ? `${t(stage.hint)} · ${t("support", { n: placement.support })}` : t("lookoutReady"); $("sessionPicks").textContent = String(state.totalPicks); renderTower();
