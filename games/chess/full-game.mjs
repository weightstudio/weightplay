...
const text=key=>chessLocales[locale][key]??challengeLocales[locale][key]??campaignLocales[locale][key];
const challengeButton=document.createElement('button');challengeButton.id='challenges';$('resume').after(challengeButton);
...
function stop(){epoch++;ai.cancel();busy=false;selected=null;promotion=null;if($('promotion').open)$('promotion').close();view?.dispose();view=null;}
...
function stages(){active=false;stop();$('main').hidden=true;$('battle').hidden=true;$('result').hidden=true;$('stages').hidden=false;renderStages();const target=Math.min(challenges.length,(Math.max(0,...progress.cleared)+1));$('stageList').querySelector(`[data-stage="${target}"]`)?.scrollIntoView({block:'nearest',inline:'center'});}
function settleChallenge(move){const passed=challengeOutcome(currentChallenge,session.game,move);if(passed===null)return false;if(passed){try{progress=recordChallengeClear(localStorage,progress,currentChallenge.id);}catch{progress={cleared:[...new Set([...progress.cleared,currentChallenge.id])]};}}active=false;stop();$('battle').hidden=true;$('result').hidden=false;outcomeKey=passed?'cleared':'tryAgain';$('outcome').textContent=text(outcomeKey);$('again').textContent=text('retry');nextButton.hidden=!passed||!challenges.some(c=>c.id===currentChallenge.id+1);return true;}
function main(){currentChallenge=null;$("stages").hidden=true;nextButton.hidden=true;active=false;stop();for(const d of document.querySelectorAll('dialog[open]'))d.close();$('battle').hidden=true;$('result').hidden=true;$('main').hidden=false;$('resume').hidden=!saved();}
...
document.addEventListener('visibilitychange',()=>{if(document.hidden){epoch++;ai.cancel();busy=false;}else if(active){render();reply();}});
