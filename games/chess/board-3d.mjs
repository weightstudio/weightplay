...
export class ChessBoard3D {
 constructor(host,{onSquare=()=>{},onUnavailable=()=>{}}={}){
  ...
  this.scene.add(new THREE.HemisphereLight(0xfff4db,0x124453,1.6));const light=new THREE.DirectionalLight(0xffefd5,2.6);light.position.set(-4,10,5);light.castShadow=true;light.shadow.bias=-.0002;light.shadow.normalBias=.025;light.shadow.mapSize.set(1024,1024);light.shadow.camera.left=-6;light.shadow.camera.right=6;light.shadow.camera.top=6;light.shadow.camera.bottom=-6;this.scene.add(light);this.light=light;this.createStudio();
  ...
  this.materials.detail=new THREE.MeshStandardMaterial({color:0x152328,roughness:.4});this.addWoodGrain();
 ...
 finishAnimation(){cancelAnimationFrame(this.animation);this.animation=null;for(const motion of this.motions||[])motion.piece.position.copy(motion.target);this.motions=[];this.effects?.clear();}
 ...
 animateMove(move){
  ...
  if(!move||matchMedia('(prefers-reduced-motion: reduce)').matches){this.render();return;}
  const piece=this.pieces.children.find(p=>p.userData.square===move.to);if(!piece)return;
  ...
  const frame=now=>{if(this.disposed)return;const elapsed=Math.max(0,now-start),t=Math.min(1,elapsed/220),e=t*t*(3-2*t);
   ...
   if(elapsed>=duration)this.finishAnimation();else this.animation=requestAnimationFrame(frame);this.render();
