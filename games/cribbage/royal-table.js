...
(function () {
  ...
  function decorate(root) {
    if(root.id==='cardGameCenter'){
      const cues=[...root.children].filter(node=>node.matches('.card-crib-transition,.card-crib-score-cue'));
      if(cues.length){
        const panel=document.createElement('div');panel.className='crib-coach-panel';panel.tabIndex=0;
        root.insertBefore(panel,cues[0]);panel.append(...cues);
      }
    }
    const signature=[...root.querySelectorAll('.playing-card')].map(card=>card.getAttribute('aria-label')).join('|');
    const changed=signatures.has(root)&&signatures.get(root)!==signature;
    ...
    root.querySelectorAll('.playing-card:not([data-deck-rank]):not(.is-face-down)').forEach(card=>{
      ...
      if(changed&&root.id==='cardGameCenter')card.classList.add('cribbage-card-arrival');
