...
(() => {
  ...
  function cost(b,type){return has(b,'supply3')&&(b.recruitIndex+1)%4===0?0:troop(b,type).cost;}
      ...
      if(a.windup){
        if(--a.windup.ticks<=0){const hit=a.windup;a.windup=null;
          if(hit.target==='base'&&Math.abs((a.enemy?4:96)-a.x)<=a.reach+1){
            ...
            else {b.commandHp=Math.max(0,b.commandHp-a.damage);effect(b,'hit',96,'−'+a.damage);if(a.type==='cannon'){effect(b,'blast',96,'',a.x,{strong:true});b.events.push('rocket');}}
          } else {
            ...
            if(target&&Math.abs(target.x-a.x)<=a.reach+5){
              ...
              const amount=a.damage*(!a.enemy&&a.type==='spear'&&cavalry?1.8:1)*(!a.enemy&&a.type==='horse'&&!a.charged?1.6:1)*(a.trait==='ambush'&&!a.charged?2:1);
  ...
  window.ZhaoPush={troops,troop,create,cost,deploy,charge,step};
})();
