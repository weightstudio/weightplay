(() => {
  ...
  function drawEnemy(enemy) {
    ...
    ctx.shadowBlur = enemy.hitTimer > 0 ? 38 : 26;
    ctx.fillStyle = enemy.kind === "boss" ? "rgba(40, 5, 34, 0.88)" : "rgba(1, 6, 22, 0.86)";
    ctx.beginPath();
    ctx.ellipse(enemy.x, enemy.y + size * 0.1, size * 0.7, size * 0.54, 0, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.shadowBlur = 0;
    ctx.strokeStyle = enemy.hitTimer > 0 ? "#fff7a8" : "rgba(244, 255, 236, 0.92)";
    ctx.lineWidth = enemy.kind === "boss" ? 4 : 2.5;
    ctx.beginPath();
