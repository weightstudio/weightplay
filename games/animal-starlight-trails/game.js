(() => {
  ...
  dom.start.addEventListener("click",()=>{sound("click");showStage();});dom.stageBack.addEventListener("click",()=>{sound("click");showMain();});dom.battleBack.addEventListener("click",()=>{sound("click");openBattleLeave();});dom.continueBattle.addEventListener("click",()=>{sound("click");closeBattleLeave();});dom.leaveBattle.addEventListener("click",leaveBattle);dom.undo.addEventListener("click",undo);dom.restart.addEventListener("click",()=>restartAttempt(true));dom.hint.addEventListener("click",showHint);dom.retry.addEventListener("click",()=>commitResultAction(()=>startStage(stageIndex)));dom.resultStages.addEventListener("click",()=>commitResultAction(()=>showStage(stageIndex)));dom.next.addEventListener("click",()=>commitResultAction(()=>startStage(Math.min(29,stageIndex+1))));dom.locale.addEventListener("change",()=>{locale=dom.locale.value;applyLocale();});
