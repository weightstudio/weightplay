(() => {
  ...
  function leaveBattle(){(__wpNotifyMeasurement(), dom.leave.hidden=true);setBattleLeaveCoverage(false);pauseRunClock();sound("click");analytics("return_stage",{from_screen:"battle",input_type:"leave"});showStage(stageIndex,"leave");}
  ...
  function reject(reason,vars={}){mistakes+=1;sound("wrong");analytics("invalid_route",{stage:currentStage().id,reason,path_length:path.length});setFeedback(t(reason,vars),"error");dom.scene.classList.remove("shake");void dom.scene.offsetWidth;dom.scene.classList.add("shake");updateAssist();}
  ...
  function chooseNode(node){
    ...
    const hadKey=state.keyFound;path.push(node);analytics("node_input",{stage:stage.id,node,path_length:path.length});sound("click");updateGraph();const nextState=deriveState();
  ...
  function undo(){if(completing||path.length===0)return;path.pop();hintNode=null;analytics("undo",{stage:currentStage().id,remaining_path:path.length});sound("click");setFeedback(path.length?t("continue"):t("ready"));updateGraph();}
  function restartAttempt(count=true){
    const __wpPreviousRound = __wpMeasurement.roundKey;
if(completing)return;if(count&&path.length)restarts+=1;path=[];hintNode=null;sound("click");setFeedback(t("ready"));updateGraph();if(count)analytics("game_restart",{stage:stageIndex+1});
