(() => {
  ...
  function openBattleLeave(){if(document.body.dataset.screen!=="battle"||!dom.result.hidden||completing)return;cancelActivePointer();pauseRunClock();renderBattleLeave();setBattleLeaveCoverage(true);dom.leave.hidden=false;dom.continueBattle.focus({preventScroll:true});}
  ...
  dom.leave.addEventListener("keydown",event=>{if(event.key==="Enter"||event.key===" "){if(event.repeat){event.preventDefault();return;}heldLeaveDecisionKeys.add(event.key);}if(event.key==="Escape"){event.preventDefault();closeBattleLeave();return;}if(event.key!=="Tab")return;const first=dom.continueBattle,last=dom.leaveBattle;if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus();}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus();}});
