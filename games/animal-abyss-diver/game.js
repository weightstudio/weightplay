(() => {
  ...
  function renderBattle(){ $("battleTitle").textContent=t("route",{n:state.route});$("zoneText").textContent=t("zone",{n:state.zone});$("oxygenText").textContent=`${t("oxygen")} ${state.oxygen}%`;$("objectiveText").textContent=state.sonar?t("sonarLane"):t("chooseLane");$("salvageText").textContent=t("salvage",{n:state.salvage});$("diverArt").style.left=state.zone===1?"8%":state.zone===2?"36%":"62%";$("hazardArt").style.opacity=state.sonar?".25":".8";$("relicArt").style.opacity=state.sonar?"1":".76";$("fxArt").classList.toggle("hidden",!state.sonar);$("diamondText").querySelector("b").textContent=wallet();$("beaconBtn").textContent=state.beaconUsed?t("beaconUsed"):t("beacon");$("beaconBtn").disabled=state.beaconUsed;}
  function start(route){state={route,zone:1,oxygen:100,salvage:0,sonar:false,beaconUsed:false};show("battleShell");renderBattle();}
