(() => {
  ...
  function newRun(region=selectedRegion){selectedRegion=Math.min(Number(progress.unlockedRegion)||0,Math.max(0,Number(region)||0));const enemy=enemyAt(0,selectedRegion);run={region:selectedRegion,room:0,armor:36,maxArmor:36,gold:8,placed:[],tray:["forge-hammer","leaf-buckler","berry-potion","crystal-lens"],enemyHp:enemy.hp,enemyMaxHp:enemy.hp};selectedItem=run.tray[0];rotated=false;battleReady=false;resolving=false;setFeedback(t("battleSteps"));saveRun();setTimeout(()=>sound("start"),0);track("game_start",{region:selectedRegion+1});showScreen("battle");}
  ...
  function placeSelected(x,y){if(!selectedItem)return;const item=itemById(selectedItem);if(!canPlace(item,x,y)){sound("wrong");setFeedback(t("blocked"));return;}run.placed.push({id:item.id,x,y,rotated});run.tray.splice(run.tray.indexOf(item.id),1);progress.discoveries=[...new Set([...progress.discoveries,item.id])];selectedItem=run.tray[0]||null;rotated=false;saveProgress();saveRun();sound("click");setFeedback(`${t("placed")}: ${itemName(item)}`);renderBattle();}
  ...
  function sellSelected(){if(!selectedItem)return;const item=itemById(selectedItem),index=run.tray.indexOf(selectedItem);if(index<0)return;run.tray.splice(index,1);const value=Math.max(1,Math.floor(item.gold/2));run.gold+=value;selectedItem=run.tray[0]||null;rotated=false;saveRun();sound("coin");setFeedback(`${t("sold")}: ${itemName(item)} +${value} ${t("gold")}`);renderBattle();}
  ...
  function fight(){if(resolving)return;if(!run.placed.length){setFeedback(t("needGear"));return;}if(!battleReady){battleReady=true;setFeedback(t("strike"));renderBattle();return;}resolving=true;renderBattle();const enemy=enemyAt(run.room,run.region),s=stats();animateAttack();run.enemyHp-=s.atk;$("#combatFx").textContent=`-${s.atk}`;setTimeout(()=>$("#combatFx").textContent="",420);if(run.enemyHp<=0){run.gold+=4+run.room*2;progress.bestRoom=Math.max(progress.bestRoom,run.room+1);saveProgress();saveRun();setTimeout(roomVictory,420);return;}const incoming=Math.max(1,enemy.damage-s.armor);run.armor=Math.min(run.maxArmor,run.armor+s.heal)-incoming;setFeedback(`${t(enemy.name)} -${incoming} ${t("health")}`);saveRun();setTimeout(()=>{animateCounterattack();renderBattle();$("#playerCombatFx").textContent=`-${incoming}`;},180);setTimeout(()=>$("#playerCombatFx").textContent="",1400);if(run.armor<=0){setTimeout(()=>showResult(false),650);return;}setTimeout(()=>{resolving=false;renderBattle();},560);}
  function roomVictory(){if(run.room===4){showResult(true);return;}showLoot(()=>{run.room+=1;const enemy=enemyAt(run.room,run.region);run.enemyHp=enemy.hp;run.enemyMaxHp=enemy.hp;battleReady=false;resolving=false;saveRun();renderBattle();if(run.room===3)showMerchant();});}
  ...
  function showMerchant(){const render=()=>{const choices=showModal(t("merchant"),`${t("gold")} ${run.gold} · ${t("diamonds")} ${diamondBalance()}`,"../../assets/animal-gearpack-expedition-orla.webp");randomItems(3).forEach((item)=>choices.append(choiceButton(`${t("buy")} ${item.gold} · ${itemName(item)}`,()=>{if(run.gold<item.gold){$("#modalText").textContent=t("notEnough");return;}if(addToTray(item)){run.gold-=item.gold;saveRun();render();}},item)));choices.append(choiceButton(t("refresh"),()=>{if(diamondBalance()<3){$("#modalText").textContent=t("notEnough");return;}if(confirm(t("confirmRefresh"))&&spendDiamonds(3)){sound("upgrade");track("diamond_spend",{sink:"merchant_refresh",amount:3});render();}}));choices.append(choiceButton(t("leave"),()=>{closeModal();renderBattle();}));};render();}
  function showResult(won){const xp=won?35:8+run.room*4;progress.workshopXp+=xp;if(won){progress.unlockedRegion=Math.max(Number(progress.unlockedRegion)||0,Math.min(2,(run.region||0)+1));selectedRegion=Math.min(2,progress.unlockedRegion);}saveProgress();sound(won?"win":"wrong");track("game_end",{result:won?"win":"loss",room:run.room+1,region:(run.region||0)+1});const choices=showModal(won?t("victory"):t("defeat"),`+${xp} XP · ${t("discoveries")} ${progress.discoveries.length}/12`,won?"../../assets/animal-gearpack-expedition-icons/victory-sparkle.webp":"../../assets/animal-gearpack-expedition-rux.webp");choices.append(choiceButton(won?t("regions"):t("retry"),()=>{closeModal();run=null;saveRun();won?showScreen("stage"):newRun();}));}
