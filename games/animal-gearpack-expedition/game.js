...
function showScreen(name,focusTarget=false){if(name!=="stage"&&railSettleTimer){clearTimeout(railSettleTimer);railSettleTimer=0;}Object.entries(screens).forEach(([key,node])=>node.hidden=key!==name);document.body.dataset.screen=name;document.body.classList.toggle("is-game-playing",name==="battle");if(name==="main")renderMain();if(name==="stage")renderStage();if(name==="battle")renderBattle();if(focusTarget)requestAnimationFrame(()=>requestAnimationFrame(()=>{const target=name==="main"?$("#startBtn"):name==="stage"?$(`.region-card[data-stage="${selectedStage}"]`):$(".pack-cell[tabindex='0']");target?.focus({preventScroll:true});}));}
  ...
  function roomVictory(){combatPaused=false;resolving=false;renderBattle();track("encounter_result",{stage:run.stage+1,encounter:run.room+1,outcome:"win",result_reason:"enemy_defeated"});if(run.room===ROOMS_PER_STAGE-1){showResult(true);return;}showLoot(()=>{run.room+=1;prepareEnemyState(enemyAt(run.room,run.stage));resolving=false;saveRun();setFeedback(t("repack"));renderBattle();if(run.room===stages[run.stage].merchantAfter)showMerchant();});}
  ...
  function showLoot(done){const hasRoom=run.tray.length+run.placed.length<12,reward=`${t("reward")}: +${4+run.room*2} ${t("gold")}`;const choices=showModal(t("chooseLoot"),hasRoom?reward:`${reward} · ${t("fullLoot")}`);randomItems(3).forEach((item)=>{const choice=itemChoiceButton(item,()=>{if(!addToTray(item))return;track("loot_choice",{stage:run.stage+1,encounter:run.room+1,choice:"take",item_tag:item.tag});closeModal();done();});choice.disabled=!hasRoom;choices.append(choice);});choices.append(choiceButton(t("leaveLoot"),()=>{track("continue_without_loot",{stage:run.stage+1,encounter:run.room+1,choice:"skip"});closeModal();done();}));focusModalChoice(null,true);}
