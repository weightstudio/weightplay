(() => {
  ...
  function openBattleReturn(){if(!run||!$("#modal").hidden||resolving&&!combatPaused)return;const choices=showModal(t("leaveBattleTitle"),t("leaveBattleText"));const resume=choiceButton(t("continueBattle"),closeModal);const leave=choiceButton(t("confirmLeaveBattle"),()=>{closeModal();if(combatPaused){resetCombatLifecycle();resolving=false;}saveRun();showScreen("stage",true);});choices.append(resume,leave);modalEscapeHandler=closeModal;focusModalChoice(resume);}
  ...
  let railSettleTimer=0;$("#regionRail").addEventListener("scroll",()=>{clearTimeout(railSettleTimer);railSettleTimer=setTimeout(()=>{const rail=$("#regionRail"),center=rail.getBoundingClientRect().left+rail.clientWidth/2,cards=[...rail.querySelectorAll(".region-card")];const nearest=cards.sort((a,b)=>Math.abs((a.getBoundingClientRect().left+a.offsetWidth/2)-center)-Math.abs((b.getBoundingClientRect().left+b.offsetWidth/2)-center))[0];if(!nearest)return;centeredStage=Number(nearest.dataset.stage)||0;cards.forEach((card)=>card.classList.toggle("is-selected",card===nearest));if(nearest.disabled)return;selectedStage=centeredStage;progress.selectedStage=selectedStage;saveProgress();},100);},{passive:true});
