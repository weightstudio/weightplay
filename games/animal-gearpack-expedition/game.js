(() => {
  ...
  function showResult(won){clearTimeout(combatTimer);resolving=false;const xp=won?35:8+run.room*4;progress.workshopXp+=xp;if(won){progress.unlockedRegion=Math.max(Number(progress.unlockedRegion)||0,Math.min(2,(run.region||0)+1));selectedRegion=Math.min(2,progress.unlockedRegion);}saveProgress();sound(won?"win":"wrong");track("game_end",{result:won?"win":"loss",room:run.room+1,region:(run.region||0)+1});const choices=showModal(won?t("victory"):t("defeat"),`+${xp} XP · ${t("discoveries")} ${progress.discoveries.length}/12`,won?"../../assets/animal-gearpack-expedition-icons/victory-sparkle.webp":"../../assets/animal-gearpack-expedition-rux.webp");choices.append(choiceButton(won?t("regions"):t("retry"),()=>{closeModal();run=null;saveRun();won?showScreen("stage"):newRun();}));}
