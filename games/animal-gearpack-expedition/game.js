...
function showScreen(name,focusTarget=false){if(name!=="stage"&&railSettleTimer){clearTimeout(railSettleTimer);railSettleTimer=0;}Object.entries(screens).forEach(([key,node])=>node.hidden=key!==name);document.body.dataset.screen=name;document.body.classList.toggle("is-game-playing",name==="battle");if(name==="main")renderMain();if(name==="stage")renderStage();if(name==="battle")renderBattle();if(focusTarget)requestAnimationFrame(()=>requestAnimationFrame(()=>{const target=name==="main"?$("#startBtn"):name==="stage"?$(`.region-card[data-stage="${selectedStage}"]`):$(".pack-cell[tabindex='0']");target?.focus({preventScroll:true});}));
  ...
  function newRun(stageIndex=selectedStage){resetCombatLifecycle();resultSettled=false;selectedStage=Math.max(0,Math.min((Number(progress.unlockedStage)||1)-1,Number(stageIndex)||0));centeredStage=selectedStage;progress.selectedStage=selectedStage;saveProgress();const workshopLevel=1+Math.floor(progress.workshopXp/40),maxArmor=36+Math.min(18,(workshopLevel-1)*2);run={stage:selectedStage,region:Math.floor(selectedStage/5),room:0,armor:maxArmor,maxArmor,gold:8,placed:[],tray:["forge-hammer","leaf-buckler","berry-potion","crystal-lens"]};prepareEnemyState(enemyAt(0,selectedStage));selectedItem=run.tray[0];selectedTrayIndex=0;rotated=false;resolving=false;setItemFeedback(itemById(selectedItem));saveRun();setTimeout(()=>sound("start"),0);track("game_start",{stage:selectedStage+1,region:run.region+1});showScreen("battle",true);
