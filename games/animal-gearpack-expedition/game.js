(() => {
  ...
  Object.assign(copy.en,{battleSteps:"1 Select gear  2 Tap a green cell  3 Start encounter",attack:"Attack",defense:"Defense",healing:"Heal",links:"Links",attackHint:"Tap Attack to resolve the clash."});
  Object.assign(copy["zh-Hant"],{battleSteps:"① 選下方裝備　② 點綠色格子放入　③ 開始遭遇",attack:"攻擊",defense:"防禦",healing:"恢復",links:"連結",attackHint:"按「攻擊」進行這一回合。"});
  ...
  function renderStage(){$("#stageGold").textContent=`${t("diamonds")} ${diamondBalance()}`;$("#loadoutText").textContent=`${t("discoveries")} ${progress.discoveries.length}/12 · ${t("workshop")} Lv.${1+Math.floor(progress.workshopXp/40)}`;requestAnimationFrame(()=>$(".region-card.is-selected")?.scrollIntoView({behavior:"smooth",inline:"center",block:"nearest"}));}

  function newRun(){run={room:0,armor:36,maxArmor:36,gold:8,placed:[],tray:["forge-hammer","leaf-buckler","berry-potion","crystal-lens"],enemyHp:enemies[0].hp,enemyMaxHp:enemies[0].hp};selectedItem=run.tray[0];rotated=false;battleReady=false;resolving=false;setFeedback(t("battleSteps"));saveRun();setTimeout(()=>sound("start"),0);track("game_start");showScreen("battle");}
  ...
  function itemDetails(item){const shape=shapeFor(item),width=Math.max(...shape.map(([x])=>x))+1,height=Math.max(...shape.map(([,y])=>y))+1;return `${itemName(item)} · ${width}×${height} · ${t("attack")} ${item.atk} · ${t("defense")} ${item.armor} · ${t("healing")} ${item.heal}`;}
  ...
  function showResult(won){const xp=won?35:8+run.room*4;progress.workshopXp+=xp;saveProgress();sound(won?"win":"wrong");track("game_end",{result:won?"win":"loss",room:run.room+1});const choices=showModal(won?t("victory"):t("defeat"),`+${xp} XP · ${t("discoveries")} ${progress.discoveries.length}/12`,won?"../../assets/animal-gearpack-expedition-icons/victory-sparkle.webp":"../../assets/animal-gearpack-expedition-rux.webp");choices.append(choiceButton(won?t("regions"):t("retry"),()=>{closeModal();run=null;saveRun();won?showScreen("stage"):newRun();}));}
