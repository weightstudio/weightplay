(() => {
  ...
  function sellSelected(){if(resolving||!selectedItem)return;const item=itemById(selectedItem),index=selectedTrayPosition();if(index<0)return;run.tray.splice(index,1);const value=Math.max(1,Math.floor(item.gold/2));run.gold+=value;selectFirstTrayItem();rotated=false;saveRun();sound("coin");setFeedback(`${t("sold")}: ${itemName(item)} +${value} ${t("gold")}`);renderBattle();}
  ...
  function resetCombatLifecycle(){clearTimeout(combatTimer);combatTimer=0;combatPendingCallback=null;combatRemaining=0;combatDueAt=0;combatPaused=false;}
  ...
  function showMerchant(){let stock=randomItems(3),refreshPending=false,refreshUsed=false,confirmTimer=0;const clearConfirmation=()=>{clearTimeout(confirmTimer);refreshPending=false;};const render=()=>{const balance=diamondBalance();const message=refreshUsed?t("refreshUsed"):refreshPending?t("refreshDecision",{before:balance,after:balance-3}):`${t("gold")} ${run.gold} · ${t("diamonds")} ${balance}`;const choices=showModal(t("merchant"),message,"../../assets/animal-gearpack-expedition-orla.webp");stock.forEach((item)=>choices.append(choiceButton(`${t("buy")} ${item.gold} · ${itemName(item)}`,()=>{clearConfirmation();if(run.gold<item.gold){$("#modalText").textContent=t("notEnough");return;}if(addToTray(item)){run.gold-=item.gold;saveRun();stock=randomItems(3);render();}},item)));const refresh=choiceButton(refreshUsed?t("refreshUsed"):refreshPending?t("refreshConfirm",{before:balance,after:balance-3}):t("refresh"),()=>{if(refreshUsed)return;const current=diamondBalance();if(current<3){clearConfirmation();$("#modalText").textContent=t("refreshNeed",{balance:current});refresh.classList.remove("is-confirming");return;}if(!refreshPending){refreshPending=true;render();confirmTimer=setTimeout(()=>{refreshPending=false;render();},5000);return;}clearConfirmation();if(spendDiamonds(3)){refreshUsed=true;sound("upgrade");track("diamond_spend",{sink:"merchant_refresh",amount:3});stock=randomItems(3);render();}});refresh.addEventListener("keydown",(event)=>{if(event.repeat&&(event.key==="Enter"||event.key===" "))event.preventDefault();});refresh.dataset.merchantRefresh="";refresh.disabled=refreshUsed;refresh.classList.toggle("is-confirming",refreshPending&&!refreshUsed);refresh.setAttribute("aria-label",refreshUsed?t("refreshUsed"):refreshPending?t("refreshConfirmLabel",{before:balance,after:balance-3}):t("refreshLabel",{balance}));choices.append(refresh);choices.append(choiceButton(t("leave"),()=>{clearConfirmation();closeModal();renderBattle();}));focusModalChoice(refreshPending?refresh:null);};render();}
  ...
  $("#localeSelect").addEventListener("change",(event)=>applyLocale(event.target.value));$("#startBtn").addEventListener("click",()=>showScreen("stage",true));$("#stageBackBtn").addEventListener("click",()=>showScreen("main",true));$("#battleBackBtn").addEventListener("click",()=>{if(resolving)return;saveRun();showScreen("stage",true)});$("#pauseBtn").addEventListener("click",()=>setFeedback(localized(stages[run?.stage??selectedStage].rule)));$("#rotateBtn").addEventListener("click",()=>{if(resolving||!selectedItem)return;rotated=!rotated;setItemFeedback(itemById(selectedItem));renderBattle();});$("#sellBtn").addEventListener("click",sellSelected);$("#fightBtn").addEventListener("click",fight);
