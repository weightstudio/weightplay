(() => {
  ...
  const __wpReadMeasurement = () => ({ ...__wpMeasurement,
    ...
    paused: Boolean(combatPaused || resolving && document.hidden), node: document.body,
  ...
  document.querySelector("#stageScreen")?.setAttribute("data-wp-canvas-max-width", "920");
  ...
  Object.assign(copy.en,{chooseExpedition:"Choose Expedition",stage:"Stage",encounter:"Encounter",checkpoint:"Guardian",completed:"Cleared",unlocked:"unlocked",stageRule:"Route rule",nextExpedition:"Next Stage",allStagesCleared:"All 30 expeditions cleared",stageCleared:"Expedition cleared",retryStage:"Replay",backToStages:"Stages",openingStrike:"Opening strike",shielded:"Shield",reflected:"Reflected shard",regrowth:"Guardian regrew",seal:"Sealed",isolated:"isolated items",corrosion:"corrosion",heat:"top-row heat",overload:"overload",bossPhase:"Boss phase"});
  Object.assign(copy.en,{leaveBattleTitle:"Leave this expedition?",leaveBattleText:"Your current pack and encounter are preserved while you decide.",continueBattle:"Continue expedition",confirmLeaveBattle:"Return to Stages"});
  Object.assign(copy["zh-Hant"],{leaveBattleTitle:"\u96e2\u958b\u9019\u6b21\u9060\u5f81\uff1f",leaveBattleText:"\u6c7a\u5b9a\u671f\u9593\u6703\u4fdd\u7559\u76ee\u524d\u884c\u56ca\u8207\u6230\u9b25\u72c0\u614b\u3002",continueBattle:"\u7e7c\u7e8c\u9060\u5f81",confirmLeaveBattle:"\u8fd4\u56de\u95dc\u5361"});
  ...
  copy.es = {
    title:"Expedición de la Mochila Animal",internal:"Prueba interna",pitch:"Coloca equipo, crea enlaces adyacentes y guía a Rux por la ruta del Bosque Engranaje.",start:"Comenzar juego",chooseRegion:"Elegir región",packmaster:"Rux, maestro de la mochila",
  ...
  const blueprintDefinitions = [
    {id:"forge",nameKey:"blueprintForgeName",requirementKey:"blueprintForgeRequirement",unlockLevel:1,attack:3,armor:0,heal:0,matches:(members)=>members.length>=3&&members.every((placed)=>itemById(placed.id).tag==="forge")},
    {id:"nature",nameKey:"blueprintNatureName",requirementKey:"blueprintNatureRequirement",unlockLevel:2,attack:0,armor:4,heal:1,matches:(members)=>members.length>=3&&members.every((placed)=>itemById(placed.id).tag==="nature")},
    {id:"caravan",nameKey:"blueprintCaravanName",requirementKey:"blueprintCaravanRequirement",unlockLevel:3,attack:2,armor:2,heal:0,matches:(members)=>new Set(members.map((placed)=>itemById(placed.id).tag)).size>=3},
  ];
  ...
  Object.assign(copy.en,{
    blueprints:"Blueprints",blueprintForgeName:"Ember Drive",blueprintForgeRequirement:"Connect 3 Forge items (+3 ATK)",
    blueprintNatureName:"Verdant Shelter",blueprintNatureRequirement:"Connect 3 Nature items (+4 DEF / +1 Heal)",
    blueprintCaravanName:"Caravan Relay",blueprintCaravanRequirement:"Connect 3 different materials (+2 ATK / +2 DEF)",
    blueprintBuildRecap:"Build: {blueprints} · Strongest {material} · Links {links}",blueprintPressure:"Pressure: {rule}",blueprintNone:"No active blueprint"
  });
  Object.assign(copy["zh-Hant"],{
    blueprints:"藍圖",blueprintForgeName:"餘燼驅動",blueprintForgeRequirement:"連接 3 件鍛造裝備（+3 攻擊）",
    blueprintNatureName:"翠綠庇護",blueprintNatureRequirement:"連接 3 件自然裝備（+4 防禦／+1 治療）",
    blueprintCaravanName:"商隊接力",blueprintCaravanRequirement:"連接 3 種不同材料（+2 攻擊／+2 防禦）",
    blueprintBuildRecap:"配置：{blueprints} · 最強材料 {material} · 連結 {links}",blueprintPressure:"壓力：{rule}",blueprintNone:"沒有啟用藍圖"
  });
  Object.assign(copy.es,{
    blueprints:"Planos",blueprintForgeName:"Impulso de brasas",blueprintForgeRequirement:"Conecta 3 objetos de forja (+3 ATQ)",
    blueprintNatureName:"Refugio verde",blueprintNatureRequirement:"Conecta 3 objetos naturales (+4 DEF / +1 curación)",
    blueprintCaravanName:"Relevo de caravana",blueprintCaravanRequirement:"Conecta 3 materiales distintos (+2 ATQ / +2 DEF)",
    blueprintBuildRecap:"Configuración: {blueprints} · Material principal {material} · Enlaces {links}",blueprintPressure:"Presión: {rule}",blueprintNone:"Ningún plano activo"
  });
  Object.assign(copy.ar,{
    blueprints:"المخططات",blueprintForgeName:"دفع الجمر",blueprintForgeRequirement:"صِل 3 قطع حدادة (+3 هجوم)",
    blueprintNatureName:"الملاذ الأخضر",blueprintNatureRequirement:"صِل 3 قطع طبيعة (+4 دفاع / +1 علاج)",
    blueprintCaravanName:"ترابط القافلة",blueprintCaravanRequirement:"صِل 3 مواد مختلفة (+2 هجوم / +2 دفاع)",
    blueprintBuildRecap:"البناء: {blueprints} · أقوى مادة {material} · الروابط {links}",blueprintPressure:"الضغط: {rule}",blueprintNone:"لا يوجد مخطط نشط"
  });
  Object.assign(copy["zh-Hans"],{
    blueprints:"蓝图",blueprintForgeName:"余烬驱动",blueprintForgeRequirement:"连接 3 件锻造装备（+3 攻击）",
    blueprintNatureName:"翠绿庇护",blueprintNatureRequirement:"连接 3 件自然装备（+4 防御／+1 治疗）",
    blueprintCaravanName:"商队接力",blueprintCaravanRequirement:"连接 3 种不同材料（+2 攻击／+2 防御）",
    blueprintBuildRecap:"配置：{blueprints} · 最强材料 {material} · 连接 {links}",blueprintPressure:"压力：{rule}",blueprintNone:"没有启用蓝图"
  });
  Object.assign(copy["zh-Hans"],{lootCurrent:"当前行囊：攻击 {attack} · 防御 {defense} · 恢复 {healing} · 连接 {links}。",lootAdds:"这件装备增加：+{attack} 攻击 · +{defense} 防御 · +{healing} 恢复。",lootSpaceFit:"空间：{width}×{height} 可以放入空位。",lootSpaceNoFit:"空间：当前没有可放置 {width}×{height} 的位置；放置前先重新整理。",lootLinkJoin:"连接展望：如果放在相邻的相同材质旁，{tag} 可以加入连接。",lootLinkNone:"连接展望：当前没有能与 {tag} 装备相邻的合法位置。",lootLinkSealed:"压力：当前封印会抑制 {tag} 连接。"});
  Object.assign(copy.en,{lootCurrent:"Current pack: Attack {attack} · Defense {defense} · Heal {healing} · Links {links}.",lootAdds:"This item adds: +{attack} Attack · +{defense} Defense · +{healing} Heal.",lootSpaceFit:"Space: {width}×{height} fits in an open slot.",lootSpaceNoFit:"Space: no current {width}×{height} placement; rearrange before placing.",lootLinkJoin:"Link outlook: {tag} can join matching gear in a legal adjacent placement.",lootLinkNone:"Link outlook: no legal adjacent placement with matching {tag} gear.",lootLinkSealed:"Pressure: the current seal suppresses {tag} links."});
  Object.assign(copy["zh-Hant"],{lootCurrent:"目前行囊：攻擊 {attack} · 防禦 {defense} · 恢復 {healing} · 連結 {links}。",lootAdds:"這件裝備增加：+{attack} 攻擊 · +{defense} 防禦 · +{healing} 恢復。",lootSpaceFit:"空間：{width}×{height} 可放入空位。",lootSpaceNoFit:"空間：目前沒有可放置 {width}×{height} 的位置；放置前先重新整理。",lootLinkJoin:"連結展望：若放在相鄰的相同材質旁，{tag} 可加入連結。",lootLinkNone:"連結展望：目前沒有能與 {tag} 裝備相鄰的合法位置。",lootLinkSealed:"壓力：目前封印會抑制 {tag} 連結。"});
  Object.assign(copy.es,{lootCurrent:"Mochila actual: Ataque {attack} · Defensa {defense} · Curación {healing} · Enlaces {links}.",lootAdds:"Este objeto añade: +{attack} Ataque · +{defense} Defensa · +{healing} Curación.",lootSpaceFit:"Espacio: {width}×{height} cabe en un hueco libre.",lootSpaceNoFit:"Espacio: no hay una posición {width}×{height} disponible; reorganiza antes de colocarlo.",lootLinkJoin:"Enlace posible: {tag} puede unirse al equipo del mismo material en una posición adyacente legal.",lootLinkNone:"Enlace posible: no hay una posición adyacente legal junto a equipo {tag}.",lootLinkSealed:"Presión: el sello actual suprime los enlaces {tag}."});
  Object.assign(copy.ar,{lootCurrent:"الحقيبة الحالية: هجوم {attack} · دفاع {defense} · علاج {healing} · روابط {links}.",lootAdds:"تضيف هذه القطعة: +{attack} هجوم · +{defense} دفاع · +{healing} علاج.",lootSpaceFit:"المساحة: يمكن وضع {width}×{height} في خانة فارغة.",lootSpaceNoFit:"المساحة: لا يوجد موضع {width}×{height} حاليًا؛ أعد الترتيب قبل الوضع.",lootLinkJoin:"توقع الرابط: يمكن لـ {tag} الانضمام إلى عتاد مطابق في موضع متجاور قانوني.",lootLinkNone:"توقع الرابط: لا يوجد موضع متجاور قانوني مع عتاد {tag} مطابق.",lootLinkSealed:"الضغط: الختم الحالي يثبط روابط {tag}."});
  ...
  const stageTabLabels={"en":"Stages","zh-Hant":"關卡","zh-Hans":"关卡","ja":"ステージ","ko":"스테이지","es":"Niveles","pt-BR":"Fases","fr":"Niveaux","de":"Stufen","it":"Livelli","ru":"Уровни","hi":"चरण","ar":"المراحل"};
  const resultActionLabels={
    "en":["Stages","Next Stage","Replay"],"zh-Hant":["關卡","下一關","重玩"],"zh-Hans":["关卡","下一关","重玩"],
    "ja":["ステージ","次のステージ","リプレイ"],"ko":["스테이지","다음 스테이지","다시 플레이"],"es":["Niveles","Siguiente nivel","Repetir"],
    "pt-BR":["Fases","Próxima fase","Jogar novamente"],"fr":["Niveaux","Niveau suivant","Rejouer"],"de":["Stufen","Nächste Stufe","Erneut spielen"],
    "it":["Livelli","Livello successivo","Rigioca"],"ru":["Уровни","Следующий уровень","Переиграть"],"hi":["चरण","अगला चरण","फिर खेलें"],"ar":["المراحل","المرحلة التالية","إعادة اللعب"]
  };
  const activeRouteLocale=()=>document.documentElement.lang||"en";
  const activeResultLabels=()=>resultActionLabels[activeRouteLocale()]||resultActionLabels.en;
  function ensureSoundButton(){
    let button=$("#soundBtn");
    if(button)return button;
    const header=$(".main-topbar");
    if(!header)return null;
    button=document.createElement("button");
    button.id="soundBtn";
    button.className="icon-button main-sound-button";
    button.type="button";
  ...
  const runtimeCatalogLocales = new Set(["ja","ko","pt-BR","fr","de","it","ru","hi"]);
  const detectedLocale=window.WonderI18n?.actualLocale?.()||window.WonderI18n?.locale?.()||storedLocale||"en";
  let locale=copy[detectedLocale]||runtimeCatalogLocales.has(detectedLocale)?detectedLocale:"en";
  const runtimeCopySupplements = {
    ja:{items:"アイテム",unlocked:"解放済み",isolated:"孤立した装備",corrosion:"腐食",heat:"上段の熱",overload:"過負荷"},
    ko:{items:"아이템",unlocked:"잠금 해제",isolated:"고립된 장비",corrosion:"부식",heat:"맨 윗줄 열기",overload:"과부하"},
    "pt-BR":{items:"itens",unlocked:"desbloqueado",isolated:"itens isolados",corrosion:"corrosão",heat:"calor da fileira superior",overload:"sobrecarga",packmaster:"Rux, mestre da mochila"},
    fr:{items:"objets",unlocked:"débloqué",isolated:"objets isolés",corrosion:"corrosion",heat:"chaleur de la rangée supérieure",overload:"surcharge",tagNature:"Nature"},
    de:{items:"Gegenstände",unlocked:"freigeschaltet",isolated:"isolierte Gegenstände",corrosion:"Korrosion",heat:"Hitze in der oberen Reihe",overload:"Überlastung",region1:"Zahnwaldpfad",region3:"Uhrwerkhöhle",gold:"Gold",links:"Verbindungen",gearwoodTrailAlt:"Zahnwaldpfad"},
    it:{items:"oggetti",unlocked:"sbloccato",isolated:"oggetti isolati",corrosion:"corrosione",heat:"calore della riga superiore",overload:"sovraccarico"},
    ru:{items:"предметы",unlocked:"открыто",isolated:"изолированные предметы",corrosion:"коррозия",heat:"жар верхнего ряда",overload:"перегрузка"},
    hi:{items:"वस्तुएँ",unlocked:"अनलॉक",isolated:"अलग-थलग सामान",corrosion:"क्षरण",heat:"ऊपरी पंक्ति की गर्मी",overload:"अतिभार",bossPhase:"बॉस चरण"},
  };
  Object.assign(runtimeCopySupplements.ja,{blueprints:"設計図",blueprintForgeName:"残り火駆動",blueprintNatureName:"緑の守り",blueprintCaravanName:"隊商連結",blueprintPressure:"脅威：{rule}",blueprintNone:"有効な設計図なし"});
  Object.assign(runtimeCopySupplements.ko,{blueprints:"설계도",blueprintForgeName:"잿불 구동",blueprintNatureName:"초록 방벽",blueprintCaravanName:"대상 연결",blueprintPressure:"압박: {rule}",blueprintNone:"활성 설계도 없음"});
  Object.assign(runtimeCopySupplements["pt-BR"],{blueprints:"Projetos",blueprintForgeName:"Propulsão de brasa",blueprintNatureName:"Abrigo verdejante",blueprintCaravanName:"Elo da caravana",blueprintPressure:"Pressão: {rule}",blueprintNone:"Nenhum projeto ativo"});
  Object.assign(runtimeCopySupplements.fr,{blueprints:"Plans",blueprintForgeName:"Propulsion de braise",blueprintNatureName:"Abri verdoyant",blueprintCaravanName:"Relais de caravane",blueprintPressure:"Pression : {rule}",blueprintNone:"Aucun plan actif"});
  Object.assign(runtimeCopySupplements.de,{blueprints:"Baupläne",blueprintForgeName:"Glutantrieb",blueprintNatureName:"Grüner Schutz",blueprintCaravanName:"Karawanenrelais",blueprintPressure:"Druck: {rule}",blueprintNone:"Kein aktiver Bauplan"});
  Object.assign(runtimeCopySupplements.it,{blueprints:"Progetti",blueprintForgeName:"Spinta di brace",blueprintNatureName:"Rifugio verde",blueprintCaravanName:"Collegamento carovana",blueprintPressure:"Pressione: {rule}",blueprintNone:"Nessun progetto attivo"});
  Object.assign(runtimeCopySupplements.ru,{blueprints:"Чертежи",blueprintForgeName:"Угольный привод",blueprintNatureName:"Зелёное укрытие",blueprintCaravanName:"Караванная связь",blueprintPressure:"Давление: {rule}",blueprintNone:"Нет активного чертежа"});
  Object.assign(runtimeCopySupplements.hi,{blueprints:"खाके",blueprintForgeName:"अंगारा चाल",blueprintNatureName:"हरित आश्रय",blueprintCaravanName:"कारवां कड़ी",blueprintPressure:"दबाव: {rule}",blueprintNone:"कोई सक्रिय खाका नहीं"});
  Object.assign(runtimeCopySupplements.ja,{blueprintForgeRequirement:"鍛造装備を3個つなぐ（攻撃+3）",blueprintNatureRequirement:"自然装備を3個つなぐ（防御+4／回復+1）",blueprintCaravanRequirement:"異なる素材を3種つなぐ（攻撃+2／防御+2）",blueprintBuildRecap:"構成：{blueprints} · 最多素材 {material} · 連結 {links}"});
  Object.assign(runtimeCopySupplements.ko,{blueprintForgeRequirement:"대장간 장비 3개 연결 (공격 +3)",blueprintNatureRequirement:"자연 장비 3개 연결 (방어 +4 / 회복 +1)",blueprintCaravanRequirement:"서로 다른 재료 3종 연결 (공격 +2 / 방어 +2)",blueprintBuildRecap:"구성: {blueprints} · 주 재료 {material} · 연결 {links}"});
  Object.assign(runtimeCopySupplements["pt-BR"],{blueprintForgeRequirement:"Conecte 3 itens de forja (+3 ATQ)",blueprintNatureRequirement:"Conecte 3 itens naturais (+4 DEF / +1 cura)",blueprintCaravanRequirement:"Conecte 3 materiais diferentes (+2 ATQ / +2 DEF)",blueprintBuildRecap:"Construção: {blueprints} · Material principal {material} · Conexões {links}"});
  Object.assign(runtimeCopySupplements.fr,{blueprintForgeRequirement:"Reliez 3 objets de forge (+3 ATQ)",blueprintNatureRequirement:"Reliez 3 objets naturels (+4 DÉF / +1 soin)",blueprintCaravanRequirement:"Reliez 3 matériaux différents (+2 ATQ / +2 DÉF)",blueprintBuildRecap:"Configuration : {blueprints} · Matériau principal {material} · Liens {links}"});
  Object.assign(runtimeCopySupplements.de,{blueprintForgeRequirement:"Verbinde 3 Schmiedeobjekte (+3 ANG)",blueprintNatureRequirement:"Verbinde 3 Naturobjekte (+4 VER / +1 Heilung)",blueprintCaravanRequirement:"Verbinde 3 verschiedene Materialien (+2 ANG / +2 VER)",blueprintBuildRecap:"Bau: {blueprints} · Stärkstes Material {material} · Verbindungen {links}"});
  Object.assign(runtimeCopySupplements.it,{blueprintForgeRequirement:"Collega 3 oggetti da forgia (+3 ATT)",blueprintNatureRequirement:"Collega 3 oggetti naturali (+4 DIF / +1 cura)",blueprintCaravanRequirement:"Collega 3 materiali diversi (+2 ATT / +2 DIF)",blueprintBuildRecap:"Configurazione: {blueprints} · Materiale principale {material} · Collegamenti {links}"});
  Object.assign(runtimeCopySupplements.ru,{blueprintForgeRequirement:"Соедините 3 кузнечных предмета (+3 АТК)",blueprintNatureRequirement:"Соедините 3 природных предмета (+4 ЗАЩ / +1 лечение)",blueprintCaravanRequirement:"Соедините 3 разных материала (+2 АТК / +2 ЗАЩ)",blueprintBuildRecap:"Сборка: {blueprints} · Главный материал {material} · Связи {links}"});
  Object.assign(runtimeCopySupplements.hi,{blueprintForgeRequirement:"3 गढ़ाई वस्तुएँ जोड़ें (+3 आक्रमण)",blueprintNatureRequirement:"3 प्रकृति वस्तुएँ जोड़ें (+4 रक्षा / +1 उपचार)",blueprintCaravanRequirement:"3 अलग सामग्री जोड़ें (+2 आक्रमण / +2 रक्षा)",blueprintBuildRecap:"बनावट: {blueprints} · प्रमुख सामग्री {material} · कड़ियाँ {links}"});
  Object.assign(runtimeCopySupplements.ja,{lootCurrent:"現在のパック：攻撃 {attack} · 防御 {defense} · 回復 {healing} · リンク {links}。",lootAdds:"このアイテムの追加：攻撃 +{attack} · 防御 +{defense} · 回復 +{healing}。",lootSpaceFit:"空き：{width}×{height}で置けます。",lootSpaceNoFit:"空き：現在は{width}×{height}を置ける場所がありません。置く前に組み直してください。",lootLinkJoin:"リンク見込み：合法な隣接配置なら{tag}が同じ素材の装備につながります。",lootLinkNone:"リンク見込み：{tag}と同じ素材の装備に隣接できる合法配置がありません。",lootLinkSealed:"圧力：現在の封印は{tag}リンクを無効にします。"});
  Object.assign(runtimeCopySupplements.ko,{lootCurrent:"현재 가방: 공격 {attack} · 방어 {defense} · 회복 {healing} · 연결 {links}개.",lootAdds:"이 장비가 더하는 값: 공격 +{attack} · 방어 +{defense} · 회복 +{healing}.",lootSpaceFit:"공간: {width}×{height}로 빈칸에 놓을 수 있습니다.",lootSpaceNoFit:"공간: 현재 {width}×{height}를 놓을 곳이 없습니다. 놓기 전에 재배치하세요.",lootLinkJoin:"연결 전망: 합법적인 인접 배치라면 {tag}가 같은 재료 장비와 연결될 수 있습니다.",lootLinkNone:"연결 전망: {tag} 장비와 인접할 수 있는 합법적인 위치가 없습니다.",lootLinkSealed:"압박: 현재 봉인이 {tag} 연결을 억제합니다."});
  Object.assign(runtimeCopySupplements["pt-BR"],{lootCurrent:"Mochila atual: Ataque {attack} · Defesa {defense} · Cura {healing} · Conexões {links}.",lootAdds:"Este item adiciona: +{attack} Ataque · +{defense} Defesa · +{healing} Cura.",lootSpaceFit:"Espaço: {width}×{height} cabe em um espaço vazio.",lootSpaceNoFit:"Espaço: não há posição {width}×{height} disponível agora; reorganize antes de colocar.",lootLinkJoin:"Possível conexão: {tag} pode se ligar a equipamento do mesmo material em uma posição adjacente válida.",lootLinkNone:"Possível conexão: não há posição adjacente válida junto a equipamento {tag}.",lootLinkSealed:"Pressão: o selo atual suprime conexões {tag}."});
  Object.assign(runtimeCopySupplements.fr,{lootCurrent:"Sac actuel : Attaque {attack} · Défense {defense} · Soin {healing} · Liens {links}.",lootAdds:"Cet objet ajoute : +{attack} Attaque · +{defense} Défense · +{healing} Soin.",lootSpaceFit:"Espace : {width}×{height} tient dans un emplacement libre.",lootSpaceNoFit:"Espace : aucun emplacement {width}×{height} n’est disponible ; réorganisez avant de le placer.",lootLinkJoin:"Lien possible : {tag} peut rejoindre un équipement du même matériau dans une position adjacente valide.",lootLinkNone:"Lien possible : aucune position adjacente valide avec un équipement {tag} correspondant.",lootLinkSealed:"Pression : le sceau actuel neutralise les liens {tag}."});
  Object.assign(runtimeCopySupplements.de,{lootCurrent:"Aktuelle Tasche: Angriff {attack} · Verteidigung {defense} · Heilung {healing} · Verbindungen {links}.",lootAdds:"Dieser Gegenstand gibt: +{attack} Angriff · +{defense} Verteidigung · +{healing} Heilung.",lootSpaceFit:"Platz: {width}×{height} passt in einen freien Slot.",lootSpaceNoFit:"Platz: Aktuell passt kein {width}×{height}-Feld; vor dem Platzieren umpacken.",lootLinkJoin:"Verbindungs-Chance: {tag} kann sich bei einer gültigen Nachbarposition mit gleichem Material verbinden.",lootLinkNone:"Verbindungs-Chance: Keine gültige Nachbarposition für passendes {tag}-Material.",lootLinkSealed:"Druck: Das aktuelle Siegel unterdrückt {tag}-Verbindungen."});
  Object.assign(runtimeCopySupplements.it,{lootCurrent:"Equipaggiamento attuale: Attacco {attack} · Difesa {defense} · Cura {healing} · Legami {links}.",lootAdds:"Questo oggetto aggiunge: +{attack} Attacco · +{defense} Difesa · +{healing} Cura.",lootSpaceFit:"Spazio: {width}×{height} entra in uno spazio libero.",lootSpaceNoFit:"Spazio: non c’è una posizione {width}×{height} disponibile; riordina prima di posizionarlo.",lootLinkJoin:"Possibile legame: {tag} può unirsi a un equipaggiamento dello stesso materiale in una posizione adiacente valida.",lootLinkNone:"Possibile legame: non c’è una posizione adiacente valida accanto a equipaggiamento {tag}.",lootLinkSealed:"Pressione: il sigillo attuale sopprime i legami {tag}."});
  Object.assign(runtimeCopySupplements.ru,{lootCurrent:"Текущая сумка: атака {attack} · защита {defense} · лечение {healing} · связи {links}.",lootAdds:"Этот предмет добавляет: атака +{attack} · защита +{defense} · лечение +{healing}.",lootSpaceFit:"Место: предмет {width}×{height} помещается в свободную ячейку.",lootSpaceNoFit:"Место: сейчас нет позиции для {width}×{height}; перестройте сумку перед размещением.",lootLinkJoin:"Связь: {tag} может соединиться с таким же материалом при допустимом соседнем размещении.",lootLinkNone:"Связь: нет допустимой соседней позиции рядом с предметом {tag}.",lootLinkSealed:"Давление: текущая печать подавляет связи {tag}."});
  Object.assign(runtimeCopySupplements.hi,{lootCurrent:"मौजूदा पैक: आक्रमण {attack} · रक्षा {defense} · उपचार {healing} · कड़ियाँ {links}।",lootAdds:"यह वस्तु जोड़ती है: +{attack} आक्रमण · +{defense} रक्षा · +{healing} उपचार।",lootSpaceFit:"जगह: {width}×{height} खाली खाने में फिट होती है।",lootSpaceNoFit:"जगह: अभी {width}×{height} रखने की जगह नहीं है; रखने से पहले पैक फिर सजाएँ।",lootLinkJoin:"कड़ी की संभावना: वैध पड़ोसी जगह में {tag} समान सामग्री वाले सामान से जुड़ सकता है।",lootLinkNone:"कड़ी की संभावना: {tag} सामान के पास कोई वैध पड़ोसी जगह नहीं है।",lootLinkSealed:"दबाव: मौजूदा सील {tag} कड़ियों को रोकती है।"});
  const runtimeSourceSupplements = {
    de:{
      "Gearwood Trail":"Zahnwaldpfad",
      "Moonlit Quarry":"Mondlichtsteinbruch",
      "Clockwork Hollow":"Uhrwerkhöhle",
      "Workshop Gate":"Werkstatttor",
      "Learn the five-room rhythm and build one material link.":"Lerne den Rhythmus der fünf Räume kennen und stelle eine Materialverbindung her.",
      "Ironroot Crossing":"Eisenwurzel-Kreuzung",
      "Plated Boars absorb the first hit with armor.":"Gepanzerte Eber fangen den ersten Treffer mit ihrer Rüstung ab.",
      "Rustwing Crow":"Rostflügelkrähe",
      "Gale Fox Scout":"Sturmfuchs-Späher",
    },
    it:{
      "Gale Fox Scout":"Esploratore volpe della burrasca",
    },
    hi:{
      "Boiler Walk":"बॉयलर मार्ग",
  ...
  Object.assign(copy.en,{packPlanDefault:"Pack plan: Attack {attack}, Defense {defense}, Links {links}; connect matching materials before fighting.",packPlanShield:"Pack plan: break Shield {shield} with Attack {attack} and {links} Links.",packPlanOpening:"Pack plan: keep Defense {defense} to absorb Opening strike {opening}.",packPlanHeat:"Pack plan: keep the top row open ({topRow} occupied); build Links below.",packPlanIsolation:"Pack plan: connect {isolated} isolated items before fighting.",packPlanCorrosion:"Pack plan: use Attack {attack} to finish before Defense {defense} corrodes.",packPlanTags:"Pack plan: split material tags; current Links {links} must survive the seal.",packPlanPhase:"Pack plan: use Attack {attack} in the brace phase; keep Defense {defense} for rage.",packPlanNature:"Pack plan: keep a Nature Link active; current Links {links} answer the regrowth rule."});
  Object.assign(copy["zh-Hant"],{packPlanDefault:"行囊策略：攻擊 {attack}、防禦 {defense}、連結 {links}；戰鬥前先連起相同材質。",packPlanShield:"行囊策略：用攻擊 {attack} 與 {links} 個連結擊破護盾 {shield}。",packPlanOpening:"行囊策略：保留防禦 {defense}，承受開場突襲 {opening}。",packPlanHeat:"行囊策略：保持頂排空間（目前佔用 {topRow} 格），在下方建立連結。",packPlanIsolation:"行囊策略：戰鬥前連起 {isolated} 件孤立裝備。",packPlanCorrosion:"行囊策略：用攻擊 {attack} 快速結束，避免防禦 {defense} 被腐蝕。",packPlanTags:"行囊策略：分散材質標籤；目前 {links} 個連結要避開封印。",packPlanPhase:"行囊策略：在蓄力階段用攻擊 {attack}，為狂怒階段保留防禦 {defense}。",packPlanNature:"行囊策略：維持自然連結；目前 {links} 個連結可應對再生規則。"});
  Object.assign(copy["zh-Hans"],{packPlanDefault:"行囊策略：攻击 {attack}、防御 {defense}、连接 {links}；战斗前先连起相同材质。",packPlanShield:"行囊策略：用攻击 {attack} 和 {links} 个连接击破护盾 {shield}。",packPlanOpening:"行囊策略：保留防御 {defense}，承受开场突袭 {opening}。",packPlanHeat:"行囊策略：保持顶排空间（当前占用 {topRow} 格），在下方建立连接。",packPlanIsolation:"行囊策略：战斗前连起 {isolated} 件孤立装备。",packPlanCorrosion:"行囊策略：用攻击 {attack} 快速结束，避免防御 {defense} 被腐蚀。",packPlanTags:"行囊策略：分散材质标签；当前 {links} 个连接要避开封印。",packPlanPhase:"行囊策略：在蓄力阶段用攻击 {attack}，为狂怒阶段保留防御 {defense}。",packPlanNature:"行囊策略：维持自然连接；当前 {links} 个连接可应对再生规则。"});
  Object.assign(copy.es,{packPlanDefault:"Plan de mochila: Ataque {attack}, Defensa {defense}, Enlaces {links}; conecta materiales iguales antes de luchar.",packPlanShield:"Plan de mochila: rompe el Escudo {shield} con Ataque {attack} y {links} Enlaces.",packPlanOpening:"Plan de mochila: conserva Defensa {defense} para absorber el ataque inicial {opening}.",packPlanHeat:"Plan de mochila: deja libre la fila superior ({topRow} ocupadas); crea Enlaces abajo.",packPlanIsolation:"Plan de mochila: conecta {isolated} objetos aislados antes de luchar.",packPlanCorrosion:"Plan de mochila: usa Ataque {attack} para terminar antes de que la Defensa {defense} se corroa.",packPlanTags:"Plan de mochila: divide las etiquetas; los {links} Enlaces actuales deben superar el sello.",packPlanPhase:"Plan de mochila: usa Ataque {attack} en la fase de defensa; guarda Defensa {defense} para la rabia.",packPlanNature:"Plan de mochila: mantén un Enlace de Naturaleza; los {links} Enlaces actuales responden a la regeneración."});
  Object.assign(copy.ar,{packPlanDefault:"خطة الحقيبة: هجوم {attack}، دفاع {defense}، روابط {links}؛ صِل المواد المتطابقة قبل القتال.",packPlanShield:"خطة الحقيبة: اكسر الدرع {shield} بهجوم {attack} ومع {links} من الروابط.",packPlanOpening:"خطة الحقيبة: احتفظ بدفاع {defense} لامتصاص الضربة الافتتاحية {opening}.",packPlanHeat:"خطة الحقيبة: اترك الصف العلوي مفتوحًا ({topRow} مشغولة) وابنِ الروابط أسفلًا.",packPlanIsolation:"خطة الحقيبة: صِل {isolated} من القطع المعزولة قبل القتال.",packPlanCorrosion:"خطة الحقيبة: استخدم هجوم {attack} للإنهاء قبل أن يتآكل دفاع {defense}.",packPlanTags:"خطة الحقيبة: وزّع وسوم المواد؛ يجب أن تصمد الروابط الحالية وعددها {links} أمام الختم.",packPlanPhase:"خطة الحقيبة: استخدم هجوم {attack} في طور التثبيت واحتفظ بدفاع {defense} للغضب.",packPlanNature:"خطة الحقيبة: حافظ على رابطة الطبيعة؛ الروابط الحالية وعددها {links} تواجه قاعدة التجدد."});
  Object.assign(runtimeCopySupplements.ja,{packPlanDefault:"パック計画：攻撃 {attack}、防御 {defense}、リンク {links}。戦闘前に同じ素材をつなぐ。",packPlanShield:"パック計画：攻撃 {attack} と {links} リンクでシールド {shield} を破る。",packPlanOpening:"パック計画：防御 {defense} を残して開幕攻撃 {opening} を受ける。",packPlanHeat:"パック計画：最上段を空け（使用 {topRow} マス）、下段でリンクを作る。",packPlanIsolation:"パック計画：戦闘前に孤立した装備 {isolated} 個をつなぐ。",packPlanCorrosion:"パック計画：攻撃 {attack} で早く終え、防御 {defense} の腐食を抑える。",packPlanTags:"パック計画：素材タグを分散し、封印に対して {links} リンクを保つ。",packPlanPhase:"パック計画：構えの段階は攻撃 {attack}、怒りの段階は防御 {defense}。",packPlanNature:"パック計画：自然リンクを保ち、再生ルールに備える。現在のリンク {links}。"});
  Object.assign(runtimeCopySupplements.ko,{packPlanDefault:"가방 계획: 공격 {attack}, 방어 {defense}, 연결 {links}; 싸우기 전에 같은 재료를 연결하세요.",packPlanShield:"가방 계획: 공격 {attack}과 연결 {links}개로 보호막 {shield}을 부수세요.",packPlanOpening:"가방 계획: 방어 {defense}를 남겨 개시 공격 {opening}을 버티세요.",packPlanHeat:"가방 계획: 윗줄을 비우고({topRow}칸 사용), 아래에서 연결하세요.",packPlanIsolation:"가방 계획: 싸우기 전에 고립된 장비 {isolated}개를 연결하세요.",packPlanCorrosion:"가방 계획: 공격 {attack}으로 빨리 끝내 방어 {defense}의 부식을 줄이세요.",packPlanTags:"가방 계획: 재료 태그를 나누고 봉인에 맞서 연결 {links}개를 유지하세요.",packPlanPhase:"가방 계획: 버티기 단계에는 공격 {attack}, 분노 단계에는 방어 {defense}를 쓰세요.",packPlanNature:"가방 계획: 자연 연결을 유지해 재생 규칙에 대응하세요. 현재 연결 {links}개."});
  Object.assign(runtimeCopySupplements["pt-BR"],{packPlanDefault:"Plano da mochila: Ataque {attack}, Defesa {defense}, Conexões {links}; conecte materiais iguais antes de lutar.",packPlanShield:"Plano da mochila: quebre o Escudo {shield} com Ataque {attack} e {links} Conexões.",packPlanOpening:"Plano da mochila: mantenha Defesa {defense} para absorver o ataque inicial {opening}.",packPlanHeat:"Plano da mochila: deixe a fileira superior livre ({topRow} ocupadas); crie Conexões abaixo.",packPlanIsolation:"Plano da mochila: conecte {isolated} itens isolados antes de lutar.",packPlanCorrosion:"Plano da mochila: use Ataque {attack} para terminar antes que a Defesa {defense} sofra corrosão.",packPlanTags:"Plano da mochila: divida os materiais; mantenha {links} Conexões contra o selo.",packPlanPhase:"Plano da mochila: use Ataque {attack} na fase de bloqueio; guarde Defesa {defense} para a fúria.",packPlanNature:"Plano da mochila: mantenha uma Conexão de Natureza; as {links} atuais respondem à regeneração."});
  Object.assign(runtimeCopySupplements.fr,{packPlanDefault:"Plan de sac : Attaque {attack}, Défense {defense}, Liens {links} ; reliez les matériaux identiques avant le combat.",packPlanShield:"Plan de sac : brisez le Bouclier {shield} avec Attaque {attack} et {links} Liens.",packPlanOpening:"Plan de sac : gardez Défense {defense} pour absorber l’attaque initiale {opening}.",packPlanHeat:"Plan de sac : gardez la rangée du haut libre ({topRow} occupées) ; créez des Liens dessous.",packPlanIsolation:"Plan de sac : reliez les {isolated} objets isolés avant le combat.",packPlanCorrosion:"Plan de sac : utilisez Attaque {attack} pour finir avant la corrosion de Défense {defense}.",packPlanTags:"Plan de sac : répartissez les matériaux ; gardez {links} Liens contre le sceau.",packPlanPhase:"Plan de sac : utilisez Attaque {attack} pendant la garde ; gardez Défense {defense} pour la rage.",packPlanNature:"Plan de sac : gardez un Lien de Nature ; les {links} Liens actuels répondent à la régénération."});
  Object.assign(runtimeCopySupplements.de,{packPlanDefault:"Taschenplan: Angriff {attack}, Verteidigung {defense}, Verbindungen {links}; gleiche Materialien vor dem Kampf verbinden.",packPlanShield:"Taschenplan: Schild {shield} mit Angriff {attack} und {links} Verbindungen brechen.",packPlanOpening:"Taschenplan: Verteidigung {defense} gegen den Eröffnungsschlag {opening} bewahren.",packPlanHeat:"Taschenplan: obere Reihe frei lassen ({topRow} belegt); darunter Verbindungen bauen.",packPlanIsolation:"Taschenplan: {isolated} isolierte Gegenstände vor dem Kampf verbinden.",packPlanCorrosion:"Taschenplan: mit Angriff {attack} schnell enden, bevor Verteidigung {defense} korrodiert.",packPlanTags:"Taschenplan: Materialien teilen und {links} Verbindungen gegen das Siegel bewahren.",packPlanPhase:"Taschenplan: in der Blockphase Angriff {attack}, für die Wutphase Verteidigung {defense} nutzen.",packPlanNature:"Taschenplan: eine Naturverbindung gegen die Regeneration halten; aktuell {links} Verbindungen."});
  Object.assign(runtimeCopySupplements.it,{packPlanDefault:"Piano equipaggiamento: Attacco {attack}, Difesa {defense}, Legami {links}; collega materiali uguali prima del combattimento.",packPlanShield:"Piano equipaggiamento: spezza lo Scudo {shield} con Attacco {attack} e {links} Legami.",packPlanOpening:"Piano equipaggiamento: conserva Difesa {defense} per assorbire l’attacco iniziale {opening}.",packPlanHeat:"Piano equipaggiamento: lascia libera la riga superiore ({topRow} occupate); crea Legami sotto.",packPlanIsolation:"Piano equipaggiamento: collega {isolated} oggetti isolati prima del combattimento.",packPlanCorrosion:"Piano equipaggiamento: usa Attacco {attack} per finire prima che la Difesa {defense} si corroda.",packPlanTags:"Piano equipaggiamento: dividi i materiali e mantieni {links} Legami contro il sigillo.",packPlanPhase:"Piano equipaggiamento: usa Attacco {attack} nella guardia e conserva Difesa {defense} per la furia.",packPlanNature:"Piano equipaggiamento: mantieni un Legame Natura; gli attuali {links} Legami contrastano la rigenerazione."});
  Object.assign(runtimeCopySupplements.ru,{packPlanDefault:"План снаряжения: атака {attack}, защита {defense}, связи {links}; соедините одинаковые материалы до боя.",packPlanShield:"План снаряжения: пробейте щит {shield} атакой {attack} и связями: {links}.",packPlanOpening:"План снаряжения: сохраните защиту {defense}, чтобы выдержать первый удар {opening}.",packPlanHeat:"План снаряжения: оставьте верхний ряд свободным (занято {topRow}); стройте связи ниже.",packPlanIsolation:"План снаряжения: соедините изолированные предметы: {isolated}, до боя.",packPlanCorrosion:"План снаряжения: завершите бой атакой {attack}, пока защита {defense} не разъедена.",packPlanTags:"План снаряжения: разделите материалы и сохраните {links} связей против печати.",packPlanPhase:"План снаряжения: атакуйте на фазе защиты ({attack}), сохраните защиту {defense} для ярости.",packPlanNature:"План снаряжения: сохраните связь Природы; текущие связи ({links}) отвечают на регенерацию."});
  Object.assign(runtimeCopySupplements.hi,{packPlanDefault:"पैक योजना: आक्रमण {attack}, रक्षा {defense}, कड़ियाँ {links}; लड़ाई से पहले समान सामग्री जोड़ें।",packPlanShield:"पैक योजना: आक्रमण {attack} और {links} कड़ियों से शील्ड {shield} तोड़ें।",packPlanOpening:"पैक योजना: शुरुआती प्रहार {opening} सहने के लिए रक्षा {defense} रखें।",packPlanHeat:"पैक योजना: ऊपरी पंक्ति खाली रखें ({topRow} भरे); नीचे कड़ियाँ बनाएँ।",packPlanIsolation:"पैक योजना: लड़ाई से पहले {isolated} अलग वस्तुओं को जोड़ें।",packPlanCorrosion:"पैक योजना: रक्षा {defense} के क्षरण से पहले आक्रमण {attack} से समाप्त करें।",packPlanTags:"पैक योजना: सामग्री टैग बाँटें और सील के विरुद्ध {links} कड़ियाँ रखें।",packPlanPhase:"पैक योजना: रोक चरण में आक्रमण {attack}, क्रोध चरण के लिए रक्षा {defense} रखें।",packPlanNature:"पैक योजना: प्रकृति कड़ी बनाए रखें; वर्तमान {links} कड़ियाँ पुनर्जनन नियम का उत्तर हैं।"});

  Object.assign(copy.en,{encounterLesson:"Lesson: {enemy} fell with {health} left. {build} Enemy pressure: {pressure}.",encounterBuildLinked:"{links} active Links added {attack} Attack and {defense} Defense.",encounterBuildNoLinks:"No active Links; placement and base stats carried this room."});
  Object.assign(copy["zh-Hant"],{encounterLesson:"回顧：{enemy} 倒下時還剩 {health}。{build} 敵人壓力：{pressure}。",encounterBuildLinked:"{links} 個啟用中的連結帶來 {attack} 點攻擊與 {defense} 點防禦。",encounterBuildNoLinks:"沒有啟用中的連結；本房間靠放置與基礎數值撐過。"});
  Object.assign(copy["zh-Hans"],{encounterLesson:"复盘：{enemy} 倒下时还剩 {health}。{build} 敌人压力：{pressure}。",encounterBuildLinked:"{links} 个启用中的连接带来 {attack} 点攻击与 {defense} 点防御。",encounterBuildNoLinks:"没有启用中的连接；本房间靠放置与基础数值撑过。"});
  Object.assign(copy.es,{encounterLesson:"Lección: {enemy} cayó con {health} restante. {build} Presión enemiga: {pressure}.",encounterBuildLinked:"{links} Enlaces activos añadieron {attack} Ataque y {defense} Defensa.",encounterBuildNoLinks:"No hubo Enlaces activos; la colocación y los valores base sostuvieron este encuentro."});
  Object.assign(copy.ar,{encounterLesson:"درس: سقط {enemy} وبقيت لديه {health}. {build} ضغط العدو: {pressure}.",encounterBuildLinked:"أضافت {links} من الروابط النشطة {attack} هجوم و{defense} دفاع.",encounterBuildNoLinks:"لا توجد روابط نشطة؛ حافظ الوضع والقيم الأساسية على هذه المواجهة."});
  Object.assign(runtimeCopySupplements.ja,{encounterLesson:"学び：{enemy}を{health}残して倒しました。{build} 敵の圧力：{pressure}。",encounterBuildLinked:"有効なリンク{links}個が攻撃{attack}と防御{defense}を加えました。",encounterBuildNoLinks:"有効なリンクはありません。配置と基礎値でこの部屋を乗り切りました。"});
  Object.assign(runtimeCopySupplements.ko,{encounterLesson:"복기: {enemy}를 {health} 남기고 쓰러뜨렸습니다. {build} 적의 압박: {pressure}.",encounterBuildLinked:"활성 연결 {links}개가 공격 {attack}과 방어 {defense}를 더했습니다.",encounterBuildNoLinks:"활성 연결이 없습니다. 배치와 기본 능력치로 이 방을 버텼습니다."});
  Object.assign(runtimeCopySupplements["pt-BR"],{encounterLesson:"Lição: {enemy} caiu com {health} restante. {build} Pressão inimiga: {pressure}.",encounterBuildLinked:"{links} Conexões ativas adicionaram {attack} de Ataque e {defense} de Defesa.",encounterBuildNoLinks:"Sem Conexões ativas; o posicionamento e os atributos básicos sustentaram este encontro."});
  Object.assign(runtimeCopySupplements.fr,{encounterLesson:"Leçon : {enemy} est tombé avec {health} restant. {build} Pression ennemie : {pressure}.",encounterBuildLinked:"{links} Liens actifs ont ajouté {attack} Attaque et {defense} Défense.",encounterBuildNoLinks:"Aucun Lien actif ; le placement et les valeurs de base ont tenu ce combat."});
  Object.assign(runtimeCopySupplements.de,{encounterLesson:"Lektion: {enemy} fiel mit {health} übrig. {build} Feinddruck: {pressure}.",encounterBuildLinked:"{links} aktive Verbindungen gaben {attack} Angriff und {defense} Verteidigung.",encounterBuildNoLinks:"Keine aktiven Verbindungen; Platzierung und Grundwerte trugen diesen Raum."});
  Object.assign(runtimeCopySupplements.it,{encounterLesson:"Lezione: {enemy} è caduto lasciando {health}. {build} Pressione nemica: {pressure}.",encounterBuildLinked:"{links} Legami attivi hanno aggiunto {attack} Attacco e {defense} Difesa.",encounterBuildNoLinks:"Nessun Legame attivo; posizionamento e valori base hanno retto questa stanza."});
  Object.assign(runtimeCopySupplements.ru,{encounterLesson:"Разбор: {enemy} повержен, осталось {health}. {build} Давление врага: {pressure}.",encounterBuildLinked:"Активные связи ({links}) добавили атаки: {attack}, защиты: {defense}.",encounterBuildNoLinks:"Активных связей нет; в этой комнате помогли размещение и базовые параметры."});
  Object.assign(runtimeCopySupplements.hi,{encounterLesson:"सीख: {enemy} गिर गया और {health} बचा। {build} दुश्मन का दबाव: {pressure}।",encounterBuildLinked:"{links} सक्रिय कड़ियों ने {attack} आक्रमण और {defense} रक्षा जोड़ी।",encounterBuildNoLinks:"कोई सक्रिय कड़ी नहीं थी; इस कमरे में स्थान और आधार आँकड़ों ने साथ दिया।"});
  ...
  function translateForLocale(value,target=locale){
    ...
    return runtimeCatalogLocales.has(target)&&localizer?.locale===target
      ? localizer.translate(source)
      : source;
  }
  const startButton=$("#startBtn");
  const syncStartButton=()=>{const expected=t("start");if(startButton.textContent!==expected)startButton.textContent=expected;};
  new MutationObserver(syncStartButton).observe(startButton,{childList:true,characterData:true,subtree:true});
  document.addEventListener("DOMContentLoaded",()=>setTimeout(syncStartButton,0),{once:true});
  ...
  let stageBrowseLogical = centeredStage;
  let stageSettleFrame = 0;
  let lastInputType = "pointer";

  function t(key,vars={}){const source=(copy[locale]?.[key]||runtimeCopySupplements[locale]?.[key]||copy.en[key]||key).replace(/\{(\w+)\}/g,(_,name)=>vars[name]??`{${name}}`);return translateForLocale(source);}
  const levelTerms={en:"Level","zh-Hant":"等級","zh-Hans":"等级",ja:"レベル",ko:"레벨",es:"Nivel","pt-BR":"Nível",fr:"Niveau",de:"Stufe",it:"Livello",ru:"Уровень",hi:"स्तर",ar:"المستوى"};
  const stageUnlockedTemplates={en:"Stage {stage} unlocked","zh-Hant":"第 {stage} 關已解鎖","zh-Hans":"第 {stage} 关已解锁",ja:"ステージ{stage}をアンロック",ko:"스테이지 {stage} 잠금 해제",es:"Etapa {stage} desbloqueada","pt-BR":"Fase {stage} desbloqueada",fr:"Étape {stage} débloquée",de:"Stufe {stage} freigeschaltet",it:"Fase {stage} sbloccata",ru:"Этап {stage} открыт",hi:"चरण {stage} अनलॉक",ar:"تم فتح المرحلة {stage}"};
  function activeLocale(){return window.WonderI18n?.actualLocale?.()||document.documentElement.lang||locale||"en";}
  function formatLevel(level){return `${levelTerms[activeLocale()]||levelTerms.en} ${level}`;}
  function formatStageUnlocked(stage){return (stageUnlockedTemplates[activeLocale()]||stageUnlockedTemplates.en).replace("{stage}",String(stage));}
  ...
  function saveRun(){return run?writeStorage(RUN_KEY,JSON.stringify(run)):removeStorage(RUN_KEY);}
  function sound(name){window.WeightPlayAudio?.play(name);}
  ...
  function noteInput(event){lastInputType=event?.detail===0?"keyboard":event?.pointerType==="touch"?"touch":event?.pointerType==="mouse"?"mouse":"pointer";}
  function track(name,data={}){try{window.WonderAnalytics?.track(name,{game_id:GAME_ID,game_version:GAME_VERSION,interface_version:INTERFACE_VERSION,locale:window.WonderI18n?.actualLocale?.()||document.documentElement.lang||locale,viewport_bucket:viewportBucket(),input_type:lastInputType,...data});}catch{}}
  ...
  function itemName(item){return item.name[locale]||translateForLocale(item.name.en);}
  function itemAsset(item){return `${ASSET}${item.id}.webp`;}
  function setFeedback(text){const row=$("#feedbackRow");row.classList.remove("is-item-details");row.removeAttribute("title");row.removeAttribute("aria-label");row.textContent=text;}
  function setItemFeedback(item){
    ...
    const blueprint=blueprintDefinitions.find((definition)=>definition.id===item.tag)||blueprintDefinitions.find((definition)=>definition.id==="caravan");
    effect.textContent+=` · ${t(blueprint.nameKey)}: ${t(blueprint.requirementKey)}`;
...
function showScreen(name,focusTarget=false){if(name!=="stage"&&railSettleTimer){clearTimeout(railSettleTimer);railSettleTimer=0;}Object.entries(screens).forEach(([key,node])=>node.hidden=key!==name);document.body.dataset.screen=name;document.body.classList.toggle("is-game-playing",name==="battle");if(name==="main")renderMain();if(name==="stage")renderStage();if(name==="battle")renderBattle();if(focusTarget)requestAnimationFrame(()=>requestAnimationFrame(()=>{const target=name==="main"?$("#startBtn"):name==="stage"?$(`.region-card[data-stage="${selectedStage}"]`):$(".pack-cell[tabindex='0']");target?.focus({preventScroll:true});}));
  ...
  function localized(pair){const index=locale==="zh-Hant"?1:locale==="es"?2:0;return pair?.[locale]||translateForLocale(pair?.[index]||pair?.[0]||"");}
  const clampStage=(index)=>Math.max(0,Math.min(STAGE_COUNT-1,Number(index)||0));
  const stageWindowLimit=()=>Math.max(0,STAGE_COUNT-STAGE_CARD_POOL_SIZE);
  ...
  function createStageCard(){
    const card=document.createElement("button");card.type="button";card.className="region-card";
    card.addEventListener("focus",()=>{const index=clampStage(card.dataset.stage);railKeyboardOwner=index;centeredStage=index;stageBrowseLogical=index;syncStageCards();if(card.getAttribute("aria-disabled")!=="true"){selectedStage=index;progress.selectedStage=index;saveProgress();}});
    card.addEventListener("click",()=>{const index=clampStage(card.dataset.stage);if(card.getAttribute("aria-disabled")==="true")return;centeredStage=index;selectedStage=index;stageBrowseLogical=index;progress.selectedStage=index;saveProgress();newRun(index);});
    return card;
  }
  function bindStageCard(card,index){
    const definition=stages[index],region=regions[Math.floor(index/5)],unlocked=Math.max(1,Math.min(STAGE_COUNT,Number(progress.unlockedStage)||1)),available=index<unlocked,cleared=progress.completedStages.includes(index+1),checkpoint=(index+1)%5===0;
    card.className=`region-card${cleared?" is-cleared":""}${checkpoint?" is-checkpoint":""}`;
    card.dataset.stage=String(index);card.setAttribute("aria-posinset",String(index+1));card.setAttribute("aria-setsize",String(STAGE_COUNT));card.setAttribute("aria-disabled",available?"false":"true");card.toggleAttribute("data-wp-stage-unlocked",available);card.setAttribute("aria-keyshortcuts","ArrowLeft ArrowRight Home End");
    if(index%5===0){card.dataset.region=String(Math.floor(index/5));delete card.dataset.regionGroup;}else{card.dataset.regionGroup=String(Math.floor(index/5));delete card.dataset.region;}
  ...
  function syncStageCards(){stageCardPool.forEach((card)=>{const index=clampStage(card.dataset.stage),active=index===centeredStage;bindStageCard(card,index);card.tabIndex=active?0:-1;card.classList.toggle("is-selected",active);if(active)card.setAttribute("aria-current","true");else card.removeAttribute("aria-current");});}
  function buildStageCardPool(){
    const rail=$("#regionRail"),count=Math.min(STAGE_CARD_POOL_SIZE,STAGE_COUNT);stageWindowStart=desiredStageWindow(centeredStage);
    if(!stageCardPool.length){stageCardPool=Array.from({length:count},()=>createStageCard());rail.replaceChildren(...stageCardPool);}
    stageCardPool.forEach((card,offset)=>bindStageCard(card,stageWindowStart+offset));
    rail.dataset.wpStageVirtualized="bounded-recycle";rail.dataset.wpStagePoolSize=String(count);rail.dataset.wpStageTotal=String(STAGE_COUNT);rail.dataset.wpStageWindowStart=String(stageWindowStart);rail.dataset.wpStageWindowEnd=String(stageWindowStart+count-1);if(!rail.dataset.wpStageRecycleCount)rail.dataset.wpStageRecycleCount="0";syncStageCards();
  }
  function moveStageWindow(targetStart){
    const rail=$("#regionRail"),target=Math.max(0,Math.min(stageWindowLimit(),targetStart));if(!stageCardPool.length)buildStageCardPool();let recycled=0;
  ...
  function setCenteredStageCard(card){if(!card)return;centeredStage=clampStage(card.dataset.stage);stageBrowseLogical=centeredStage;syncStageCards();}
  function settleStageRailSelection(){
    ...
    if(rail.dataset.wpDragDown==="1"||rail.dataset.wpStageSettling==="true")return;
  ...
  function installVirtualStageDrag(){
    ...
    document.addEventListener("pointermove",(event)=>{if(event.pointerId!==pointerId)return;const delta=event.clientX-lastX;lastX=event.clientX;if(!moved&&Math.abs(event.clientX-startX)>4){moved=true;rail.classList.add("wp-stage-dragging");}if(moved){if(event.cancelable)event.preventDefault();const rect=rail.getBoundingClientRect(),scale=rect.width?rail.clientWidth/rect.width:1;dragLogical=positionStageRail(dragLogical-delta*scale/stageRailGeometry().pitch);stageBrowseLogical=dragLogical;}event.stopImmediatePropagation();},true);
    const finish=(event)=>{if(pointerId===null||(event.pointerId!==undefined&&event.pointerId!==pointerId))return;pointerId=null;rail.dataset.wpDragDown="0";rail.classList.remove("wp-stage-dragging");if(!moved){restore();return;}if(event.cancelable)event.preventDefault();const from=dragLogical,target=clampStage(Math.round(from)),start=performance.now();centeredStage=target;if(target<Math.max(1,Number(progress.unlockedStage)||1)){selectedStage=target;progress.selectedStage=target;saveProgress();}syncStageCards();rail.dataset.wpStageSettling="true";const settle=(now)=>{const p=Math.min(1,(now-start)/340),eased=p*p*(3-2*p);stageBrowseLogical=positionStageRail(from+(target-from)*eased);if(p<1)stageSettleFrame=requestAnimationFrame(settle);else{stageSettleFrame=0;positionStageRail(target);syncStageCards();restore();}};stageSettleFrame=requestAnimationFrame(settle);suppressClick=true;setTimeout(()=>{suppressClick=false},0);moved=false;event.stopImmediatePropagation();};
  ...
  function renderStage(){
    ...
    centeredStage=Math.max(0,Math.min(STAGE_COUNT-1,centeredStage));
    $("#stageGold").textContent=`${t("diamonds")} ${diamondBalance()}`;
    $("#loadoutText").textContent=`${t("discoveries")} ${progress.discoveries.length}/12 · ${t("workshop")} ${formatLevel(1+Math.floor(progress.workshopXp/40))}`;
    stageBrowseLogical=centeredStage;buildStageCardPool();moveStageWindow(desiredStageWindow(centeredStage));syncStageCards();
  ...
  function prepareEnemyState(enemy){if(!Number.isFinite(run.blueprintLevel))run.blueprintLevel=workshopLevel();run.enemyHp=enemy.hp;run.enemyMaxHp=enemy.hp;run.enemyShield=Math.max(0,Number(enemy.shield)||0);run.enemyTurn=0;run.corrosion=0;run.sealIndex=0;run.openingApplied=false;}
  function newRun(stageIndex=selectedStage){resetCombatLifecycle();resultSettled=false;selectedStage=Math.max(0,Math.min((Number(progress.unlockedStage)||1)-1,Number(stageIndex)||0));centeredStage=selectedStage;progress.selectedStage=selectedStage;saveProgress();const workshopLevel=1+Math.floor(progress.workshopXp/40),maxArmor=36+Math.min(18,(workshopLevel-1)*2);run={stage:selectedStage,region:Math.floor(selectedStage/5),room:0,armor:maxArmor,maxArmor,gold:8,placed:[],tray:["forge-hammer","leaf-buckler","berry-potion","crystal-lens"]};prepareEnemyState(enemyAt(0,selectedStage));selectedItem=run.tray[0];selectedTrayIndex=0;rotated=false;resolving=false;setItemFeedback(itemById(selectedItem));saveRun();setTimeout(()=>sound("game.start"),0);track("game_start",{stage:selectedStage+1,region:run.region+1});showScreen("battle",true);
  ...
  function placeSelected(x,y){if(resolving||!selectedItem)return;const item=itemById(selectedItem),index=selectedTrayPosition();if(index<0)return;if(!canPlace(item,x,y)){sound("feedback.error");setFeedback(t("blocked"));return;}const previousLinks=stats().links;run.placed.push({id:item.id,x,y,rotated});run.tray.splice(index,1);progress.discoveries=[...new Set([...progress.discoveries,item.id])];selectFirstTrayItem();rotated=false;saveProgress();saveRun();sound("board.move");setFeedback(`${t("placed")}: ${itemName(item)}`);const nextLinks=stats().links;track("item_place",{stage:run.stage+1,encounter:run.room+1,item_tag:item.tag,placement_index:Math.min(12,run.placed.length),outcome:"legal"});if(nextLinks>previousLinks)track("link_activated",{stage:run.stage+1,encounter:run.room+1,link_tag:item.tag,links:Math.min(9,nextLinks)});renderBattle();}
  ...
  function sellSelected(){if(resolving||!selectedItem)return;const item=itemById(selectedItem),index=selectedTrayPosition();if(index<0)return;run.tray.splice(index,1);const value=Math.max(1,Math.floor(item.gold/2));run.gold+=value;selectFirstTrayItem();rotated=false;saveRun();sound("reward.coin");setFeedback(`${t("sold")}: ${itemName(item)} +${value} ${t("gold")}`);renderBattle();if(!selectedItem)$("#fightBtn").focus({preventScroll:true});}
  ...
  function itemComponents(){const occupied=occupiedCells(),graph=new Map(run.placed.map((placed)=>[placed,new Set()]));for(const [key,a] of occupied){const [x,y]=key.split(",").map(Number);[[1,0],[0,1]].forEach(([dx,dy])=>{const b=occupied.get(`${x+dx},${y+dy}`);if(b&&b!==a){graph.get(a).add(b);graph.get(b).add(a);}});}const seen=new Set(),components=[];run.placed.forEach((start)=>{if(seen.has(start))return;const members=[],stack=[start];seen.add(start);while(stack.length){const current=stack.pop();members.push(current);graph.get(current).forEach((next)=>{if(!seen.has(next)){seen.add(next);stack.push(next);}});}components.push(members);});return components;}
  function workshopLevel(){return 1+Math.floor(progress.workshopXp/40);}
  function activeBlueprints(){const components=itemComponents(),active=[],level=Number.isFinite(run?.blueprintLevel)?run.blueprintLevel:workshopLevel();blueprintDefinitions.filter((definition)=>level>=definition.unlockLevel).forEach((definition)=>{const members=components.find((component)=>definition.matches(component));if(members)active.push({...definition,members});});return active;}
  function blueprintSnapshot(){const active=activeBlueprints();return {active,members:new Set(active.flatMap((blueprint)=>blueprint.members)),attack:active.reduce((sum,blueprint)=>sum+blueprint.attack,0),armor:active.reduce((sum,blueprint)=>sum+blueprint.armor,0),heal:active.reduce((sum,blueprint)=>sum+blueprint.heal,0)};}
  function strongestMaterial(){const order=["forge","nature","crystal","moon"],counts=new Map(order.map((tag)=>[tag,0]));run.placed.forEach((placed)=>{const tag=itemById(placed.id).tag;counts.set(tag,(counts.get(tag)||0)+1);});const strongest=[...counts.entries()].sort((a,b)=>b[1]-a[1]||order.indexOf(a[0])-order.indexOf(b[0]))[0];return strongest?.[1]?strongest[0]:null;}
  ...
  function encounterPressureText(enemy,s){const parts=[];if(enemy.shield)parts.push(`${t("shielded")} ${enemy.shield}`);if(enemy.openingHit)parts.push(`${t("openingStrike")} ${enemy.openingHit}`);if(enemy.isolation&&s.isolated)parts.push(`${s.isolated} ${t("isolated")}`);if(enemy.corrosion)parts.push(`${t("corrosion")} ${Math.max(1,run.corrosion||enemy.corrosion)}`);if(enemy.rowHeat&&s.topRow)parts.push(`${t("heat")} ${s.topRow}`);if(s.seal)parts.push(`${t("seal")} ${t(`tag${s.seal[0].toUpperCase()}${s.seal.slice(1)}`)}`);if(enemy.phase)parts.push(t("bossPhase"));return parts.join(" · ")||localized(stages[run.stage].rule);}
  function encounterLessonText(enemy,s){const build=s.links?t("encounterBuildLinked",{links:s.links,attack:s.links*2,defense:s.links}):t("encounterBuildNoLinks"),pressure=encounterPressureText(enemy,s).replace(/[.!?。！？]+$/u,"");return t("encounterLesson",{enemy:localized(enemy.name),health:`${Math.max(0,Math.ceil(run.armor))} ${t("health")}`,build,pressure});}
  function packPlanText(enemy,s){
    if(run.enemyShield>0)return t("packPlanShield",{shield:run.enemyShield,attack:s.atk,links:s.links});
    if(enemy.openingHit)return t("packPlanOpening",{defense:s.armor,opening:enemy.openingHit});
    if(enemy.rowHeat)return t("packPlanHeat",{topRow:s.topRow,links:s.links});
    if(enemy.isolation&&s.isolated)return t("packPlanIsolation",{isolated:s.isolated});
    if(enemy.suppressTag||enemy.overload||enemy.rotatingSeal)return t("packPlanTags",{links:s.links});
    if(enemy.phase)return t("packPlanPhase",{attack:s.atk,defense:s.armor});
    if(enemy.regrow)return t("packPlanNature",{links:s.links});
    if(enemy.corrosion)return t("packPlanCorrosion",{attack:s.atk,defense:s.armor});
    return t("packPlanDefault",{attack:s.atk,defense:s.armor,links:s.links});
  }
  function syncHealthMeter(node,value,max,label,valueText){const safeMax=Math.max(1,Math.ceil(Number(max)||1)),safeValue=Math.max(0,Math.min(safeMax,Math.ceil(Number(value)||0)));node.setAttribute("role","meter");node.setAttribute("aria-valuemin","0");node.setAttribute("aria-valuenow",String(safeValue));node.setAttribute("aria-valuemax",String(safeMax));node.setAttribute("aria-label",label);node.setAttribute("aria-valuetext",valueText);node.querySelector("span").style.width=`${safeValue/safeMax*100}%`;}
  ...
  function pauseCombat(){if(!resolving||combatPaused)return;if(combatTimer){combatRemaining=Math.max(0,combatDueAt-performance.now());clearTimeout(combatTimer);combatTimer=0;}suspendCombatFx();combatPaused=true;setFeedback(t("combatPaused"));renderBattle();$("#battleBackBtn").disabled=false;}
  ...
  function fight(){if(resolving){resumeCombat();return;}if(!run.placed.length){setFeedback(t("needGear"));return;}const enemy=enemyAt(run.room,run.stage),s=stats();if(enemy.openingHit&&!run.openingApplied){run.openingApplied=true;const opening=Math.max(1,enemy.openingHit-Math.floor(s.armor/2));run.armor-=opening;setFeedback(`${t("openingStrike")} -${opening}`);if(run.armor<=0){showResult(false);return;}}track("encounter_start",{stage:run.stage+1,encounter:run.room+1,placed_count:Math.min(12,run.placed.length),links:Math.min(9,s.links)});combatPaused=false;resolving=true;setFeedback(t("autoFighting"));renderBattle();autoCombatRound();}
  function roomVictory(){combatPaused=false;resolving=false;const lesson=encounterLessonText(enemyAt(run.room,run.stage),stats());renderBattle();track("encounter_result",{stage:run.stage+1,encounter:run.room+1,outcome:"win",result_reason:"enemy_defeated"});if(run.room===ROOMS_PER_STAGE-1){showResult(true);return;}showLoot(()=>{run.room+=1;prepareEnemyState(enemyAt(run.room,run.stage));resolving=false;saveRun();setFeedback(t("repack"));renderBattle();if(run.room===stages[run.stage].merchantAfter)showMerchant();},lesson);}
  ...
  function showModal(title,text,art=""){
    ...
    (__wpNotifyMeasurement(), modal.classList.remove("is-result"));
    $("#modalTitle").textContent=title;
    $("#modalText").textContent=text;
    $("#modalArt").src=art;
    (__wpNotifyMeasurement(), $("#modalArt").hidden=!art);
  ...
  function closeModal(){const modal=$("#modal");(__wpNotifyMeasurement(), modal.hidden=true);(__wpNotifyMeasurement(), modal.classList.remove("is-result"));modalEscapeHandler=null;modalCoveredRegions().forEach((region)=>{region.inert=false;region.removeAttribute("aria-hidden");});const previous=modalReturnFocus;modalReturnFocus=null;const canRestore=previous?.isConnected&&previous.matches?.('button:not(:disabled),a[href],select:not(:disabled),[tabindex]:not([tabindex="-1"])')&&previous.getClientRects().length;if(canRestore){previous.focus({preventScroll:true});return;}const fallback=screens.battle.hidden?$(".region-card.is-selected:not(:disabled)"):$("#fightBtn:not(:disabled)");fallback?.focus({preventScroll:true});}
  function choiceButton(label,handler,item){const button=document.createElement("button");button.type="button";button.innerHTML=item?`<img src="${itemAsset(item)}" alt="">${label}`:label;button.addEventListener("click",(event)=>{noteInput(event);handler(event);});return button;}
  function lootComparison(item){
    const current=stats(),occupied=occupiedCells();
    let fits=false,joins=false;
    [false,true].forEach((isRotated)=>{
      const shape=shapeFor(item,isRotated);
      for(let y=0;y<PACK_ROWS;y++)for(let x=0;x<PACK_COLS;x++){
        const legal=shape.every(([dx,dy])=>x+dx>=0&&x+dx<PACK_COLS&&y+dy>=0&&y+dy<PACK_ROWS&&!occupied.has(`${x+dx},${y+dy}`));
        if(!legal)continue;
        fits=true;
        if(shape.some(([dx,dy])=>[[1,0],[-1,0],[0,1],[0,-1]].some(([nx,ny])=>{const placed=occupied.get(`${x+dx+nx},${y+dy+ny}`);return placed&&itemById(placed.id).tag===item.tag;})))joins=true;
      }
    });
    const dimensions=shapeDimensions(item,false),tagKey=`tag${item.tag[0].toUpperCase()}${item.tag.slice(1)}`,tag=t(tagKey);
    const space=t(fits?"lootSpaceFit":"lootSpaceNoFit",dimensions);
    const link=activeSeal(enemyAt(run.room,run.stage))===item.tag?t("lootLinkSealed",{tag}):joins?t("lootLinkJoin",{tag}):t("lootLinkNone",{tag});
    return [t("lootCurrent",{attack:current.atk,defense:current.armor,healing:current.heal,links:current.links}),t("lootAdds",{attack:item.atk,defense:item.armor,healing:item.heal}),space,link].join(" ");
  }
  function itemChoiceButton(item,handler,prefix="",withComparison=false){
    ...
    if(withComparison){const comparison=document.createElement("small");comparison.className="item-comparison";comparison.textContent=lootComparison(item);copy.append(comparison);}
    button.append(image,copy);
    button.classList.add("item-decision");
    button.setAttribute("aria-label",`${name.textContent}. ${details.textContent}${withComparison?`. ${lootComparison(item)}`:""}`);
    return button;
  }
  ...
  function showLoot(done,lesson=""){const hasRoom=run.tray.length+run.placed.length<12,reward=`${t("reward")}: +${4+run.room*2} ${t("gold")}`,message=[reward,lesson,hasRoom?"":t("fullLoot")].filter(Boolean).join(" · ");const choices=showModal(t("chooseLoot"),message);randomItems(3).forEach((item)=>{const choice=itemChoiceButton(item,()=>{if(!addToTray(item))return;track("loot_choice",{stage:run.stage+1,encounter:run.room+1,choice:"take",item_tag:item.tag});closeModal();done();},"",true);choice.disabled=!hasRoom;choices.append(choice);});choices.append(choiceButton(t("leaveLoot"),()=>{track("continue_without_loot",{stage:run.stage+1,encounter:run.room+1,choice:"skip"});closeModal();done();}));focusModalChoice(null,true);}
  function showMerchant(){
    ...
    const render=()=>{
      ...
      const refresh=choiceButton(refreshUsed?t("refreshUsed"):refreshPending?t("refreshConfirm",{before:balance,after:balance-3}):t("refresh"),()=>{
        ...
        clearConfirmation();
        if(spendDiamonds(3)){refreshUsed=true;sound("reward.upgrade");track("diamond_spend",{sink:"merchant_refresh",amount:3});stock=randomItems(3);render();}
      });
  ...
  function showResult(won){if(!run||resultSettled)return;resultSettled=true;clearTimeout(combatTimer);resolving=false;const finishedStage=run.stage,xp=won?35+Math.floor(finishedStage/5)*5:8+run.room*4,previousUnlocked=progress.unlockedStage;progress.workshopXp+=xp;if(won){progress.completedStages=[...new Set([...progress.completedStages,finishedStage+1])].sort((a,b)=>a-b);progress.unlockedStage=Math.max(previousUnlocked,Math.min(STAGE_COUNT,finishedStage+2));selectedStage=Math.min(STAGE_COUNT-1,finishedStage+1);centeredStage=selectedStage;progress.selectedStage=selectedStage;}saveProgress();sound(won ? "result.win" : "result.lose");track("encounter_result",{stage:finishedStage+1,encounter:run.room+1,outcome:won?"win":"loss",result_reason:won?"stage_cleared":"player_defeated"});track("result",{stage:finishedStage+1,encounter:run.room+1,outcome:won?"win":"loss",result_reason:won?"stage_cleared":"player_defeated"});track("game_end",{result:won?"win":"loss",room:run.room+1,stage:finishedStage+1,region:run.region+1});const resultParts=[`+${xp} XP`,`${t("workshop")} ${formatLevel(1+Math.floor(progress.workshopXp/40))} · ${progress.workshopXp} XP`,`${t("discoveries")} ${progress.discoveries.length}/12`];if(won&&finishedStage===STAGE_COUNT-1)resultParts.unshift(t("allStagesCleared"));else if(won&&progress.unlockedStage>previousUnlocked)resultParts.push(formatStageUnlocked(finishedStage+2));let decisionClaimed=false;const claim=(action)=>()=>{if(decisionClaimed)return;decisionClaimed=true;action();};const [stagesLabel,nextLabel,replayLabel]=activeResultLabels(),choices=showModal(won?t("stageCleared"):t("defeat"),resultParts.join(" · "),won?"../../assets/animal-gearpack-expedition-icons/victory-sparkle.webp":"../../assets/animal-gearpack-expedition-rux.webp");(__wpNotifyMeasurement(), $("#modal").classList.add("is-result"));const stagesChoice=choiceButton(stagesLabel,claim(()=>{closeModal();run=null;saveRun();showScreen("stage",true);})),nextChoice=choiceButton(nextLabel,claim(()=>{track("next_stage",{stage:finishedStage+2,from_stage:finishedStage+1,outcome:"result_next"});closeModal();run=null;saveRun();newRun(finishedStage+1);})),replayChoice=choiceButton(replayLabel,claim(()=>{track("retry",{stage:finishedStage+1,outcome:"result_retry"});closeModal();run=null;saveRun();__wpReplayStart(() => newRun(finishedStage));}));stagesChoice.dataset.resultAction="stages";nextChoice.dataset.resultAction="next";replayChoice.dataset.resultAction="replay";nextChoice.disabled=!won||finishedStage>=STAGE_COUNT-1;choices.append(stagesChoice,nextChoice,replayChoice);const primaryChoice=nextChoice.disabled?(won?stagesChoice:replayChoice):nextChoice;primaryChoice.classList.add("primary");focusModalChoice(primaryChoice);
  ...
  function enhancePackKeyboard(){
    ...
    if(restorePackFocus){restorePackFocus=false;requestAnimationFrame(()=>{const active=document.activeElement;if(active===document.body||active?.closest?.("#packGrid"))focusPackCell();});}
  }
  ...
  function resultBuildRecap(){const s=stats(),material=strongestMaterial(),names=s.blueprints.map((blueprint)=>t(blueprint.nameKey)).join(", ")||t("blueprintNone"),materialLabel=material?t(`tag${material[0].toUpperCase()}${material.slice(1)}`):"—";return [t("blueprintBuildRecap",{blueprints:names,material:materialLabel,links:s.links}),t("blueprintPressure",{rule:mechanicText(enemyAt(run.room,run.stage),s)})];}
  function setBlueprintPackForTest(id){if(!run)newRun(0);progress.workshopXp=Math.max(progress.workshopXp,80);const fixtures={forge:[{id:"forge-hammer",x:0,y:0},{id:"gear-boots",x:1,y:1},{id:"repair-kit",x:3,y:1}],nature:[{id:"leaf-buckler",x:0,y:0},{id:"berry-potion",x:2,y:0},{id:"moss-armor",x:3,y:0}],caravan:[{id:"forge-hammer",x:0,y:0},{id:"leaf-buckler",x:1,y:1},{id:"crystal-lens",x:3,y:1}]};run.placed=(fixtures[id]||fixtures.forge).map((placed)=>({...placed,rotated:false}));run.tray=[];selectedItem=null;selectedTrayIndex=-1;rotated=false;resolving=false;renderBattle();const s=stats();return {active:s.blueprints.map((blueprint)=>t(blueprint.nameKey)),attackBonus:s.blueprintBonus.attack,defenseBonus:s.blueprintBonus.armor,healBonus:s.blueprintBonus.heal,markedCells:document.querySelectorAll(".pack-cell.is-blueprint").length};}
  ...
  $("#soundBtn").addEventListener("click",()=>{window.WeightPlayAudio?.unlock?.();window.WeightPlayAudio?.setMuted?.(!window.WeightPlayAudio?.isMuted?.());syncSoundButton();});
  $("#localeSelect").addEventListener("change",(event)=>applyLocale(event.target.value));$("#startBtn").addEventListener("click",(event)=>{noteInput(event);showScreen("stage",true);});$("#stageBackBtn").addEventListener("click",(event)=>{noteInput(event);track("main_return",{from_screen:"stage"});track("return_session",{from_screen:"stage"});showScreen("main",true);});$("#battleBackBtn").addEventListener("click",()=>{if(resolving&&!combatPaused)return;if(combatPaused){resetCombatLifecycle();resolving=false;}saveRun();showScreen("stage",true)});$("#pauseBtn").addEventListener("keydown",(event)=>{if(event.repeat&&(event.key==="Enter"||event.key===" "))event.preventDefault();});$("#pauseBtn").addEventListener("click",(event)=>{noteInput(event);if(!resolving){setFeedback(localized(stages[run?.stage??selectedStage].rule));return;}if(combatPaused)resumeCombat();else pauseCombat();});$("#rotateBtn").addEventListener("click",(event)=>{noteInput(event);if(resolving||!selectedItem)return;rotated=!rotated;setItemFeedback(itemById(selectedItem));renderBattle();});$("#sellBtn").addEventListener("click",(event)=>{noteInput(event);sellSelected();});$("#fightBtn").addEventListener("click",(event)=>{noteInput(event);fight();});
