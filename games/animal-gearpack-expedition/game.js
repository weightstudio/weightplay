...
function showScreen(name,focusTarget=false){if(name!=="stage"&&railSettleTimer){clearTimeout(railSettleTimer);railSettleTimer=0;}Object.entries(screens).forEach(([key,node])=>node.hidden=key!==name);document.body.dataset.screen=name;document.body.classList.toggle("is-game-playing",name==="battle");if(name==="main")renderMain();if(name==="stage")renderStage();if(name==="battle")renderBattle();if(focusTarget)requestAnimationFrame(()=>{const target=name==="main"?$("#startBtn"):name==="stage"?$(".region-card.is-selected:not(:disabled)"):$(".pack-cell[tabindex='0']");target?.focus({preventScroll:true});});}
  ...
  function showModal(title,text,art=""){const modal=$("#modal");if(modal.hidden)modalReturnFocus=document.activeElement;$("#modalTitle").textContent=title;$("#modalText").textContent=text;$("#modalArt").src=art;$("#modalArt").hidden=!art;$("#modalChoices").replaceChildren();modalCoveredRegions().forEach((region)=>{region.inert=true;region.setAttribute("aria-hidden","true");});modal.hidden=false;return $("#modalChoices");}
  ...
  function showResult(won){if(!run||resultSettled)return;resultSettled=true;clearTimeout(combatTimer);resolving=false;const finishedStage=run.stage,xp=won?35+Math.floor(finishedStage/5)*5:8+run.room*4,previousUnlocked=progress.unlockedStage;progress.workshopXp+=xp;if(won){progress.completedStages=[...new Set([...progress.completedStages,finishedStage+1])].sort((a,b)=>a-b);progress.unlockedStage=Math.max(previousUnlocked,Math.min(STAGE_COUNT,finishedStage+2));selectedStage=Math.min(STAGE_COUNT-1,finishedStage+1);centeredStage=selectedStage;progress.selectedStage=selectedStage;}saveProgress();sound(won?"win":"wrong");track("game_end",{result:won?"win":"loss",room:run.room+1,stage:finishedStage+1,region:run.region+1});const resultParts=[`+${xp} XP`,`${t("workshop")} ${formatLevel(1+Math.floor(progress.workshopXp/40))} · ${progress.workshopXp} XP`,`${t("discoveries")} ${progress.discoveries.length}/12`];if(won&&finishedStage===STAGE_COUNT-1)resultParts.unshift(t("allStagesCleared"));else if(won&&progress.unlockedStage>previousUnlocked)resultParts.push(formatStageUnlocked(finishedStage+2));const choices=showModal(won?t("stageCleared"):t("defeat"),resultParts.join(" · "),won?"../../assets/animal-gearpack-expedition-icons/victory-sparkle.webp":"../../assets/animal-gearpack-expedition-rux.webp"),stagesChoice=choiceButton(t("backToStages"),()=>{closeModal();run=null;saveRun();showScreen("stage",true);}),retryChoice=choiceButton(t("retryStage"),()=>{closeModal();run=null;saveRun();newRun(finishedStage);});stagesChoice.dataset.resultAction="stages";retryChoice.dataset.resultAction="retry";choices.append(stagesChoice,retryChoice);let primaryChoice=won&&finishedStage===STAGE_COUNT-1?stagesChoice:retryChoice;if(won&&finishedStage<STAGE_COUNT-1){const nextChoice=choiceButton(t("nextExpedition"),()=>{closeModal();run=null;saveRun();newRun(finishedStage+1);});nextChoice.dataset.resultAction="next";choices.append(nextChoice);primaryChoice=nextChoice;}primaryChoice.classList.add("primary");focusModalChoice(primaryChoice);}
