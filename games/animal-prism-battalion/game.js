(function(){
  ...
  function distinctLaneCount(mask=run?.laneMask||0){let count=0;for(let value=mask;value;value>>=1)count+=value&1;return count}
  ...
  function buyUpgrade(id){const level=save.upgrades[id];if(level>=5)return;const cost=upgradeCost(level),name=t(upgradeData[id].name);if(save.shards<cost){$("labFeedback").textContent=t("needShardsDetail",{name,cost,balance:save.shards});return}save.shards-=cost;save.upgrades[id]+=1;persist();renderLab(id);$("labFeedback").textContent=t("upgradePurchasedDetail",{name,level:save.upgrades[id],balance:save.shards});window.WeightPlayAudio?.play?.("board.move")}
  ...
  function showPriorityReason(){if(!run||run.laneSwitchCount>0)return;const priority=priorityThreat();if(!priority)return;const reason=t("priorityReason",{lane:priority.lane+1});if((!run.priorityReasonText&&!$("feedback").textContent)||$("feedback").textContent===run.priorityReasonText){if($("feedback").textContent!==reason)$("feedback").textContent=reason;run.priorityReasonText=reason}}
  function announceOverdriveReady(){if(!run||run.charge<100||run.readyCueShown)return;const priority=priorityThreat();if(!priority)return;const vars={seconds:"3.6",lane:priority.lane+1,count:priority.count},cue=t("overdriveReadyCue",vars);$("feedback").textContent=t("overdriveReadyCueShort",vars);$("feedback").classList.add("ready-cue");$("overdrive").setAttribute("aria-label",cue);run.readyCueShown=true;run.readyTargetLane=priority.lane;run.readyTargetCount=priority.count;run.lastReadyUsed=false;run.overdriveReadyCount+=1;trackFunnel("overdrive_ready",{lane:priority.lane+1,count:priority.count,seconds:3.6})}
  function updateHud(force=false){if(!run)return;const priority=priorityThreat(),firstSwitchCue=Boolean(priority&&run.wave===1&&priority.lane!==run.lane),key=`${Math.ceil(run.time)}|${Math.ceil(run.core)}|${run.wave}|${run.resolved}|${Math.floor(run.charge)}|${Math.ceil(run.overdrive*10)}|${run.attack}|${run.weapon}|${run.lane}|${priority?.lane}|${priority?.count}|${firstSwitchCue}|${locale}`;if(!force&&key===lastHud)return;lastHud=key;const weaponMark=run.weapon==="double"?"×2":run.weapon==="rapid"?"≫":"•";$("missionLabel").textContent=`${chapterName(run.stage)} · ${run.stage.n}/30`;$("fortressValue").textContent=`⚔ ${Math.round(run.attack)} ${weaponMark} · ${t("waveLabel",{wave:run.wave,total:run.totalWaves})}`;$("coreValue").textContent=`${Math.max(0,Math.ceil(run.core))}/${run.maxCore}`;$("timeValue").textContent=Math.max(0,Math.ceil(run.time));$("objective").textContent=priority?t("priorityLane",{lane:priority.lane+1,count:priority.count}):t("laneObjective");$("objective").dataset.priorityLane=priority?String(priority.lane+1):"";$("objective").classList.toggle("first-switch-cue",firstSwitchCue);showPriorityReason();const ready=run.overdrive<=0&&run.charge>=100&&priority;if(ready){run.readyTargetLane=priority.lane;run.readyTargetCount=priority.count;$("overdrive").querySelector("span").textContent=t("overdriveReadyTarget",{lane:priority.lane+1});$("overdrive").setAttribute("aria-label",t("overdriveReadyCue",{seconds:"3.6",lane:priority.lane+1,count:priority.count}))}else if(run.overdrive>0){$("overdrive").querySelector("span").textContent=t("overdriveActive");$("overdrive").setAttribute("aria-label",t("overdriveActive"))}else{$("overdrive").querySelector("span").textContent=t("overdrive");$("overdrive").removeAttribute("aria-label")}$("overdriveValue").textContent=run.overdrive>0?t("secondsShort",{seconds:run.overdrive.toFixed(1)}):`${Math.floor(run.charge)}%`;$("overdrive").classList.toggle("ready",run.charge>=100);$("overdrive").classList.toggle("active",run.overdrive>0)}
  ...
  function announceMidwavePayoff(type,vars){if(!run||run.feedbackLock>0||run.midwavePayoffCooldown>0)return;run.midwavePayoffCooldown=.9;$("feedback").classList.remove("ready-cue");$("feedback").textContent=t(type==="core"?"laneCoreSecured":"laneThreatCleared",vars)}
  function updatePeak(){if(run)run.peak=Math.max(run.peak,Math.round(run.attack))}
  function recordCoreDamage(loss,lane,source){if(!run||loss<=0)return;run.core-=loss;run.coreHits+=loss;run.laneDamage[lane]+=loss;trackFunnel("core_damage",{lane:lane+1,amount:loss,source})}
  function announceImminentBreach(lane){if(!run||run.finished||run.imminentBreachShown||run.core<=0||run.core>run.maxCore*.5)return;run.imminentBreachShown=true;$('feedback').classList.remove('ready-cue');$('feedback').textContent=t('imminentBreach',{core:Math.max(0,Math.ceil(run.core)),max:run.maxCore,lane:lane+1})}
  ...
  function resolveEncounter(entry){
    ...
    updatePeak();announceImminentBreach(entry.lane)
  }
  function update(dt){
  ...
  function suspendForLifecycle(){held.clear();clearArenaPointer();if(!run||run.finished||run.paused||currentScreen!=="battle")return;(__wpNotifyMeasurement(), lifecyclePaused=true);(__wpNotifyMeasurement(), run.paused=true);stopLoop()}
  function resumeFromLifecycle(){held.clear();if(!lifecyclePaused||document.hidden)return;(__wpNotifyMeasurement(), lifecyclePaused=false);if(!run||run.finished||currentScreen!=="battle"||activeModal())return;(__wpNotifyMeasurement(), run.paused=false);ensureVisibleTick();resumeLoop()}
  ...
  function activateOverdrive(){if(!run||run.paused||run.finished)return false;if(run.charge<100){$("feedback").textContent=t("overdriveNeed",{charge:Math.floor(run.charge)});return false}run.overdriveActivationCount+=1;run.overdriveActivationLane=run.lane;run.overdriveActivationWave=run.wave;run.lastReadyUsed=true;trackFunnel("overdrive_activate",{lane:run.lane+1,target_lane:(run.readyTargetLane??run.lane)+1,seconds:3.6});run.charge=0;run.overdrive=3.6;run.feedbackLock=.7;run.readyCueShown=false;$("feedback").classList.remove("ready-cue");$("overdrive").setAttribute("aria-label",t("overdriveActive"));$("feedback").textContent=t("overdriveUsed");updateHud(true);addBurst(run.aimX,.86,"#ffe273",24);window.WeightPlayAudio?.play?.("board.move");return true}
  ...
  function openModal(modal,focusTarget){clearArenaPointer();modalReturnFocus=document.activeElement;if(run)(__wpNotifyMeasurement(), run.paused=true);stopLoop();(__wpNotifyMeasurement(), modal.hidden=false);$("battleLive").inert=true;requestAnimationFrame(()=>(focusTarget||modalButtons(modal)[0])?.focus())}
  function closeModal(modal,restore=true){(__wpNotifyMeasurement(), modal.hidden=true);$("battleLive").inert=false;if(run&&!run.finished&&!lifecyclePaused&&!document.hidden){(__wpNotifyMeasurement(), run.paused=false);ensureVisibleTick();resumeLoop()}if(restore)(modalReturnFocus?.isConnected?modalReturnFocus:$("battleBack"))?.focus();modalReturnFocus=null}
  ...
  window.__animalPrismBattalionTest={
    ...
    snapshot(){return{locale,screen:currentScreen,windowFocused,lifecyclePaused,rafActive:Boolean(raf),save:JSON.parse(JSON.stringify(save)),run:run&&{stageIndex:run.stageIndex,stageTime:run.stage.time,time:run.time,core:run.core,maxCore:run.maxCore,wave:run.wave,totalWaves:run.totalWaves,bossDefeated:run.bossDefeated,lane:run.lane,laneMask:run.laneMask,laneSwitchCount:run.laneSwitchCount,distinctLaneCount:distinctLaneCount(),aimX:run.aimX,attack:run.attack,weapon:run.weapon,charge:run.charge,overdrive:run.overdrive,imminentBreachShown:run.imminentBreachShown,readyCueShown:run.readyCueShown,readyTargetLane:run.readyTargetLane,readyTargetCount:run.readyTargetCount,lastReadyUsed:run.lastReadyUsed,overdriveReadyCount:run.overdriveReadyCount,overdriveActivationCount:run.overdriveActivationCount,overdriveActivationLane:run.overdriveActivationLane,overdriveActivationWave:run.overdriveActivationWave,peak:run.peak,coreHits:run.coreHits,laneDamage:[...run.laneDamage],resolved:run.resolved,units:run.units.map((unit)=>({...unit})),enemies:run.enemies.map((enemy)=>({...enemy})),encounters:run.encounters.map((entry)=>({...entry})),gates:run.gates.map((entry)=>({...entry})),paused:run.paused,finished:run.finished,won:run.won}}}
  };
})();
