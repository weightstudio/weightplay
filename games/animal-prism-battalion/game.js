(function(){
  ...
  function startBattle(index){
    ...
    run={stageIndex,stage,time:stage.time,core:maxCore,maxCore,wave:1,totalWaves:stage.waves,bossDefeated:false,lane:1,aimX:laneCenters[1],attack:6+save.upgrades.power*2,units:[],encounters,gates:encounters,enemies:encounters.filter(entry=>entry.kind==="enemy"),particles:[],texts:[],fireClock:0,charge:0,overdrive:0,feedbackLock:0,peak:6+save.upgrades.power*2,coreHits:0,resolved:0,paused:false,finished:false,lastGateMessage:""};
  ...
  function resolveEncounter(entry){
    ...
    else if(entry.kind==="enemy"){const loss=same?Math.max(2,Math.ceil(entry.hp)):Math.max(3,Math.ceil(entry.hp*.42));run.core-=loss;run.coreHits+=loss;$("feedback").textContent=t(same?"enemyCollision":"enemyLeak",{count:loss});addBurst(entry.x,.84,"#ff637e",entry.boss?34:18)}
    updatePeak()
  }
  function update(dt){
    ...
    run.units=run.units.filter((unit)=>!unit.dead&&unit.y>0);run.enemies=run.encounters.filter(entry=>entry.kind==="enemy"&&!entry.resolved);const firstActive=run.encounters.filter(entry=>!entry.resolved).reduce((min,entry)=>Math.min(min,entry.row),run.totalWaves);run.wave=Math.min(run.totalWaves,firstActive+1);updatePeak();
  ...
  function draw(){
    ...
    const waveRatio=Math.max(0,Math.min(1,run.resolved/run.encounters.length));rounded(w*.19,h*.025,w*.62,h*.038,12);ctx.fillStyle="#071326dd";ctx.fill();rounded(w*.195,h*.031,w*.61*waveRatio,h*.026,10);ctx.fillStyle=run.wave>=run.totalWaves&&run.stage.boss?"#ff647e":"#64efd5";ctx.fill();if(run.wave>=run.totalWaves&&run.stage.boss){ctx.fillStyle="#fff";ctx.font=`900 ${Math.max(11,d*.019)}px sans-serif`;ctx.textAlign="center";ctx.textBaseline="top";ctx.fillText(t("bossWave"),w*.5,h*.065)}
