(function(){
  ...
  function resolveEncounter(entry){
    ...
    else if(entry.kind==="enemy"){const loss=entry.boss?run.core:same?Math.max(2,Math.ceil(entry.hp)):Math.max(3,Math.ceil(entry.hp*.42));run.core-=loss;run.coreHits+=loss;$("feedback").textContent=t(same?"enemyCollision":"enemyLeak",{count:loss});addBurst(entry.x,.84,"#ff637e",entry.boss?34:18)}
    updatePeak()
  }
  function update(dt){
    ...
    for(const entry of run.encounters)if(!entry.resolved){if(entry.collecting){entry.y+=(1.15+(.84-entry.y)*1.8)*dt;entry.x+=(run.aimX-entry.x)*Math.min(1,dt*8)}else{entry.y+=run.stage.layout.speed*dt*(entry.boss?.46:1);entry.x=laneCenters[entry.lane]}}
