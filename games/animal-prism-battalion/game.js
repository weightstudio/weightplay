(function(){
  ...
  function resolveEncounter(entry){
    ...
    else if(entry.kind==="shield"&&(same||entry.collecting)){if(entry.shield<=0){run.attack=Math.min(999,run.attack+entry.reward);run.charge=Math.min(100,run.charge+entry.reward);run.weapon=run.attack>=42?"rapid":run.attack>=20?"double":"single";$("feedback").textContent=t("pickupPower",{count:entry.reward});addBurst(entry.x,.84,"#6ceaff",20)}else{const loss=Math.ceil(entry.shield);run.core-=loss;run.coreHits+=loss;run.laneDamage[entry.lane]+=loss;$("feedback").textContent=t("enemyCollision",{count:loss});addBurst(entry.x,.84,"#ff637e",18)}}
    else if(entry.kind==="bomb"){const loss=Math.ceil(entry.hp);if(loss>0){run.core-=loss;run.coreHits+=loss;run.laneDamage[entry.lane]+=loss;$("feedback").textContent=t("bombDamage",{count:loss});addBurst(entry.x,.84,"#ff9e45",24)}}
    else if(entry.kind==="enemy"){const loss=entry.boss?run.core:same?Math.max(2,Math.ceil(entry.hp)):Math.max(3,Math.ceil(entry.hp*.42));run.core-=loss;run.coreHits+=loss;run.laneDamage[entry.lane]+=loss;$("feedback").textContent=t(same?"enemyCollision":"enemyLeak",{count:loss});addBurst(entry.x,.84,"#ff637e",entry.boss?34:18)}
    updatePeak()
  }
  function update(dt){
