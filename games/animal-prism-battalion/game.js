(function(){
  ...
  function startBattle(index){
    ...
    $("leave").hidden=true;$("tutorial").hidden=true;$("result").hidden=true;$("battleLive").hidden=false;$("battleLive").inert=false;showScreen("battle");canvas.focus({preventScroll:true});updateHud(true);$("feedback").textContent="";lastTime=performance.now();stopLoop();lifecyclePaused=!windowFocused||document.hidden;run.paused=lifecyclePaused;if(!lifecyclePaused)raf=requestAnimationFrame(frame);window.WonderSound?.play?.("start");if(!save.tutorialSeen)requestAnimationFrame(()=>openTutorial())
  }
