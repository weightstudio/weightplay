(function () {
  ...
  function playCell(index){clearInvalid();if(starters.has(index))return;if(mode==="mark"){pushHistory();if(marks.has(index)){marks.delete(index);setStatus("unmarked")}else{if(cats.has(index))cats.delete(index);marks.add(index);setStatus("marked")}moves+=1;hintIndex=-1;beep("board.move");renderBoard();return}if(cats.has(index)){pushHistory();cats.delete(index);moves+=1;hintIndex=-1;setStatus("catRemoved");beep("board.move");renderBoard();return}const reason=conflict(index);if(reason){invalidIndex=index;invalidReason=reason;setStatus(reason,true);beep("feedback.error");const cell=$("#board").querySelector(`[data-index="${index}"]`);cell?.classList.add("invalid");cell?.setAttribute("aria-invalid","true");cell?.setAttribute("aria-label",cellLabel(index));return}pushHistory();marks.delete(index);cats.add(index);moves+=1;hintIndex=-1;setStatus("catPlaced");beep("board.move");renderBoard();checkComplete()}
  ...
  function undo(){clearInvalid();if(!history.length){setStatus("nothingUndo",true);beep("feedback.error");return}restore(history.pop());setStatus("restored");beep("board.move")}
  function restart(){
    const __wpPreviousRound = __wpMeasurement.roundKey;
clearInvalid();cats=new Set(starters);marks=new Set();history=[];moves=0;hintIndex=-1;renderBoard();setStatus("restarted");beep("board.move")
    __wpMeasurement.roundKey = {}; __wpMeasurement.restart = false; __wpMeasurement.started = true; __wpMeasurement.ended = false; __wpMeasurement.outcome = "complete"; __wpMeasurement.screen = "battle"; __wpNotifyMeasurement();
    if (__wpMeasurement.roundKey !== __wpPreviousRound) { __wpMeasurement.restart = true; __wpNotifyMeasurement(); }
}
  function hint(){clearInvalid();const candidate=level.solution.find(index=>!cats.has(index));if(candidate===undefined){setStatus("hintDone");return}hintIndex=candidate;renderBoard();setStatus("hintReady");beep("feedback.hint");$("#board").querySelector(`[data-index="${candidate}"]`)?.focus({preventScroll:true})}
  ...
  function claimResult(action){if(resultClaimed||!$("#resultDialog").open)return;resultClaimed=true;["#resultStages","#resultNext","#resultReplay"].forEach(selector=>$(selector).disabled=true);(__wpNotifyMeasurement(), $("#resultDialog").close());action()}
  function syncSoundUi(){if(typeof window.WeightPlayAudio?.isMuted==="function")sound=window.WeightPlayAudio.setEnabled(!window.WeightPlayAudio.isMuted());const audio=$("#audio");if(!audio)return;audio.querySelector("small").textContent=t(sound?"audioOn":"audioOff");audio.setAttribute("aria-pressed",String(sound));audio.dataset.state=sound?"on":"off"}
  function setSoundEnabled(enabled){sound=window.WeightPlayAudio.setEnabled(Boolean(enabled));write(soundKey,sound?"on":"off");if(typeof window.WeightPlayAudio?.setMuted==="function")window.WeightPlayAudio.setMuted(!sound);syncSoundUi();if(sound)beep("board.move")}
  ...
  window.addEventListener("weightplay:audio-volume-change",syncSoundUi);
  $("#battleUtility").onclick=()=>$("#audio").click();
  $("#start").onclick=()=>show("stage");
  $("#stageBack").onclick=()=>show("main");
  $("#battleBack").onclick=()=>(__wpNotifyMeasurement(), $("#leaveDialog").showModal());
  $("#leaveContinue").onclick=()=>(__wpNotifyMeasurement(), $("#leaveDialog").close());
  $("#leaveStages").onclick=()=>show("stage");
  $("#toolCat").onclick=()=>setMode("cat");
  $("#toolMark").onclick=()=>setMode("mark");
  $("#undo").onclick=undo;
  $("#hint").onclick=hint;
  $("#restart").onclick=function (...args) { return __wpReplayStart(() => restart.apply(this, args)); };
  $("#resultStages").onclick=()=>claimResult(()=>{selected=Math.min(29,selected+1);show("stage")});
  $("#resultReplay").onclick=()=>claimResult(()=>__wpReplayStart(() => startLevel(selected)));
