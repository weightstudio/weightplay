(function () {
  ...
  function applyLocale(){document.documentElement.lang=locale;document.documentElement.dir=locale==="ar"?"rtl":"ltr";document.title=`${t("title")} | WeightPlay`;document.querySelectorAll("[data-t]").forEach(node=>node.textContent=t(node.dataset.t));document.querySelectorAll("[data-t-aria]").forEach(node=>node.setAttribute("aria-label",t(node.dataset.tAria)));document.querySelectorAll("[data-t-alt]").forEach(node=>node.setAttribute("alt",t(node.dataset.tAlt)));$("#locale").value=locale;$("#audio small").textContent=t(sound?"audioOn":"audioOff");$("#mainProgress").textContent=t("progress",{done:Math.min(30,unlocked-1)});if(document.body.dataset.screen==="stage")renderStages();if(document.body.dataset.screen==="battle"&&level){$("#battleChapter").textContent=`${t("chapter",{n:level.chapter+1})} · ${t("chapters")[level.chapter]}`;$("#battleStage").textContent=t("stage",{n:selected+1});$("#ruleChapter").textContent=t("chapters")[level.chapter];$("#ruleText").textContent=t("chapterRules")[level.chapter];renderBoard();setStatus(mode==="cat"?"statusCatMode":"statusMarkMode")}}
