(() => {
  ...
  let stageIndex=0,attempts=1,visited=[],visitedSet=new Set(),drawing=false,activePointer=null,failed=false,startTime=0,lastPoint=null,hintUsed=false,leaveStartedAt=0,resultRevealTimer=null,resultSettlementPending=false;
  ...
  function setScreen(name){if(name!=="battle"){cancelPendingResult();closeResultOwnership();}document.body.dataset.screen=name;dom.main.hidden=name!=="main";dom.guide.hidden=name!=="main";dom.stage.hidden=name!=="stage";dom.battle.hidden=name!=="battle";}
  ...
  function resetAttempt(recordRestart=false){cancelPendingResult();if(recordRestart)attempts+=1;cancelDrawing();visited=[];visitedSet.clear();failed=false;hintUsed=false;renderBoard();updateHud();dom.feedback.textContent=text("ready");dom.assist.textContent="";closeResultOwnership();}
  ...
  function openBattleLeave(){if(document.body.dataset.screen!=="battle"||!dom.result.hidden||!dom.leave.hidden)return;const settlementWasPending=resultSettlementPending||resultRevealTimer!==null;cancelPendingResult();resultSettlementPending=settlementWasPending;cancelDrawing();leaveStartedAt=performance.now();dom.leaveTitle.textContent=text("leaveTitle");dom.leaveText.textContent=text("leaveText");dom.continuePuzzle.textContent=text("continuePuzzle");dom.returnStages.textContent=text("returnStages");setBattleDecisionCovered(true);dom.leave.hidden=false;requestAnimationFrame(()=>dom.continuePuzzle.focus());}
  function continuePuzzle(){if(dom.leave.hidden)return;if(startTime&&leaveStartedAt)startTime+=performance.now()-leaveStartedAt;leaveStartedAt=0;dom.leave.hidden=true;setBattleDecisionCovered(false);if(resultSettlementPending&&document.body.dataset.screen==="battle"){resultSettlementPending=false;openResultOwnership();return;}requestAnimationFrame(()=>dom.battleBack.focus());}
  function returnToStages(){if(dom.leave.hidden)return;cancelPendingResult();leaveStartedAt=0;dom.leave.hidden=true;setBattleDecisionCovered(false);showStage(stageIndex);requestAnimationFrame(()=>dom.rail.querySelector(`.stage-card[data-index="${stageIndex}"]:not(:disabled)`)?.focus());}
  ...
  dom.board.addEventListener("pointerup",finish);dom.board.addEventListener("pointercancel",e=>cancelDrawing(e.pointerId));dom.board.addEventListener("lostpointercapture",e=>cancelDrawing(e.pointerId));window.addEventListener("blur",()=>cancelDrawing());window.addEventListener("pagehide",()=>cancelDrawing());document.addEventListener("visibilitychange",()=>{if(document.hidden)cancelDrawing();});
