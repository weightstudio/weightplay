(() => {
  ...
  function setScreen(name){if(name!=="battle")cancelPendingResult();document.body.dataset.screen=name;dom.main.hidden=name!=="main";dom.guide.hidden=name!=="main";dom.stage.hidden=name!=="stage";dom.battle.hidden=name!=="battle";}
  ...
  function resetAttempt(resetCount=false){cancelPendingResult();if(resetCount)attempts=1;cancelDrawing();visited=[];visitedSet.clear();failed=false;hintUsed=false;renderBoard();updateHud();dom.feedback.textContent=text("ready");dom.assist.textContent="";dom.result.hidden=true;}
