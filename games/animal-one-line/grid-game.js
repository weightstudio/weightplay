(() => {
  ...
  function setScreen(name){if(name!=="battle"){cancelPendingResult();clearHint();closeResultOwnership();runClockPausedAt=0;}document.body.dataset.screen=name;dom.main.hidden=name!=="main";dom.guide.hidden=name!=="main";dom.stage.hidden=name!=="stage";dom.battle.hidden=name!=="battle";}
  ...
  function updateLesson(i){stageIndex=i;const ch=chapter(i);dom.lessonKicker.textContent=`${text("stage")} ${i+1} / 30`;dom.lessonTitle.textContent=text(`chapter${ch+1}`);dom.lessonRule.textContent=text(`rule${ch+1}`);dom.rail.querySelectorAll(".stage-card").forEach((c,n)=>{const selected=n===i;c.classList.toggle("selected",selected);if(selected)c.setAttribute("aria-current","true");else c.removeAttribute("aria-current");});}
  ...
  dom.rail.addEventListener("wonder:stage-snap",()=>scheduleStageCenterSync());window.addEventListener("resize",()=>scheduleStageCenterSync(140),{passive:true});
