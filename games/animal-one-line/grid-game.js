(() => {
  ...
  let stageIndex=0,attempts=1,visited=[],visitedSet=new Set(),drawing=false,activePointer=null,failed=false,startTime=0,lastPoint=null,hintUsed=false,leaveStartedAt=0,resultRevealTimer=null;
  ...
  function cancelPendingResult(){if(resultRevealTimer!==null){clearTimeout(resultRevealTimer);resultRevealTimer=null;}}
  ...
  function openBattleLeave(){if(document.body.dataset.screen!=="battle"||!dom.result.hidden||!dom.leave.hidden)return;cancelDrawing();leaveStartedAt=performance.now();dom.leaveTitle.textContent=text("leaveTitle");dom.leaveText.textContent=text("leaveText");dom.continuePuzzle.textContent=text("continuePuzzle");dom.returnStages.textContent=text("returnStages");setBattleDecisionCovered(true);dom.leave.hidden=false;requestAnimationFrame(()=>dom.continuePuzzle.focus());}
  function continuePuzzle(){if(dom.leave.hidden)return;if(startTime&&leaveStartedAt)startTime+=performance.now()-leaveStartedAt;leaveStartedAt=0;dom.leave.hidden=true;setBattleDecisionCovered(false);requestAnimationFrame(()=>dom.battleBack.focus());}
  function returnToStages(){if(dom.leave.hidden)return;leaveStartedAt=0;dom.leave.hidden=true;setBattleDecisionCovered(false);showStage(stageIndex);requestAnimationFrame(()=>dom.rail.querySelector(`.stage-card[data-index="${stageIndex}"]:not(:disabled)`)?.focus());}
