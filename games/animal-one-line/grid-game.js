(() => {
  ...
  let stageIndex=0,attempts=1,visited=[],visitedSet=new Set(),drawing=false,activePointer=null,failed=false,startTime=0,lastPoint=null,hintUsed=false,hintCell=null,hintTimer=null,hintDueAt=0,hintRemaining=0,resultRevealTimer=null,resultSettlementPending=false,resultActionCommitted=false,windowFocused=true,lifecycleSuspended=false,runClockPausedAt=0;
  ...
  function clearHint(){if(hintTimer!==null)clearTimeout(hintTimer);hintTimer=null;hintDueAt=0;hintRemaining=0;hintCell?.classList.remove("is-hint");hintCell=null;}
  function finishHint(){hintTimer=null;hintDueAt=0;hintRemaining=0;hintCell?.classList.remove("is-hint");hintCell=null;}
  ...
  function showHint(cell){clearHint();if(!cell)return;hintCell=cell;hintRemaining=1300;cell.classList.add("is-hint");if(!visited.length)setKeyboardCurrent(cell);armHintTimer();requestAnimationFrame(()=>dom.board.querySelector("#lineGrid")?.focus({preventScroll:true}));}
  ...
  function updateLesson(i){stageIndex=i;const ch=chapter(i),cards=[...dom.rail.querySelectorAll(".stage-card")],unlocked=cards.filter(c=>!c.disabled),tabOwner=unlocked.find(c=>Number(c.dataset.index)===i)||unlocked.sort((a,b)=>Math.abs(Number(a.dataset.index)-i)-Math.abs(Number(b.dataset.index)-i))[0];dom.lessonKicker.textContent=`${text("stage")} ${i+1} / 30`;dom.lessonTitle.textContent=text(`chapter${ch+1}`);dom.lessonRule.textContent=text(`rule${ch+1}`);cards.forEach((c,n)=>{const selected=n===i;c.classList.toggle("selected",selected);c.tabIndex=c===tabOwner?0:-1;if(selected)c.setAttribute("aria-current","true");else c.removeAttribute("aria-current");});}
  ...
  function moveStageFocus(key){const cards=[...dom.rail.querySelectorAll(".stage-card.unlocked:not(:disabled)")];if(!cards.length)return;const activeIndex=Math.max(0,cards.indexOf(document.activeElement)),rtl=getComputedStyle(dom.rail).direction==="rtl",step=key==="ArrowRight"?(rtl?-1:1):(rtl?1:-1),target=key==="Home"?cards[0]:key==="End"?cards.at(-1):cards[Math.max(0,Math.min(cards.length-1,activeIndex+step))];if(!target)return;const index=Number(target.dataset.index);clearTimeout(stageCenterTimer);centerStageCard(target);updateLesson(index);target.focus({preventScroll:true});}
  ...
  function showStage(focus=stageIndex){setScreen("stage");renderStages();updateLesson(Math.min(focus,(save.unlocked||1)-1));dom.stageSummary.textContent=text("summary",{cleared:Object.keys(save.cleared||{}).length,stars:Object.values(save.stars||{}).reduce((a,b)=>a+Number(b||0),0)});requestAnimationFrame(()=>{centerStageCard(dom.rail.querySelector(`[data-index="${stageIndex}"]`));scheduleStageCenterSync(180);});}
  function renderBoard(){const level=levels[stageIndex];dom.board.innerHTML=`<div id="lineGrid" class="line-grid" role="grid" tabindex="0" aria-label="${text("boardLabel")}" style="--rows:${level.rows};--cols:${level.cols}">${[...Array(level.rows)].flatMap((_,r)=>[...Array(level.cols)].map((__,c)=>{const open=level.open.has(`${r},${c}`),start=open&&level.solution[0][0]===r&&level.solution[0][1]===c,id=`lineCell-${r}-${c}`;return `<div id="${id}" class="line-cell ${open?"is-open":"is-blocked"} ${start?"is-start":""}" role="gridcell" data-row="${r}" data-col="${c}"${start?` aria-label="${text("startCell")}"`:""}>${start?'<span aria-hidden="true">🐾</span>':""}</div>`;})).join("")}</div>`;}
  function resetAttempt(recordRestart=false){cancelPendingResult();clearHint();if(recordRestart)attempts+=1;cancelDrawing();visited=[];visitedSet.clear();failed=false;hintUsed=false;renderBoard();updateHud();dom.feedback.textContent=text("ready");dom.assist.textContent="";closeResultOwnership();}
  ...
  function startStage(index){stageIndex=Math.max(0,Math.min(29,index));attempts=1;setScreen("battle");dom.stageLabel.textContent=`${text("stage")} ${stageIndex+1}`;dom.objective.textContent=text("ready");resetAttempt(false);startTime=performance.now();lifecycleSuspended=document.hidden||!windowFocused;runClockPausedAt=lifecycleSuspended?startTime:0;}
  ...
  function closeResultOwnership(){dom.result.hidden=true;setResultCovered(false);}
  function openResultOwnership(){resultActionCommitted=false;const terminal=stageIndex===29;dom.retry.disabled=false;dom.resultStages.disabled=false;dom.next.hidden=false;dom.next.disabled=terminal;dom.next.setAttribute("aria-disabled",String(terminal));dom.next.classList.toggle("primary-action",!terminal);dom.resultStages.classList.toggle("primary-action",terminal);dom.retry.classList.remove("primary-action");dom.result.scrollTop=0;setResultCovered(true);dom.result.hidden=false;requestAnimationFrame(()=>(terminal?dom.resultStages:dom.next).focus({preventScroll:true}));}
  function commitResultAction(action){if(resultActionCommitted||dom.result.hidden)return;resultActionCommitted=true;[dom.retry,dom.resultStages,dom.next].forEach(button=>{button.disabled=true;});action();}
  ...
  function openBattleLeave(){if(document.body.dataset.screen!=="battle"||!dom.result.hidden||!dom.leave.hidden)return;const settlementWasPending=resultSettlementPending||resultRevealTimer!==null;cancelPendingResult();resultSettlementPending=settlementWasPending;cancelDrawing();pauseRunClock();dom.leaveTitle.textContent=text("leaveTitle");dom.leaveText.textContent=text("leaveText");dom.continuePuzzle.textContent=text("continuePuzzle");dom.returnStages.textContent=text("returnStages");setBattleDecisionCovered(true);dom.leave.hidden=false;requestAnimationFrame(()=>dom.continuePuzzle.focus());}
  function continuePuzzle(){if(dom.leave.hidden)return;dom.leave.hidden=true;setBattleDecisionCovered(false);resumeRunClock();if(resultSettlementPending&&document.body.dataset.screen==="battle"){resultSettlementPending=false;openResultOwnership();return;}requestAnimationFrame(()=>dom.battleBack.focus());}
  function returnToStages(){if(dom.leave.hidden)return;cancelPendingResult();runClockPausedAt=0;dom.leave.hidden=true;setBattleDecisionCovered(false);showStage(stageIndex);requestAnimationFrame(()=>dom.rail.querySelector(`.stage-card[data-index="${stageIndex}"]:not(:disabled)`)?.focus());}
  ...
  const finish=e=>{if(!drawing||activePointer!==e.pointerId)return;sampleTo(e.clientX,e.clientY);cancelDrawing(e.pointerId);if(visited.length<levels[stageIndex].solution.length)dom.feedback.textContent=text("incomplete");};
  function suspendForeground(){lifecycleSuspended=true;cancelDrawing();pauseRunClock();suspendHint();}
  ...
  dom.start.addEventListener("click",()=>{showStage();if(!readStorage(TUTORIAL_KEY))dom.tutorial.hidden=false;});
  dom.stageBack.addEventListener("click",showMain);dom.battleBack.addEventListener("click",openBattleLeave);dom.continuePuzzle.addEventListener("click",continuePuzzle);dom.returnStages.addEventListener("click",returnToStages);dom.leave.addEventListener("keydown",trapBattleLeave);
  ...
  dom.rail.addEventListener("keydown",e=>{if(!["ArrowLeft","ArrowRight","Home","End"].includes(e.key)||!e.target.closest(".stage-card.unlocked"))return;e.preventDefault();moveStageFocus(e.key);});
  dom.rail.addEventListener("scroll",()=>scheduleStageCenterSync(),{passive:true});dom.rail.addEventListener("scrollend",syncStageSelectionToCenter);
  ...
  dom.hint.addEventListener("click",()=>{hintUsed=true;const next=levels[stageIndex].solution.find(([r,c])=>!visitedSet.has(`${r},${c}`));if(next)showHint(dom.board.querySelector(`[data-row="${next[0]}"][data-col="${next[1]}"]`));dom.feedback.textContent=text("hintText");});
  function showHighestUnlockedStage(){const target=Math.max(0,Math.min(29,(save.unlocked||1)-1));stageIndex=target;showStage(target);setTimeout(()=>{updateLesson(target);const card=dom.rail.querySelector(`.stage-card[data-index="${target}"]:not(:disabled)`);card?.scrollIntoView({behavior:"instant",inline:"center",block:"nearest"});card?.focus({preventScroll:true});},240);}
  dom.result.addEventListener("keydown",trapResult);dom.retry.addEventListener("click",()=>commitResultAction(()=>startStage(stageIndex)));dom.resultStages.addEventListener("click",()=>commitResultAction(showHighestUnlockedStage));dom.next.addEventListener("click",()=>commitResultAction(()=>startStage(Math.min(29,stageIndex+1))));
