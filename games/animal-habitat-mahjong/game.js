(() => {
  ...
  function fitCanvases() { const viewportWidth = visualViewport?.width || innerWidth; const viewportHeight = visualViewport?.height || innerHeight; const scale = Math.min(Math.max(1, viewportWidth) / LOGICAL_W, Math.max(1, viewportHeight) / LOGICAL_H); document.documentElement.style.setProperty("--scale", scale); document.documentElement.style.setProperty("--slot-w", `${LOGICAL_W * scale}px`); document.documentElement.style.setProperty("--slot-h", `${LOGICAL_H * scale}px`); }
