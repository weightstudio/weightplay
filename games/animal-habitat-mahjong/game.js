(() => {
  ...
  function fitCanvases() { const viewportWidth = visualViewport?.width || innerWidth; const viewportHeight = visualViewport?.height || innerHeight; const scale = Math.min(Math.max(1, viewportWidth - 8) / LOGICAL_W, Math.max(1, viewportHeight - 8) / LOGICAL_H); document.documentElement.style.setProperty("--scale", scale); document.documentElement.style.setProperty("--slot-w", `${LOGICAL_W * scale}px`); document.documentElement.style.setProperty("--slot-h", `${LOGICAL_H * scale}px`); }
  ...
  nodes.stageRail.addEventListener("pointerdown", (event) => { if (event.button !== 0) return; const targetCard = event.target.closest?.(".stage-card") || stageCardAtPoint(event.clientX, event.clientY); nodes.stageRail.style.scrollSnapType = "none"; drag = { id:event.pointerId, x:event.clientX, scrollLeft:nodes.stageRail.scrollLeft, moved:false, cardIndex:Number(targetCard?.dataset.index) }; nodes.stageRail.setPointerCapture?.(event.pointerId); }, true);
  document.addEventListener("pointermove", (event) => { if (!drag || drag.id !== event.pointerId) return; const scale = Number.parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--scale")) || 1; const delta = (event.clientX - drag.x) / scale; drag.moved ||= Math.abs(delta) > 8; nodes.stageRail.scrollLeft = drag.scrollLeft - delta; }, true);
  const settleStageRail = (event) => { if (!drag || drag.id !== event.pointerId) return; const { moved, cardIndex } = drag; suppressStageClick = moved; const next = Math.max(0, Math.min(habitats.length - 1, Math.round(nodes.stageRail.scrollLeft / 276))); drag = null; nodes.stageRail.style.scrollSnapType = "x mandatory"; nodes.stageRail.scrollTo({ left:next * 276, behavior:"smooth" }); syncStageSelection(next); if (event.type === "pointerup" && !moved && Number.isInteger(cardIndex)) { if (cardIndex < save.unlocked) startStage(cardIndex); else rejectLockedStage(cardIndex); return; } window.setTimeout(() => { suppressStageClick = false; }, 0); };
  nodes.stageRail.addEventListener("pointerup", settleStageRail); nodes.stageRail.addEventListener("pointercancel", settleStageRail); document.addEventListener("pointerup", settleStageRail, true); document.addEventListener("pointercancel", settleStageRail, true);
  nodes.stageRail.addEventListener("click", (event) => { if (suppressStageClick || (event.detail !== 0 && nodes.stageRail.dataset.wpStageRail !== "true")) return; const card = event.target.closest(".stage-card"); if (!card) return; const index = Number(card.dataset.index); if (index < save.unlocked) startStage(index); else rejectLockedStage(index); });
