(() => {
  ...
  const GAME_ID="animal-2048",GAME_VERSION="v18",INTERFACE_VERSION="6";
  ...
  function startStage(index){cancelResultReveal();discardMotion();stageIndex=Math.max(0,Math.min(INFINITE_INDEX,index));const level=activeLevel(),endless=level.type==="endless";score=0;moves=0;merges=0;rngState=endless?((Date.now()^Math.floor(performance.now()*1000))>>>0)||1:level.seed>>>0;board=endless?Array(16).fill(0):[...level.start];if(endless){spawnTile();spawnTile();}undoState=null;resultState="";hintUsed=false;restarts=0;startTime=performance.now();dom.leave.hidden=true;setBattleCovered(false);dom.feedback.textContent="";dom.announcement.textContent="";setScreen("battle");renderBattle();track("game_start",{stage:endless?"infinite":stageIndex+1,mode:endless?"endless":"mission"});requestAnimationFrame(()=>dom.board.focus({preventScroll:true}));}
  ...
  dom.undo.addEventListener("click",undo);dom.restart.addEventListener("click",restart);dom.hint.addEventListener("click",showHint);document.querySelectorAll("[data-direction]").forEach(button=>button.addEventListener("click",()=>performMove(button.dataset.direction)));dom.retry.addEventListener("click",()=>commitResultDecision(()=>{track("game_restart",{...resultAnalyticsPayload(),source:"result"});startStage(stageIndex);}));dom.resultStages.addEventListener("click",()=>commitResultDecision(()=>{track("game_result_menu",resultAnalyticsPayload());showStage(resultStageTarget());}));dom.next.addEventListener("click",()=>commitResultDecision(()=>{const payload=resultAnalyticsPayload(),nextStage=Math.min(29,stageIndex+1);track("game_next_stage",{...payload,next_stage:nextStage+1});startStage(nextStage);}));dom.tutorialClose.addEventListener("click",closeTutorial);dom.tutorialStart.addEventListener("click",()=>{closeTutorial();startStage(0);});
