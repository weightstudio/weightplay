(() => {
  ...
  function showHint(){if(battleDecisionBlocked())return;hintUsed=true;const direction=bestDirection();dom.feedback.textContent=direction?t("hintMove",{direction:t(direction)}):t("noHint");if(direction){const btn=$(`[data-direction="${direction}"]`);btn?.classList.add("is-hint");setTimeout(()=>btn?.classList.remove("is-hint"),1200);}}
