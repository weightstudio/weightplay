(function(){
  ...
  // The early upper routes place a direct step as high as 150 logical pixels
  // above the floor. Keep a generous margin so a shared Jump can reach it on a
  // narrow phone canvas instead of leaving the stage in an unwinnable state.
  ...
  const bodyNear = (body, point) => Math.hypot(body.x-point.x, (body.y-body.h/2)-point.y)<64;
  ...
  const renderStages=()=>{const count=unlocked();$("#stageGrid").innerHTML=stages.map((_,index)=>{const open=index<count;const selected=open&&index===current;const best=Number(storage(bestKey(index),0))||0;return `<button class="stage-card${open?"":" locked"}${selected?" selected":""}" type="button" data-stage="${index}" ${open?"":"disabled"}${selected?' aria-current="true"':''}><strong>${stageTitle(index)}</strong><small>${open?(best?`${text("bestStat")}: ${best}`:text("objective")):text("stageHelp")}</small><span aria-hidden="true">${open?"✦":"◌"}</span></button>`;}).join("");$("#stageGrid").querySelectorAll("[data-stage]").forEach((button)=>button.addEventListener("click",()=>startStage(Number(button.dataset.stage))));$("#stageTitle").textContent=text("stages");$("#stageEyebrow").textContent=text("eyebrow");$("#stageHelp").textContent=text("stageHelp");};
  ...
  const tick=(now)=>{const dt=Math.min(.032,Math.max(0,(now-last)/1000||0));last=now;if(!hidden){if(state&&!state.done){state.bodies.forEach((body)=>{if(state.jumpQueued&&body.ground){body.vy=JUMP_VELOCITY;body.ground=false;beep(500,.04);} });state.jumpQueued=false;update(dt);}draw();updateHud();}raf=requestAnimationFrame(tick);};
