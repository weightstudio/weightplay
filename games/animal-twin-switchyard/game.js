(function(){
  ...
  const setScreen = (screen) => { ["main","stage","battle","result"].forEach((name) => { $(`#${name}Screen`).hidden = name !== screen; }); document.body.dataset.screen = screen; };
  ...
  const draw = () => {
    ...
    data.switches.forEach((sw,index)=>{ctx.fillStyle=state?.switches.has(index)?"#78e2dc":"#ffd277";ctx.fillRect(sw.x-12,sw.y-25,24,25);ctx.fillStyle="#071627";ctx.fillRect(sw.x-3,sw.y-19,6,13);});
  ...
  const hold=(button,down,up)=>{button.addEventListener("pointerdown",(event)=>{event.preventDefault();button.setPointerCapture?.(event.pointerId);down();});["pointerup","pointercancel","lostpointercapture"].forEach((name)=>button.addEventListener(name,(event)=>{event.preventDefault();up();}));};hold($("#leftBtn"),()=>{axis=-1;state.moves+=1;},()=>{if(axis<0)axis=0;});hold($("#rightBtn"),()=>{axis=1;state.moves+=1;},()=>{if(axis>0)axis=0;});$("#jumpBtn").addEventListener("pointerdown",(event)=>{event.preventDefault();if(state&&!state.done){state.jumpQueued=true;state.moves+=1;}});
