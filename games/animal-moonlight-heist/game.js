(() => {
  ...
  const __wpReadMeasurement = () => ({ ...__wpMeasurement,
    ...
    paused: Boolean(state?.paused || state?.suspended || paused), node: document.body,
  ...
  const GAME_VERSION = 28, INTERFACE_VERSION = 6;
  ...
  copy.ar={title:"سرقة ضوء القمر الحيوان",internal:"مهمات أرشيف القمر",pitch:"اقرأ الدوريات واستعد الأثر واختر وقت الاستخراج.",start:"ابدأ اللعبة",missions:"أرشيف القمر",chooseGadget:"اختر الأداة",alert:"التنبيه",holdRoute:"اضغط باستمرار لمعاينة الطريق.",objective:"استعد كائن المهمة",locked:"أكمل المهمة السابقة",retry:"إعادة المحاولة",next:"المهمة التالية",victory:"اكتملت المهمة",captured:"تم اكتشافك",capturedText:"رفعت الدورية الإنذار. إعادة المحاولة مجانية.",treasure:"تم استعادة الكنز الإضافي",extraction:"الوصول إلى بوابة الاستخراج",dash:"البرق داش",decoy:"شرك النجم",smoke:"ورقة الدخان",dashEffect:"طريق سريع: تحرك {ms} مللي ثانية",decoyEffect:"توقف الدوريات: {seconds} ث",smokeEffect:"إعادة ضبط التنبيه + غطاء لمدة {seconds} ث",coins:"عملات القمر",safehouse:"البيت الآمن، المستوى {n}",mission:"المهمة {n}",move:"حرر للتحرك",found:"تم تأمين الكائن!",exitReady:"الاستخراج جاهز",treasureFound:"تم تأمين الكنز",paused:"متوقف مؤقتًا",diamonds:"ألماسات",reroll:"إعادة اختيار 3",insure:"تأمين 5",insured:"مؤمّن",alreadyInsured:"تأمين الاستخراج نشط بالفعل.",notEnough:"لا توجد ألماسات كافية.",rerolled:"تمت إعادة اختيار عروض الأدوات.",insuranceReady:"تأمين الاستخراج نشط للمهمة التالية.",confirmSpend:"تأكيد {cost} · {before}→{after}",rerollDecision:"أعد اختيار قوة الأدوات الثلاث. اضغط مرة أخرى للتأكيد: {before} ← {after} ألماسات.",insuranceDecision:"احتفظ بالكنز الإضافي بعد اكتشاف واحد في المهمة التالية. اضغط مرة أخرى للتأكيد: {before} ← {after} ألماسات.",rerollLabel:"إعادة اختيار قوة الأدوات الثلاث. التكلفة 3 ألماسات. الرصيد الحالي {balance}.",insuranceLabel:"تأمين الكنز الإضافي بعد اكتشاف واحد في المهمة التالية. التكلفة 5 ألماسات. الرصيد الحالي {balance}.",confirmLabel:"تأكيد {action}. إنفاق {cost} ألماسات. الرصيد من {before} إلى {after}.",rerollAction:"إعادة اختيار قوة الأدوات",insuranceAction:"تأمين الكنز",insuredLabel:"تأمين الكنز نشط للمهمة التالية.",routeExposed:"طريق أحمر: دخول مجال رؤية الدورية سيرفع التنبيه.",pickupCover:"تم تأمين الكائن — تحرك الآن بينما تتردد الدوريات!",notCleared:"لم تُستكمل",bestMedals:"أفضل نتيجة: {medals}/3 ميداليات",perfectMedals:"أفضل نتيجة: 3/3 ميداليات · اكتملت",bonusMedal:"الكنز الإضافي يمنح الميدالية الأخيرة.",medalCount:"تم الحصول على {medals} من 3 ميداليات",resultMedals:"{medals}/3 ميداليات",pauseAction:"إيقاف المهمة مؤقتًا",resumeAction:"استئناف المهمة",playFieldLabel:"منطقة طريق التخفي. استخدم WASD أو الأسهم للتحرك، واضغط المسافة لاستخدام الأداة.",playFieldPassiveLabel:"منطقة طريق التخفي. استخدم WASD أو الأسهم للتحرك. {name} تأثير سلبي: {effect}.",playFieldActiveLabel:"منطقة طريق التخفي. استخدم WASD أو الأسهم للتحرك، واضغط المسافة لاستخدام {name}: {effect}.",passiveGadgetLabel:"{name} تأثير سلبي: {effect}.",activeGadgetLabel:"استخدم {name}: {effect}.",passive:"سلبي",languageLabel:"اللغة",posterAlt:"ملصق لعبة سرقة ضوء القمر الحيوان",orlaAlt:"أورلا ذات القبعة القمرية",missionRailLabel:"اختيار المهمة",fiaAlt:"فيا ذات المخلب اللامع",stageBackLabel:"العودة إلى القائمة الرئيسية",battleBackLabel:"العودة إلى المهام",checkpoint:"نقطة تفتيش الحارس",firstSeal:"استعد ختم الكنز أولًا.",mirrorWarning:"تبديل المرآة قادم!",bellWarning:"نبضة الجرس قادمة — ادخل الظل!",clockSlow:"المرحلة الزرقاء: الدوريات بطيئة.",clockSurge:"المرحلة الكهرمانية: الدوريات تندفع!",guardianCleared:"اكتمل طريق الحارس"};
  Object.assign(copy.hi ||= {}, { victory: "मिशन पूरा हुआ" });
  Object.assign(copy.ar ||= {}, { victory: "اكتملت المهمة" });
  ...
  leaveCopy.ar={title:"مغادرة هذه المهمة؟",text:"مسارك الحالي متوقف مؤقتًا. واصل اللعب للحفاظ على الجولة، أو ارجع إلى المهام لمغادرة هذه الجولة.",continue:"مواصلة المهمة",leave:"العودة إلى المهام"};
  let leaveWasPaused=false;
  function createLeaveModal(){const modal=document.createElement("div");modal.id="battleLeaveModal";modal.className="modal";(__wpNotifyMeasurement(), modal.hidden=true);(__wpNotifyMeasurement(), modal.setAttribute("role","dialog"));(__wpNotifyMeasurement(), modal.setAttribute("aria-modal","true"));(__wpNotifyMeasurement(), modal.setAttribute("aria-labelledby","battleLeaveTitle"));(__wpNotifyMeasurement(), modal.setAttribute("aria-describedby","battleLeaveText"));modal.innerHTML='<section class="modal-card"><h2 id="battleLeaveTitle"></h2><p id="battleLeaveText"></p><div class="modal-actions leave-actions"><button id="battleContinueBtn" class="primary" type="button"></button><button id="battleLeaveBtn" type="button"></button></div></section>';nodes.modal.after(modal);nodes.leaveModal=modal}
  createLeaveModal();
  ...
  function playSound(name){window.WeightPlayAudio?.play?.(name)}
  function syncSoundToggle(){const button=$("#soundToggle");if(!button)return;const muted=Boolean(window.WeightPlayAudio?.isMuted?.());button.setAttribute("aria-pressed",String(muted));window.dispatchEvent(new Event("wonder:locale-change"))}
  ...
  function rerollOffers(){
    ...
    gadgetOffers=createOffers();gadget=gadgetOffers[0].id;economyMessage(t("rerolled"));renderGadgets();renderEconomy();updateGadget();playSound("reward.upgrade");
  }
  function buyInsurance(){
    ...
    insuranceActive=true;state.insuranceReady=true;save();economyMessage(t("insuranceReady"));renderEconomy();playSound("reward.upgrade");
  }
  ...
  function show(name){
    ...
    if(name!=="battle"){playing=false;(__wpNotifyMeasurement(), paused=false);stopBattleLoop();cancelPendingMovement();updatePauseControl()}
  ...
  function startMission(index,inputType="stage-card"){
    ...
    objectFound=false;treasureFound=preservedTreasure;preservedTreasure=false;caught=false;firstMoveTracked=false;alert=0;(__wpNotifyMeasurement(), paused=false);freezeUntil=0;smokeUntil=0;pickupCoverUntil=0;lastPulseCycle=-1;lastMirrorCycle=-1;guardianPhase=1;
    ...
    nodes.feedback.textContent=t("holdRoute");updateAlertMeter();$("#coinBattle").textContent=`${t("coins")}: ${state.coins}`;show("battle");playing=true;missionStartedAt=performance.now();playSound("game.start");startBattleLoop();
  ...
  function setPaused(next){if(!playing)return;(__wpNotifyMeasurement(), paused=Boolean(next));if(paused){freezeFia();freezePatrols();cancelPendingMovement();stopBattleLoop();nodes.feedback.textContent=t("paused")}else{patrols.forEach(p=>p.img.style.transitionDuration="");nodes.feedback.textContent=t("holdRoute");startBattleLoop()}updatePauseControl();if(!paused)nodes.field.focus({preventScroll:true})}
  ...
  function resolveArrival(){
    ...
    if(!treasureFound&&distance(p,point(nodes.treasure))<12){treasureFound=true;nodes.treasure.hidden=true;track("treasure_collected",{mission:selectedMission+1});showFx("pickup");nodes.feedback.textContent=t("treasureFound");playSound("reward.coin")}
    if(!objectFound&&distance(p,point(nodes.objective))<12){
      ...
      $("#objectiveLabel").textContent=t("extraction");updateExtractionCue();showFx("pickup");nodes.feedback.textContent=t(selectedMission<2?"pickupCover":"found");playSound("feedback.success");
    }
  ...
  function openResult(ok,medals=0,reward=0){
    ...
    (__wpNotifyMeasurement(), nodes.modal.hidden=false);
  ...
  function closeResult(){(__wpNotifyMeasurement(), nodes.modal.hidden=true);[...nodes.modal.parentElement.children].filter(node=>node!==nodes.modal).forEach(node=>{node.inert=false;node.removeAttribute("aria-hidden")});nodes.fia.classList.remove("caught")}
  ...
  function openBattleLeave(){if(!playing||!nodes.modal.hidden||!nodes.leaveModal.hidden)return;leaveWasPaused=paused;if(!paused)setPaused(true);const source=leaveCopy[locale]||leaveCopy.en,words=Object.fromEntries(Object.entries(source).map(([key,value])=>[key,runtimeText(value)]));$("#battleLeaveTitle").textContent=words.title;$("#battleLeaveText").textContent=words.text;$("#battleContinueBtn").textContent=words.continue;$("#battleLeaveBtn").textContent=words.leave;setBattleModalInert(nodes.leaveModal,true);(__wpNotifyMeasurement(), nodes.leaveModal.hidden=false);$("#battleContinueBtn").focus({preventScroll:true})}
  function closeBattleLeave(resume=true){if(nodes.leaveModal.hidden)return;(__wpNotifyMeasurement(), nodes.leaveModal.hidden=true);setBattleModalInert(nodes.leaveModal,false);if(resume&&!leaveWasPaused)setPaused(false);if(resume)$("#battleBackBtn").focus({preventScroll:true})}
  ...
  function bind(){
    ...
    if(soundToggle){
      ...
      soundToggle.addEventListener("click",()=>{window.WeightPlayAudio?.unlock?.();window.WeightPlayAudio?.setMuted?.(!window.WeightPlayAudio?.isMuted?.());syncSoundToggle()});
    }
  ...
  window.addEventListener("wonder:locale-change",async event=>{if(!event.detail?.locale)return;const requested=normalizeLocale(event.detail.locale);if(requested===locale)return;try{await ensureRuntimeCatalog(requested)}catch(error){console.error(error);return}locale=requested;writeOptionalStorage(localeKey,requested);localize()});
    ...
    $("#retryBtn").addEventListener("click",()=>{if(!claimResultAction())return;track("retry",{mission:selectedMission+1});closeResult();__wpReplayStart(() => startMission(selectedMission,"retry"))});
  ...
  if(location.hostname==="localhost"||location.hostname==="127.0.0.1"||new URLSearchParams(location.search).has("wp_test")){
    window.__ANIMAL_MOONLIGHT_HEIST_TEST__={
      ...
      showResult:(ok,medals=0,reward=0)=>{openResult(Boolean(ok),Number(medals)||0,Number(reward)||0);const __wpReturn = ({preview:document.querySelector("#nextMissionPreview")?.textContent||"",hidden:document.querySelector("#nextMissionPreview")?.hidden===true});
