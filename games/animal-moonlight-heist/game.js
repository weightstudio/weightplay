(() => {
  ...
  function economyMessage(message=""){$("#economyFeedback").textContent=message}
  ...
  function localize(){document.documentElement.lang=locale;document.title=`${t("title")} - Internal Trial`;document.querySelectorAll("[data-i18n]").forEach(n=>n.textContent=t(n.dataset.i18n));renderSummary();renderStage();renderEconomy();updateGadget()}
  ...
  function renderStage(){if(!nodes.rail)return;$("#coinLabel").textContent=`${t("coins")}: ${state.coins}`;nodes.rail.innerHTML="";missions.forEach((m,i)=>{const b=document.createElement("button");b.type="button";b.className=`mission-card${i+1>state.unlocked?" locked":""}`;b.innerHTML=`<img src="../../assets/animal-moonlight-heist-archive-background.png" alt=""><div><strong>${t("mission",{n:i+1})}: ${m.name[locale==="zh-Hant"?1:0]}</strong><span>${i+1>state.unlocked?t("locked"):(state.cleared[i]?"★★★":"☆☆☆")}</span></div>`;b.addEventListener("click",()=>{if(i+1<=state.unlocked)startMission(i)});nodes.rail.append(b)})}
  function renderGadgets(){const wrap=$("#gadgetChoices");wrap.innerHTML="";gadgetOffers.forEach(({id,level})=>{const g=gadgets[id],b=document.createElement("button");b.className=`gadget-choice${id===gadget?" selected":""}`;b.innerHTML=`<img src="../../assets/animal-moonlight-heist-gadget-${g.art}.webp" alt=""><span class="gadget-level">Lv.${level}</span>`;b.type="button";b.title=`${t(id)} Lv.${level}`;b.addEventListener("click",()=>{gadget=id;renderGadgets();updateGadget()});wrap.append(b)})}
  ...
  function startMission(index){selectedMission=index;objectFound=false;treasureFound=preservedTreasure;preservedTreasure=false;caught=false;alert=0;paused=false;smokeUntil=0;const m=missions[index];$("#missionLabel").textContent=`${t("mission",{n:index+1})}: ${m.name[locale==="zh-Hant"?1:0]}`;$("#objectiveLabel").textContent=t("objective");nodes.objective.src=`../../assets/animal-moonlight-heist-object-${missionObjects[index]}.webp`;place(nodes.objective,m.object);place(nodes.treasure,m.treasure);place(nodes.exit,m.exit);nodes.objective.hidden=false;nodes.treasure.hidden=treasureFound;nodes.exit.style.opacity=.5;place(nodes.fia,[50,88]);nodes.patrolLayer.innerHTML="";patrols=m.patrols.map((path,i)=>{const img=document.createElement("img");img.className="patrol";img.src=`../../assets/animal-moonlight-heist-patrol-${patrolArt[i%3]}.webp`;nodes.patrolLayer.append(img);const p={img,path,progress:i*.23,direction:1};updatePatrol(p);return p});nodes.feedback.textContent=t("holdRoute");nodes.alert.style.width="0";$("#coinBattle").textContent=`${t("coins")}: ${state.coins}`;show("battle");playing=true;lastTime=performance.now();requestAnimationFrame(loop)}
  ...
  function updatePatrol(p){const [x1,y1,x2,y2]=p.path;const q=p.direction>0?p.progress:1-p.progress;place(p.img,[x1+(x2-x1)*q,y1+(y2-y1)*q])}
  ...
  function openResult(ok,medals=0,reward=0){nodes.modal.hidden=false;$("#resultTitle").textContent=t(ok?"victory":"captured");$("#resultText").textContent=ok?`+${reward} ${t("coins")}${treasureFound?` · ${t("treasure")}`:""}`:t("capturedText");$("#medalRow").textContent=ok?"★".repeat(medals)+"☆".repeat(3-medals):"";$("#nextBtn").hidden=!ok||selectedMission>=4}
