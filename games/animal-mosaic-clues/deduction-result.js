(() => {
  ...
  const copy = {
    en:{time:"Time",mistakes:"Mistakes",hints:"Hints",alt:"Completed animal mosaic",row:"row",column:"column",filled:"filled",empty:"empty",boardState:"Puzzle state",rowClues:"Row clues",columnClues:"Column clues",grid:"Grid",legend:"Legend: ■ filled, × marked empty, ? unknown.",forced:"Hint: {line} {number} clue {clue} forces cell {cell} {state}.",none:"Hint: no single row or column is forced yet. Cross-check the intersecting clues."},
    "zh-Hant":{time:"時間",mistakes:"錯誤",hints:"提示",alt:"完成的動物拼圖",row:"橫列",column:"直行",filled:"填色",empty:"留白",boardState:"謎圖狀態",rowClues:"橫列線索",columnClues:"直行線索",grid:"格盤",legend:"圖例：■ 已填色、× 已標留白、? 未確定。",forced:"提示：第 {number} {line}的線索 {clue}，可確定第 {cell} 格為{state}。",none:"提示：目前沒有單一橫列或直行能直接確定；請交叉比對相交線索。"},
    "zh-Hans":{time:"时间",mistakes:"错误",hints:"提示",alt:"完成的动物拼图",row:"横行",column:"纵列",filled:"填色",empty:"留白",boardState:"谜图状态",rowClues:"横行线索",columnClues:"纵列线索",grid:"格盘",legend:"图例：■ 已填色、× 已标留白、? 未确定。",forced:"提示：第 {number} {line}的线索 {clue}，可确定第 {cell} 格为{state}。",none:"提示：目前没有单一横行或纵列能直接确定；请交叉比对相交线索。"},
    ja:{time:"時間",mistakes:"ミス",hints:"ヒント",alt:"完成した動物モザイク",row:"行",column:"列",filled:"塗る",empty:"空ける",boardState:"パズルの状態",rowClues:"行の手掛かり",columnClues:"列の手掛かり",grid:"盤面",legend:"凡例：■ 塗り、× 空き印、? 未確定。",forced:"ヒント：{number}{line}の手掛かり {clue} から、{cell}番目は「{state}」と確定します。",none:"ヒント：今は1つの行や列だけでは確定できません。交差する手掛かりを照合しましょう。"},
    ko:{time:"시간",mistakes:"실수",hints:"힌트",alt:"완성된 동물 모자이크",row:"행",column:"열",filled:"채움",empty:"비움",boardState:"퍼즐 상태",rowClues:"행 단서",columnClues:"열 단서",grid:"격자",legend:"범례: ■ 채움, × 빈칸 표시, ? 미확정.",forced:"힌트: {number}번째 {line}의 단서 {clue}로 {cell}번째 칸은 {state}으로 확정됩니다.",none:"힌트: 지금은 한 행이나 열만으로 확정할 수 없습니다. 교차 단서를 함께 확인하세요."},
    es:{time:"Tiempo",mistakes:"Errores",hints:"Pistas",alt:"Mosaico animal completado",row:"fila",column:"columna",filled:"rellena",empty:"vacía",boardState:"Estado del puzle",rowClues:"Pistas de filas",columnClues:"Pistas de columnas",grid:"Cuadrícula",legend:"Leyenda: ■ rellena, × vacía marcada, ? desconocida.",forced:"Pista: la {line} {number}, con clave {clue}, obliga a que la casilla {cell} esté {state}.",none:"Pista: ninguna fila o columna se resuelve por sí sola todavía. Cruza las pistas."},
    "pt-BR":{time:"Tempo",mistakes:"Erros",hints:"Dicas",alt:"Mosaico de animal concluído",row:"linha",column:"coluna",filled:"preenchida",empty:"vazia",boardState:"Estado do quebra-cabeça",rowClues:"Pistas das linhas",columnClues:"Pistas das colunas",grid:"Grade",legend:"Legenda: ■ preenchida, × vazia marcada, ? desconhecida.",forced:"Dica: a {line} {number}, com pista {clue}, determina que a célula {cell} fica {state}.",none:"Dica: nenhuma linha ou coluna está determinada sozinha. Cruze as pistas."},
    fr:{time:"Temps",mistakes:"Erreurs",hints:"Indices",alt:"Mosaïque animale terminée",row:"ligne",column:"colonne",filled:"remplie",empty:"vide",boardState:"État du puzzle",rowClues:"Indices des lignes",columnClues:"Indices des colonnes",grid:"Grille",legend:"Légende : ■ remplie, × vide marquée, ? inconnue.",forced:"Indice : la {line} {number}, avec l’indice {clue}, impose que la case {cell} soit {state}.",none:"Indice : aucune ligne ni colonne n’est encore déterminée seule. Croisez les indices."},
    de:{time:"Zeit",mistakes:"Fehler",hints:"Hinweise",alt:"Fertiges Tiermosaik",row:"Zeile",column:"Spalte",filled:"gefüllt",empty:"leer",boardState:"Rätselstand",rowClues:"Zeilenhinweise",columnClues:"Spaltenhinweise",grid:"Raster",legend:"Legende: ■ gefüllt, × als leer markiert, ? unbekannt.",forced:"Hinweis: In {line} {number} erzwingt die Vorgabe {clue}, dass Feld {cell} {state} ist.",none:"Hinweis: Noch ist keine einzelne Zeile oder Spalte eindeutig. Gleiche die Kreuzhinweise ab."},
    it:{time:"Tempo",mistakes:"Errori",hints:"Suggerimenti",alt:"Mosaico animale completato",row:"riga",column:"colonna",filled:"piena",empty:"vuota",boardState:"Stato del rompicapo",rowClues:"Indizi delle righe",columnClues:"Indizi delle colonne",grid:"Griglia",legend:"Legenda: ■ piena, × vuota segnata, ? sconosciuta.",forced:"Suggerimento: la {line} {number}, con indizio {clue}, impone che la casella {cell} sia {state}.",none:"Suggerimento: nessuna riga o colonna è ancora determinata da sola. Incrocia gli indizi."},
    ru:{time:"Время",mistakes:"Ошибки",hints:"Подсказки",alt:"Готовая мозаика с животным",row:"строка",column:"столбец",filled:"закрашена",empty:"пуста",boardState:"Состояние головоломки",rowClues:"Условия строк",columnClues:"Условия столбцов",grid:"Сетка",legend:"Обозначения: ■ закрашено, × отмечено пустым, ? неизвестно.",forced:"Подсказка: {line} {number} с условием {clue} определяет, что клетка {cell} — {state}.",none:"Подсказка: пока ни одна строка или столбец не определяется отдельно. Сопоставьте пересекающиеся условия."},
    hi:{time:"समय",mistakes:"गलतियाँ",hints:"संकेत",alt:"पूरा हुआ पशु मोज़ेक",row:"पंक्ति",column:"स्तंभ",filled:"भरा",empty:"खाली",boardState:"पहेली की स्थिति",rowClues:"पंक्ति संकेत",columnClues:"स्तंभ संकेत",grid:"ग्रिड",legend:"चिह्न: ■ भरा, × खाली चिह्नित, ? अज्ञात।",forced:"संकेत: {line} {number} का सुराग {clue} तय करता है कि खाना {cell} {state} है।",none:"संकेत: अभी कोई एक पंक्ति या स्तंभ अकेले तय नहीं होता। कटते हुए सुरागों को मिलाएँ।"},
  ...
  boardDescription.id="mosaicBoardDescription";
  ...
  canvas.insertAdjacentElement("afterend",boardDescription);
  ...
  function syncBoardDescription(engine){
    const signature=`${engine.size}:${engine.cells.flat().join("")}`;
    if(signature===boardDescriptionSignature)return;
    boardDescriptionSignature=signature;
    const rowClues=engine.solution.map((line,index)=>`${t.row} ${index+1}: ${runs(line).join(" ")}`).join("; ");
    const columnClues=Array.from({length:engine.size},(_,index)=>`${t.column} ${index+1}: ${runs(engine.solution.map(row=>row[index])).join(" ")}`).join("; ");
    const grid=engine.cells.map((line,index)=>`${t.row} ${index+1}: ${line.map(value=>value===1?"■":value===2?"×":"?").join(" ")}`).join("; ");
    boardDescription.textContent=`${t.boardState}. ${t.rowClues}: ${rowClues}. ${t.columnClues}: ${columnClues}. ${t.grid}: ${grid}. ${t.legend}`;
  }
  ...
  const tick=now=>{const current=state(),engine=current?.engine;if(engine?.kind==="mosaic"&&engine!==active){active=engine;recordFor(engine);last=now}if(engine?.kind==="mosaic"&&engine!==keyboardEngine){keyboardEngine=engine;keyboardActive=false;boardDescriptionSignature="";canvas.setAttribute("aria-label",boardLabel);canvas.tabIndex=0;canvas.setAttribute("aria-keyshortcuts","ArrowUp ArrowDown ArrowLeft ArrowRight Enter Space");canvas.setAttribute("aria-describedby","feedback mosaicBoardDescription")}if(engine===active&&current?.screen==="battle"&&result.hidden&&document.querySelector("#helpModal")?.hidden&&document.querySelector("#leaveModal")?.hidden&&!document.hidden)recordFor(engine).elapsed+=(now-last)/1000;if(current?.screen==="battle"&&engine===keyboardEngine){syncKeyboardCursor(engine);syncBoardDescription(engine)}else keyboardCursor.style.display="none";last=now;requestAnimationFrame(tick)};requestAnimationFrame(tick);
})();
