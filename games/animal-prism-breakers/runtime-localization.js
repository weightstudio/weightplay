(()=>{
  ...
  const resultRecapCopies={
    en:{direction:{left:"left",right:"right"},side:({score,combo,orbs,blocks,direction})=>`Score ${score}, best combo ${combo}, ${orbs} orbs left, ${blocks} crystals remain. Retry goal: move ${direction} earlier and meet the next return near the centre.`,remaining:({score,combo,orbs,blocks})=>`Score ${score}, best combo ${combo}, ${orbs} orbs left, ${blocks} crystals remain. Retry goal: clear one more crystal before the next orb falls.`,success:({score,combo,orbs,targetOrbs})=>`Score ${score}, best combo ${combo}, ${orbs} orbs left, 0 crystals remain. Retry goal: keep ${targetOrbs} orbs and beat combo ${combo}.`},
    "zh-Hant":{direction:{left:"左",right:"右"},side:({score,combo,orbs,blocks,direction})=>`分數 ${score}，最高連擊 ${combo}，剩下 ${orbs} 顆光球、${blocks} 顆水晶。重試目標：更早向${direction}移動，在中央附近接住下一次回彈。`,remaining:({score,combo,orbs,blocks})=>`分數 ${score}，最高連擊 ${combo}，剩下 ${orbs} 顆光球、${blocks} 顆水晶。重試目標：在下一顆光球落下前，再擊碎一顆水晶。`,success:({score,combo,orbs,targetOrbs})=>`分數 ${score}，最高連擊 ${combo}，剩下 ${orbs} 顆光球，水晶已清空。重試目標：保住 ${targetOrbs} 顆光球，並突破連擊 ${combo}。`},
    "zh-Hans":{direction:{left:"左",right:"右"},side:({score,combo,orbs,blocks,direction})=>`得分 ${score}，最高连击 ${combo}，剩下 ${orbs} 颗光球、${blocks} 颗水晶。重试目标：更早向${direction}移动，在中央附近接住下一次回弹。`,remaining:({score,combo,orbs,blocks})=>`得分 ${score}，最高连击 ${combo}，剩下 ${orbs} 颗光球、${blocks} 颗水晶。重试目标：在下一颗光球落下前，再击碎一颗水晶。`,success:({score,combo,orbs,targetOrbs})=>`得分 ${score}，最高连击 ${combo}，剩下 ${orbs} 颗光球，水晶已清空。重试目标：保住 ${targetOrbs} 颗光球，并突破连击 ${combo}。`},
    ja:{direction:{left:"左",right:"右"},side:({score,combo,orbs,blocks,direction})=>`スコア${score}、最高コンボ${combo}、残り光球${orbs}個、残りクリスタル${blocks}個。リトライ目標：${direction}へ早めに動き、次の返球を中央付近で受けよう。`,remaining:({score,combo,orbs,blocks})=>`スコア${score}、最高コンボ${combo}、残り光球${orbs}個、残りクリスタル${blocks}個。リトライ目標：次の光球が落ちる前に、もう1個クリスタルを壊そう。`,success:({score,combo,orbs,targetOrbs})=>`スコア${score}、最高コンボ${combo}、光球${orbs}個を残してクリスタルを全消去。リトライ目標：光球${targetOrbs}個を守り、コンボ${combo}超えを目指そう。`},
    ko:{direction:{left:"왼쪽",right:"오른쪽"},side:({score,combo,orbs,blocks,direction})=>`점수 ${score}, 최고 콤보 ${combo}, 남은 빛 구슬 ${orbs}개, 남은 수정 ${blocks}개. 재도전 목표: ${direction}으로 더 일찍 움직여 다음 반사를 중앙 근처에서 받으세요.`,remaining:({score,combo,orbs,blocks})=>`점수 ${score}, 최고 콤보 ${combo}, 남은 빛 구슬 ${orbs}개, 남은 수정 ${blocks}개. 재도전 목표: 다음 빛 구슬이 떨어지기 전에 수정 하나를 더 깨세요.`,success:({score,combo,orbs,targetOrbs})=>`점수 ${score}, 최고 콤보 ${combo}, 빛 구슬 ${orbs}개를 남기고 수정을 모두 깼습니다. 재도전 목표: 빛 구슬 ${targetOrbs}개를 지키고 콤보 ${combo}를 넘으세요.`},
    es:{direction:{left:"izquierda",right:"derecha"},side:({score,combo,orbs,blocks,direction})=>`Puntuación ${score}, combo máximo ${combo}, ${orbs} orbes restantes y ${blocks} cristales restantes. Objetivo de reintento: muévete antes a la ${direction} y recibe el siguiente rebote cerca del centro.`,remaining:({score,combo,orbs,blocks})=>`Puntuación ${score}, combo máximo ${combo}, ${orbs} orbes restantes y ${blocks} cristales restantes. Objetivo de reintento: rompe un cristal más antes de que caiga el siguiente orbe.`,success:({score,combo,orbs,targetOrbs})=>`Puntuación ${score}, combo máximo ${combo}, ${orbs} orbes restantes y ningún cristal. Objetivo de reintento: conserva ${targetOrbs} orbes y supera el combo ${combo}.`},
    "pt-BR":{direction:{left:"esquerda",right:"direita"},side:({score,combo,orbs,blocks,direction})=>`Pontuação ${score}, melhor combo ${combo}, ${orbs} orbes restantes e ${blocks} cristais restantes. Meta da nova tentativa: mova-se mais cedo para a ${direction} e receba o próximo rebote perto do centro.`,remaining:({score,combo,orbs,blocks})=>`Pontuação ${score}, melhor combo ${combo}, ${orbs} orbes restantes e ${blocks} cristais restantes. Meta da nova tentativa: quebre mais um cristal antes que o próximo orbe caia.`,success:({score,combo,orbs,targetOrbs})=>`Pontuação ${score}, melhor combo ${combo}, ${orbs} orbes restantes e nenhum cristal. Meta da nova tentativa: mantenha ${targetOrbs} orbes e supere o combo ${combo}.`},
    fr:{direction:{left:"gauche",right:"droite"},side:({score,combo,orbs,blocks,direction})=>`Score ${score}, meilleur combo ${combo}, ${orbs} orbes restants et ${blocks} cristaux restants. Objectif de la reprise : partez plus tôt vers la ${direction} et recevez le prochain rebond près du centre.`,remaining:({score,combo,orbs,blocks})=>`Score ${score}, meilleur combo ${combo}, ${orbs} orbes restants et ${blocks} cristaux restants. Objectif de la reprise : détruisez un cristal de plus avant la chute du prochain orbe.`,success:({score,combo,orbs,targetOrbs})=>`Score ${score}, meilleur combo ${combo}, ${orbs} orbes restants et aucun cristal. Objectif de la reprise : gardez ${targetOrbs} orbes et dépassez le combo ${combo}.`},
    de:{direction:{left:"links",right:"rechts"},side:({score,combo,orbs,blocks,direction})=>`Punktzahl ${score}, bester Combo ${combo}, ${orbs} Lichtkugeln und ${blocks} Kristalle übrig. Ziel für den Neustart: Bewege dich früher nach ${direction} und triff die nächste Rückkehr nahe der Mitte.`,remaining:({score,combo,orbs,blocks})=>`Punktzahl ${score}, bester Combo ${combo}, ${orbs} Lichtkugeln und ${blocks} Kristalle übrig. Ziel für den Neustart: Zerstöre einen Kristall mehr, bevor die nächste Lichtkugel fällt.`,success:({score,combo,orbs,targetOrbs})=>`Punktzahl ${score}, bester Combo ${combo}, ${orbs} Lichtkugeln übrig und keine Kristalle mehr. Ziel für den Neustart: Halte ${targetOrbs} Lichtkugeln und übertriff Combo ${combo}.`},
    it:{direction:{left:"sinistra",right:"destra"},side:({score,combo,orbs,blocks,direction})=>`Punteggio ${score}, serie migliore ${combo}, ${orbs} sfere e ${blocks} cristalli rimasti. Obiettivo del retry: spostati prima a ${direction} e ricevi il prossimo rimbalzo vicino al centro.`,remaining:({score,combo,orbs,blocks})=>`Punteggio ${score}, serie migliore ${combo}, ${orbs} sfere e ${blocks} cristalli rimasti. Obiettivo del retry: distruggi un cristallo in più prima che cada la prossima sfera.`,success:({score,combo,orbs,targetOrbs})=>`Punteggio ${score}, serie migliore ${combo}, ${orbs} sfere rimaste e nessun cristallo. Obiettivo del retry: conserva ${targetOrbs} sfere e supera la serie ${combo}.`},
    ru:{direction:{left:"влево",right:"вправо"},side:({score,combo,orbs,blocks,direction})=>`Счёт ${score}, лучшая серия ${combo}, осталось сфер света: ${orbs}, кристаллов: ${blocks}. Цель повтора: раньше двигайтесь ${direction} и встречайте следующий отскок ближе к центру.`,remaining:({score,combo,orbs,blocks})=>`Счёт ${score}, лучшая серия ${combo}, осталось сфер света: ${orbs}, кристаллов: ${blocks}. Цель повтора: разбейте ещё один кристалл до падения следующей сферы.`,success:({score,combo,orbs,targetOrbs})=>`Счёт ${score}, лучшая серия ${combo}, осталось сфер света: ${orbs}, кристаллов не осталось. Цель повтора: сохраните ${targetOrbs} сфер и превзойдите серию ${combo}.`},
    hi:{direction:{left:"बाईं ओर",right:"दाईं ओर"},side:({score,combo,orbs,blocks,direction})=>`स्कोर ${score}, सर्वश्रेष्ठ कॉम्बो ${combo}, ${orbs} प्रकाश गोले और ${blocks} क्रिस्टल बाकी हैं। दोबारा प्रयास का लक्ष्य: ${direction} पहले जाएँ और अगली वापसी को बीच के पास रोकें।`,remaining:({score,combo,orbs,blocks})=>`स्कोर ${score}, सर्वश्रेष्ठ कॉम्बो ${combo}, ${orbs} प्रकाश गोले और ${blocks} क्रिस्टल बाकी हैं। दोबारा प्रयास का लक्ष्य: अगला प्रकाश गोला गिरने से पहले एक और क्रिस्टल तोड़ें।`,success:({score,combo,orbs,targetOrbs})=>`स्कोर ${score}, सर्वश्रेष्ठ कॉम्बो ${combo}, ${orbs} प्रकाश गोले बचे और कोई क्रिस्टल नहीं बचा। दोबारा प्रयास का लक्ष्य: ${targetOrbs} प्रकाश गोले बचाएँ और कॉम्बो ${combo} से आगे जाएँ।`},
    ar:{direction:{left:"يسارًا",right:"يمينًا"},side:({score,combo,orbs,blocks,direction})=>`النقاط ${score}، أفضل سلسلة ${combo}، كرات الضوء المتبقية ${orbs} والبلورات المتبقية ${blocks}. هدف الإعادة: تحرّك ${direction} مبكرًا والتقِ بالارتداد التالي قرب الوسط.`,remaining:({score,combo,orbs,blocks})=>`النقاط ${score}، أفضل سلسلة ${combo}، كرات الضوء المتبقية ${orbs} والبلورات المتبقية ${blocks}. هدف الإعادة: حطّم بلورة إضافية قبل سقوط كرة الضوء التالية.`,success:({score,combo,orbs,targetOrbs})=>`النقاط ${score}، أفضل سلسلة ${combo}، تبقّت ${orbs} من كرات الضوء ولم تبقَ بلورات. هدف الإعادة: حافظ على ${targetOrbs} من كرات الضوء وتجاوز السلسلة ${combo}.`}
  };
  const ruleCopies={
  ...
  const resultRecap=({success=false,score=0,combo=0,orbs=0,blocks=0,side=null}={})=>{
    const recap=resultRecapCopies[locale]||resultRecapCopies.en,values={score:Number(score)||0,combo:Number(combo)||0,orbs:Number(orbs)||0,blocks:Number(blocks)||0,targetOrbs:Math.max(1,Number(orbs)||0)};
    if(success)return recap.success(values);
