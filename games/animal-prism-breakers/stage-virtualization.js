(()=>{
  ...
  const status=(stage,save)=>{const copy=window.PrismBreakersLocale?.status||{};return stage>save.unlocked?(copy.locked||"Locked"):save.cleared[stage]?(copy.clearedReplay||"Cleared · Replay"):(copy.ready||"Ready")};
  function createCard(){
    const button=document.createElement("button");
    button.type="button";button.className="stage-card";button.innerHTML="<small></small><strong></strong><span></span>";
    button.addEventListener("click",()=>{
      const stage=Number(button.dataset.stage);if(!stage)return;
      if(stage!==api.getCentered()){select(stage,true);return}
      if(stage>api.save.unlocked){api.announce(window.PrismBreakersLocale?.status?.lockedNotice||"Complete the previous stage first.");return}
      api.enter(stage);
    });
    return button;
  }
  ...
  function moveWindow(next){
    ...
    cards=[...rail.children];rail.dataset.wpStageWindowStart=String(windowStart);rail.dataset.wpStageWindowEnd=String(windowStart+POOL-1);if(recycled)rail.dataset.wpStageRecycleCount=String(Number(rail.dataset.wpStageRecycleCount||0)+recycled);
    return recycled;
  }
  ...
  function installDrag(){
    ...
    rail.addEventListener("pointerup",finish,true);rail.addEventListener("pointercancel",finish,true);rail.addEventListener("click",event=>{rail.dataset.wpStageClickCount=String(Number(rail.dataset.wpStageClickCount||0)+1);const card=event.target.closest?.(".stage-card")||document.elementFromPoint(event.clientX,event.clientY)?.closest?.(".stage-card"),stage=Number(card?.dataset.stage);if(!stage){rail.dataset.wpStageClickAction="no-stage";return}event.preventDefault();event.stopImmediatePropagation();if(suppressClick){rail.dataset.wpStageClickAction="suppressed";suppressClick=false;return}if(stage!==api.getCentered()){rail.dataset.wpStageClickAction="select";select(stage,true);return}if(stage>api.save.unlocked){rail.dataset.wpStageClickAction="locked";api.announce(window.PrismBreakersLocale?.status?.lockedNotice||"Complete the previous stage first.");return}rail.dataset.wpStageClickAction="enter";api.enter(stage)},true);
    rail.addEventListener("keydown",event=>{const rtl=getComputedStyle(rail).direction==="rtl",step=event.key==="ArrowRight"?(rtl?-1:1):event.key==="ArrowLeft"?(rtl?1:-1):0;let target=step?api.getCentered()+step:event.key==="Home"?1:event.key==="End"?TOTAL:0;if(target){event.preventDefault();event.stopImmediatePropagation();select(target,true)}else if((event.key==="Enter"||event.key===" ")&&api.getCentered()>api.save.unlocked){event.preventDefault();api.announce(window.PrismBreakersLocale?.status?.lockedNotice||"Complete the previous stage first.")}},true);
  }
