(() => {
  ...
  // Preloaded Shared PNG Textures
  const assets = {
    bg: new Image(),
    wolf: new Image(), // Lion explorer reused as hero placeholder
    bat: new Image(),  // Jaguar reused
    boar: new Image(), // Boar reused
    orb: new Image(),
    key: new Image(),
  };
  assets.bg.src = "../../assets/animal-relic-hunters-ruin-room.png";
  assets.wolf.src = "../../assets/animal-relic-hunters-lion-explorer.png";
  assets.bat.src = "../../assets/animal-relic-hunters-shadow-jaguar.png";
  ...
  const gearDb = {
    "sword-rare": { slot: "weapon", nameKey: "gear_sword_rare", typeKey: "rarity_rare", effectKey: "gear_sword_rare_desc", icon: "⚔️", bonusDmg: 4, bonusCrit: 0 },
    "dagger-epic": { slot: "weapon", nameKey: "gear_dagger_epic", typeKey: "rarity_epic", effectKey: "gear_dagger_epic_desc", icon: "🗡️", bonusDmg: 8, bonusCrit: 0.1 },
    "armor-rare": { slot: "armor", nameKey: "gear_armor_rare", typeKey: "rarity_rare", effectKey: "gear_armor_rare_desc", icon: "🛡️", bonusHp: 10 },
    "armor-epic": { slot: "armor", nameKey: "gear_armor_epic", typeKey: "rarity_epic", effectKey: "gear_armor_epic_desc", icon: "🥋", bonusHp: 20 },
    "boots-rare": { slot: "boots", nameKey: "gear_boots_rare", typeKey: "rarity_rare", effectKey: "gear_boots_rare_desc", icon: "🥾", bonusJump: 1.0, bonusSpeed: 0 },
    "boots-epic": { slot: "boots", nameKey: "gear_boots_epic", typeKey: "rarity_epic", effectKey: "gear_boots_epic_desc", icon: "👡", bonusJump: 1.5, bonusSpeed: 0.8 },
  };
  ...
  function handleLevelUp() {
    ...
    pool.forEach((relicId) => {
      ...
      let icon = "🔮";
      if (relicId === "relic_fang") icon = "🦷";
      else if (relicId === "relic_fur") icon = "🧣";
      else if (relicId === "relic_boots") icon = "🥾";
  ...
  function drawCanvasFrame() {
    ...
    state.pickups.forEach((pickup) => {
      if (pickup.type === "key") {
        ...
        ctx.fillRect(pickup.x - 16, pickup.y - 12, 32, 24);
        ctx.strokeStyle = "#eab308";
        ctx.lineWidth = 1.5;
        ctx.strokeRect(pickup.x - 16, pickup.y - 12, 32, 24);
      } else if (pickup.type === "portal") {
    ...
    state.enemies.forEach((enemy) => {
      ...
      if (enemy.type === "boss") {
        ...
        ctx.fillText("👹 BOSS", 8, 36);
      } else {
        const sprite = enemy.type === "boar" ? assets.boar : assets.bat;
        if (sprite.complete) {
    ...
    if (assets.wolf.complete) {
      ...
      ctx.drawImage(assets.wolf, 0, 0, state.width, state.height);
      ctx.globalAlpha = 1.0;
    } else {
    ...
    if (state.attackTimer > 0) {
      ...
      ctx.arc(ax, state.y + state.height / 2, 25, -Math.PI / 3, Math.PI / 3);
      ctx.stroke();
    }
