import {STAGES,TOOLS,ENEMIES,BOSS} from './game-data.js';
...
export function mission(stage){
 ...
 return {...s,req,sequence:s.id===27,protect:s.type==='survive',restricted:s.id===23};
}
export class Simulation {
 constructor(id,loadout=['bow','lantern','snare']){
  ...
  this.hero={x:0,z:6.5,hp:100,maxHp:100,angle:Math.PI,ward:0,boots:0,invulnerable:0,dash:0,recoil:0,attack:null};
 ...
 entity(kind,x,z,extra={}){const e={uid:this.nextId++,kind,x,z,hp:24,maxHp:24,r:.4,angle:0,recoil:0,...extra};this.ents.push(e);return e;}
 enemy(type,x,z,extra={}){const d=ENEMIES[type];return this.entity('enemy',x,z,{...d,type,maxHp:d.hp,cd:1+this.random(),wind:null,slow:0,stun:0,hidden:type==='sneak',...extra});}
 build(){
  const q=this.stage;
  const treePos=[[-4,5],[3.7,4.6],[-5.2,1.5],[5.1,1.1],[-4.8,-3.4],[4.5,-4.8],[-2.8,-6.5],[2.8,-6.4],[-6,4],[6,4],[-6,-1],[6,-2]];
  treePos.slice(0,Math.max(q.req.wood?Math.ceil(q.req.wood/2)+1:3,3)).forEach(([x,z],i)=>this.entity('tree',x,z,{hp:24,maxHp:24,r:.52,variant:i%3}));
  ...
  if(q.protect)this.entity('friend',0,-1,{hp:100,maxHp:100,r:.6});
  ...
  const count=q.id===1?1:q.protect?3:q.boss?0:q.req.kills||Math.min(6,2+Math.floor(q.id/6));
  for(let i=0;i<count;i++){const a=i*2.399+q.id*.4,r=3.5+i%2*1.5;this.enemy(q.enemies[i%q.enemies.length],Math.cos(a)*r,Math.sin(a)*r-2,{elite:q.id===29,...(q.id===29?{hp:75,maxHp:75}:{})});}
  if(q.boss){const d=BOSS[q.boss];for(let i=0;i<(d.count||1);i++)this.entity('boss',d.count?(i?2.4:-2.4):0,-3.5,{...d,type:q.boss,boss:q.boss,maxHp:d.hp,r:.8,cd:1.6,wind:null,phase:1,slow:0,stun:0,exposed:0,role:d.count?(i?'healer':'tank'):null,summonCd:5});}
 ...
 use(id){
  ...
  const h=this.hero;
  if(id==='bow'){const target=this.target(5.8,'bow');if(!target)return false;this.projectile(h,target,24,false,10);}
  if(id==='lantern'){this.alive().filter(e=>hostile(e)&&dist(e,h)<4.6).forEach(e=>{e.hidden=false;e.stun=1.2;e.wind=null;e.exposed=2;this.hit(e,16,'lantern');});this.log('pulse',{x:h.x,z:h.z});}
  ...
  if(id==='mastery'){this.alive().filter(e=>hostile(e)&&dist(e,h)<3.8).forEach(e=>{e.wind=null;e.exposed=2;this.hit(e,40,'mastery');});this.log('mastery',{x:h.x,z:h.z});}
 ...
 hit(e,power,source='axe'){
  ...
  if(e.hp===0){
   ...
   if(hostile(e)&&++this.combo%3===0)this.drops.push({uid:this.nextId++,kind:'heart',x:e.x,z:e.z});
 ...
 actEnemy(e,dt){
  e.recoil=Math.max(0,e.recoil-dt);e.exposed=Math.max(0,(e.exposed||0)-dt);e.slow=Math.max(0,e.slow-dt);
  ...
  const friend=this.alive().find(x=>x.kind==='friend'),target=friend&&dist(e,this.hero)>2.4?friend:this.hero;
  if(e.wind){const w=e.wind;w.left-=dt;if(w.left>0)return;e.wind=null;
   ...
   else if(w.kind==='heal'){const targets=this.alive().filter(x=>x!==e&&hostile(x)&&dist(e,x)<4);targets.forEach(x=>x.hp=Math.min(x.maxHp,x.hp+12));this.log('heal',{x:e.x,z:e.z});}
 ...
 step(dt,input={x:0,z:0}){
  ...
  let x=input.x||0,z=input.z||0,l=Math.hypot(x,z);if(l>1){x/=l;z/=l;}if(l>.1)h.angle=Math.atan2(x,z);
  ...
  if(h.attack){h.attack.left-=dt;if(h.attack.left<=0){const target=this.ents.find(e=>e.uid===h.attack.uid);if(target&&target.hp>0&&dist(h,target)<1.95&&this.clearLine(h,target))this.hit(target,16);h.attack=null;}}
  else if(this.cool.axe<=0){const target=this.target(1.65);if(target&&target.kind!=='shrine'){h.attack={uid:target.uid,left:.18,total:.18};h.angle=Math.atan2(target.x-h.x,target.z-h.z);this.cool.axe=.55;}}
  ...
  this.drops=this.drops.filter(d=>{if(dist(d,h)>1)return true;if(d.kind==='heart')h.hp=Math.min(100,h.hp+25);if(d.kind==='ward')h.ward=1;if(d.kind==='boots')h.boots=8;this.log('pickup',{kind:d.kind,x:d.x,z:d.z});return false;});
 ...
 snapshot(){return {stage:this.stage.id,time:this.time,hp:this.hero.hp,protectedHp:this.protectedHp,tally:{...this.tally},req:{...this.stage.req},won:this.won,lost:this.lost,ready:this.ready(),hero:{x:this.hero.x,z:this.hero.z},enemies:this.alive().filter(hostile).length,objects:this.alive().map(e=>({uid:e.uid,kind:e.kind,type:e.type,x:e.x,z:e.z,hp:e.hp,wind:e.wind?.kind})),cooldowns:{...this.cool}};}
}
