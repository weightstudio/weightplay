(() => {
  ...
  const text = {
    en: {
      ...
      resultWin: "Cleared Mission {mission}. Gained {xp} XP and unlocked progress.",
    ...
    "zh-Hant": {
      ...
      missionClear: "任務完成",
      missionFailed: "任務失敗",
      resultWin: "通過任務 {mission}，獲得 {xp} 經驗並推進進度。",
  ...
  const heroDefs = [
    { id: "lion", name: "lion", img: "animal-rune-tactics-hero-lion.webp", hp: 7, atk: 3, skill: "animal-rune-tactics-skill-lion-strike.webp" },
    { id: "owl", name: "owl", img: "animal-rune-tactics-hero-owl.webp", hp: 5, atk: 2, range: 2, skill: "animal-rune-tactics-skill-owl-rune-bolt.webp" },
    { id: "turtle", name: "turtle", img: "animal-rune-tactics-hero-turtle.webp", hp: 9, atk: 1, skill: "animal-rune-tactics-skill-turtle-guard.webp" },
  ];
  ...
  function loadProfile() {
    try {
      return { ...defaultProfile(), ...JSON.parse(localStorage.getItem(saveKey) || "{}") };
    } catch {
  ...
  function defaultProfile() {
    return { level: 1, xp: 0, bestMission: 1, unlockedMission: 1, training: false, bonusAtk: 0, bonusHp: 0, bonusEnergy: 0 };
  }
  ...
  function renderMenu() {
    ...
    for (let i = 1; i <= 4; i += 1) {
      ...
      btn.type = "button";
      btn.className = `mission-card ${selectedMission === i ? "is-active" : ""}`;
      btn.disabled = i > profile.unlockedMission;
      btn.innerHTML = `<strong>${t("missionCard", { n: i })}</strong><small>${i > profile.unlockedMission ? t("locked") : `${30 + i * 10} XP`}</small>`;
  ...
  function makeEnemies(mission) {
    const enemies = [
      { ...enemyDefs[0], x: 3, y: 0 },
      { ...enemyDefs[mission > 1 ? 1 : 0], x: 3, y: 2 },
    ];
    if (mission >= 3) enemies.push({ ...enemyDefs[2], x: 3, y: 1 });
    return enemies.map((e) => ({ ...e, maxHp: e.hp + mission, hp: e.hp + mission, atk: e.atk, team: "enemy" }));
  }
  ...
  function render() {
    ...
    for (let y = 0; y < rows; y += 1) {
      for (let x = 0; x < cols; x += 1) {
        ...
        if (unit) tile.appendChild(renderUnit(unit));
  ...
  function renderUnit(unit) {
    ...
    wrap.className = unit.team === "enemy" ? "enemy" : "hero";
  ...
  function renderSelected() {
    ...
    nodes.selectedCard.textContent = hero ? t("chooseTarget", { hero: t(hero.name), hp: hero.hp, maxHp: hero.maxHp, energy: hero.energy }) : t("chooseHero");
  }
  ...
  function showResult(win) {
    ...
    const xp = win ? 45 + state.mission * 15 : 12;
    profile.xp += xp;
    ...
    if (win) {
      ...
      profile.unlockedMission = Math.max(profile.unlockedMission, Math.min(4, state.mission + 1));
    }
    saveProfile();
    nodes.resultTitle.textContent = t(win ? "missionClear" : "missionFailed");
    nodes.resultText.textContent = t(win ? "resultWin" : "resultLose", { mission: state.mission, xp });
