(() => {
  ...
  const show = (screen) => { state.screen = screen; document.querySelectorAll("section[data-screen]").forEach((node) => { node.hidden = node.dataset.screen !== screen; }); document.body.dataset.screen = screen; window.dispatchEvent(new CustomEvent("weightplay:shell-sync")); };
  ...
  $("startBtn").addEventListener("click", openPatrolMap); $("mapBtn").addEventListener("click", openPatrolMap); $("stageBackBtn").addEventListener("click", () => show("main")); $("battleBackBtn").addEventListener("click", () => { show("stage"); renderStages(); }); $("resultMapBtn").addEventListener("click", () => { show("stage"); renderStages(); }); $("resultHomeBtn").addEventListener("click", () => { show("main"); applyLocale(); renderMain(); }); $("checkBtn").addEventListener("click", checkCode); $("clearBtn").addEventListener("click", clearCode); $("battleUtilityBtn").addEventListener("click", () => { announce("waiting"); $("battleUtilityBtn").setAttribute("aria-pressed", "true"); }); $("localeSelect").addEventListener("change", (event) => { state.locale = locales[event.target.value] ? event.target.value : "en"; try { localStorage.setItem("weightplayLocale", state.locale); } catch (_) {} applyLocale(); window.dispatchEvent(new CustomEvent("wonder:locale-change", { detail: { locale: state.locale } })); });
  ...
  $("stageBackBtn").addEventListener("click", applyLocale);
  applyLocale(); window.dispatchEvent(new CustomEvent("wonder:locale-change", { detail: { locale: state.locale } })); show("main");
